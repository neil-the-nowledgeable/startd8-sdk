...
| CL-8 | `dashboard_creator/perses/` — bounded Perses adoption | Neutral model, deterministic dual lowerings, pinned CUE oracle, live `dashboard create --target perses` surface, and a byte-stable validated pilot artifact are implemented; Dash0 import remains unperformed | **L3** | Dash0 team follows `docs/design/dashboard-vendor-neutrality/READY_FOR_DASH0_TEAM.md`, imports the exact pilot artifact, and returns sanitized evidence → L4/L5 | Vendor-neutral dashboard generation is proven at its first real consumer |
