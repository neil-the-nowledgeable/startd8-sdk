...
| CL-8 | `dashboard_creator/perses/` — bounded Perses adoption | Neutral model, dual lowerings, pinned CUE/live CLI (`--target perses`, #492), pilot golden — and **G3 CLOSED (2026-08-21): 14/14 SDK-generated dashboards imported into Dash0 (`dev`) with zero manual edits and live-rendering data.** Bare `kind: Dashboard` accepted as-is; consumer adds the `perses.dev/v1alpha1` `PersesDashboard` envelope on save (so bare emit is correct — SDK must NOT wrap). `spec.*` matched Dash0's stored shape key-for-key. | **L4** | Residual is ergonomics/fidelity, not correctness: bake the **Dash0 metric-name/backend profile** (G3-SDK-2 — gates *live* rendering; today a consumer-side retarget) + confirm native emit is artifact-free (G3-SDK-3/4) → L5. Spec: `docs/design/dashboard-vendor-neutrality/REQ-dash0-metric-name-profile.md`; results: `docs/G3_DASH0_PERSES_IMPORT_RESULTS_2026-08-21.md`. | Vendor-neutral dashboard generation **proven at its first real consumer** |
