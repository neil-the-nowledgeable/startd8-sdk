...
- **FR-5 — Tabbed cockpit.** Extend `build_workbook_v2` into a `TabsLayout` board with three tabs: **Status** (the current audience-personalized rows, unchanged), **Assistant** (FR-6), **Proposals** (FR-7). Reuses shipped v2 constructs (M0–M5); keeps the `-v2` UID (`workbook_v2_uid`).
  - **Construction primitive (implementation note).** Built directly with the shipped `TabsLayout`/`TabsLayoutTab`/`RowsLayoutRow` primitives — **not** the `build_sectioned_v2`/`Section` convenience wrapper. `Section` supports only a single tab-level `show_when`; FR-8 requires **intra-tab per-row conditional rendering** (Assistant: transcript always-shown + Loki row hidden-for-Beginner; Proposals: full-table hidden + simplified shown-for-Beginner), which only the lower-level primitives can express. Same semantic outcome (a 3-tab audience-conditional cockpit).
...
- **FR-7 — Proposals tab (queue + confirm affordance).** Renders pending proposals (from FR-2) as a **baked (markdown) table inside a `text` panel** — kind, target/`value_path`, one-line summary, proposal `id`. (A live Grafana `table`-viz panel would require a datasource query or an inline data frame, which contradicts the FR-8 embed/bake / NR-1 no-live-datasource posture; the markdown table embeds the same tabular content deterministically.) Each row shows the **exact CLI command** to review + apply the pending inbox: `startd8 vipp negotiate && startd8 vipp apply --apply`. **Honest about blast radius:** VIPP adjudicates + applies at the **envelope** level (there is no per-proposal apply), so the command acts on the *whole pending inbox* — and it is deliberately **interactive** (no `--yes`), so VIPP prompts the human to confirm **each** proposal at apply time (nothing auto-applies; preserves propose→confirm). **The command must be copy-safe and id-bound (R1-F4):** any `value_path`/target is shell-escaped, and a trailing `# startd8-proposal: id=… kind=… path=…` annotation **binds the row to its inbox entry for traceability** (parsed back by `parse_confirm_command`); it identifies, but does not scope, the action. **AC:** render the command for proposal `P`, parse it back, assert the parsed `id == P.id` (fixture includes a `value_path` with spaces/quotes). **No write action in the dashboard** (NR-2); the surface never renders `--yes`.
