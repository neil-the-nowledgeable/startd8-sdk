...
### Live admin toggle (FR-29 — built; un-defers NR-6)

NR-6 (the live, no-restart, DB-backed admin toggle) is now built, reconciled with the shipped
config-file model (FR-6): **a key is hidden if the config file *or* the DB says so (config ∪ DB)**.

- **FR-29a — Presence-based system table.** A generated `app/nav_store.py` owns a `nav_hidden(key,
  updated_at)` table — *presence = hidden* — created by an idempotent `CREATE TABLE IF NOT EXISTS`
  bootstrap run at startup (raw SQL via `text()`, **not** a SQLModel model). It is therefore **not** in
  the user contract and is decoupled from `create_all`/alembic, so it works byte-identically in both
  deployment modes (the v0.2 parked design).
- **FR-29b — Admin UI.** `app/nav_admin.py` serves `GET/POST /admin/nav`: a form listing every nav
  entry with a visible/hidden checkbox (reflecting config ∪ DB). POST upserts/deletes `nav_hidden` rows
  and refreshes the resolved nav. `app/templates/nav_admin.html` renders it (grouped, accessible).
- **FR-29c — Mode-aware auth, mode-INVARIANT bytes.** The admin router gates with `require_principal`
  via a **tolerant import** (`try: from .auth import require_principal`): enforced in deployed (auth.py
  present), open in installed (auth.py absent) with a loud unauthenticated banner. installed binds
  loopback, so the open admin is local-only. Same generated bytes in both modes → schema-only owned
  kind, no mode-aware drift machinery.
- **FR-29d — Live refresh.** The POST handler recomputes `app.state.nav` after writing, so the toggle
  takes effect immediately for the serving process — the existing nav/index/counts consumers
  (`request.app.state.nav`) are unchanged. *Limitation:* `app.state` is per-worker, so under multiple
  workers a toggle propagates to other workers on their next refresh/restart (eventual consistency).
  Upgrade path: per-request resolution (one tiny indexed query per HTML render) — deferred.
- **FR-29e — Discoverable.** The index page links the admin surface ("Manage navigation →").
- Owned schema-only kinds `nav-store` / `nav-admin-router` / `nav-admin-page`; emitted with the nav
  family (suppressed by `--no-nav`).

Related strategic item (separate effort): a richer generated home/index (e.g. per-entity record
counts shipped in FR-28f; further dashboards out of scope).
