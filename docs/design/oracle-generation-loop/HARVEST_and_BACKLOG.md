...
| **OL-EB-5** ✅ **DONE** | S | **A one-shot that CANNOT LAUNCH was mis-scored `fail` (model-fault) instead of degrading to `error` (infra/env).** The $0 Phase-1 dry-run surfaced it: `pytest tests/test_health.py` against `fixtures/otel-demo/email-py` returned rc 1 with a pytest **bootstrap** traceback (pytest not in that app's requirements) and the runner classified it `fail` — in the real-LLM Phase-2 loop that would wrongly trigger a regenerate for a harness gap (violating the benchmark-matrix "infra-fail ≠ the model's catastrophic 0" rule). **Fix:** exit-code-aware split in `_run_oneshot` — rc 127 (command-not-found, ANY one-shot) → `error`; a **pytest** invocation (`pytest …` or `python[3] -m pytest …`) applies pytest exit-code semantics: rc 1 = tests failed → `fail`, rc {2,3,4,5} (interrupted/internal/usage/no-tests) → `error`, and rc 1 with a bootstrap signature in stderr (`ModuleNotFoundError`/`ImportError`/`No module named`/`_pytest`) → `error`; every other one-shot rc != 0 → `fail` (unchanged). The existing `SandboxResult.violation` → `error` degrade path (timeout/launch/resource) is untouched. | `runner._classify_oneshot` / `_run_oneshot`; tests in `test_runner.py` (rc5, rc1-real-fail, rc1-bootstrap, rc127, rc0) |
...
---
*HTH harvest of the oracle-generation loop. 1 High security-hardening applied (H1); standard extracted (effect-injected loop core); 1 dormant path + 3 enhancement rows filed; OL-EB-5 (infra-vs-model one-shot classification, surfaced by the $0 email-py dry-run) landed. Surface held up well — the CRP + reflective passes front-loaded most of the risk.*
