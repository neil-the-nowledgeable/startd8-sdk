...
- **Baggage is strictly necessary only for real-time-at-ingest levers.** Sampling retention, sink-tiering, and live alerting need `business.flow` in-band (baggage). The analysis/RCA levers can instead derive flow post-hoc from the assembled trace's root-span entry route (trace-derivation); a hybrid reads the SAME single-sourced map, so FR-6's propagation-coverage check is baggage-path-only (FR-6).
- **Fidelity is a third guard, distinct from consistency and reach.** FR-1 guards the map's consistency and FR-6 its reach; neither checks whether the map still matches the running system. FR-9 adds a drift-guard that reconciles the declaration against route-discovered reality (via Weaver `live-check`) — discovery is an adversarial oracle only, never an authoring source (NR-7).
...
| reliability | A stale/wrong `route→flow` map runs RCA de-blending backwards — confidently-wrong results while FR-1 (consistency) and FR-6 (reach) both pass | FR-9: map-fidelity gate reconciles declared flow against route-discovered flow (Weaver `live-check`), alarms on divergence + coverage drift | high |
...
- **FR-6 — Propagation-coverage check (baggage-carrier-specific).** A coverage/liveness probe verifies the flow tag actually reaches downstream spans across the call graph *via the baggage carrier*; a hop that drops it is surfaced as a loud gap (the propagation analogue of the coverage RCA). This check is specific to the baggage path — `business.flow` is ALSO recoverable post-hoc from the assembled trace's root span (trace-derivation), for which the analogue is trace-assembly completeness, not propagation-coverage; both carriers read the SAME single-sourced flow map. Name: A coverage probe verifies the flow tag reaches downstream spans over the baggage carrier and a dropped hop is a loud gap. Touches: `ContextCore/src/contextcore/coverage/`, a propagation-coverage check. Lives: code ContextCore/coverage/flow_propagation. Approve?: is flow-tag propagation coverage verified end-to-end over the baggage carrier?. Verify: a service missing the propagator yields a propagation-coverage gap for flows routed through it; a fully-propagating path yields none. Serves: O-2
...
- **FR-9 — Map-fidelity reconciliation gate (drift-guard on the declaration).** A fidelity gate samples a bounded window of real assembled traces and independently re-derives each trace's flow from an ORTHOGONAL signal — the root span's entry route (`http.route`/`rpc.method`), present with zero baggage — then test-classifies that discovered flow against the DECLARED `route→flow` map and alarms when the declared-vs-discovered divergence rate exceeds ε or when unmapped-route coverage drifts, so a stale/wrong map (which runs the RCA de-blending backwards while FR-1's consistency check and FR-6's reach check both pass) fails loud rather than producing confidently-wrong results; operationalized by OTel Weaver's `weaver registry live-check` reconciling the live `business.*` values against ContextCore's declared `business.yaml` registry (the reserved Phase-2 group in `ContextCore/semconv/registry_manifest.yaml`) plus the route→flow map; discovery is used ONLY as an adversarial oracle sampling reality to ask "does the declaration still match the world?", never to author or override a value (identical to the REQ-22 verify-liveness framing). Name: A map-fidelity gate reconciles declared flow against route-discovered flow and fails loud on divergence or coverage drift. Touches: `ContextCore/semconv/registry_manifest.yaml`, `ContextCore/semconv/business.yaml`, `ContextCore/src/contextcore/coverage/`, `weaver registry live-check` config, flow-map CRD/config. Lives: code ContextCore/coverage/flow_fidelity. Approve?: is the declared route→flow map reconciled against live route-discovered flow with divergence and coverage-drift alarms?. Verify: `/cart/checkout` declared `browse` (or a real live route absent from the map) trips the divergence/coverage-drift alarm via `weaver registry live-check`; a map where every sampled root-route maps to its declared flow passes with no alarm. Serves: O-2
...
- **NR-7:** Does NOT let discovery author or override a declared value — the FR-9 route-discovery step is an adversarial DRIFT-GUARD ON the declaration (an oracle sampling reality to test the map), never a source that mints, back-fills, or corrects `business.flow`/criticality. The "declared, not discovered" discipline holds: a divergence alarm prompts a human to fix the declaration, it does not auto-reconcile.
