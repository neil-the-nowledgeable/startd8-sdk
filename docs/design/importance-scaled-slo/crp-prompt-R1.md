...
**Generated:** 2026-07-22 13:58:50 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/importance-scaled-slo/IMPORTANCE_SCALED_SLO_PLAN.md` | 101 lines · 885 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/importance-scaled-slo/REQ_IMPORTANCE_SCALED_SLO_THRESHOLDS.md` | 220 lines · 2015 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/importance-scaled-slo/crp-focus.md` | 32 lines · 292 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/importance-scaled-slo/IMPORTANCE_SCALED_SLO_PLAN.md`  ·  **Size:** 101 lines · 885 words
...
|---|---|---|---|
| 1.1 | `observability/artifact_generator_generators.py:40` | Add `_IMPORTANCE_THRESHOLDS: Dict[Tuple[str, Optional[str]], Dict[str,str]]` keyed on `(criticality, deployment_mode)`; keep `_DEFAULT_THRESHOLDS` as the no-signal fallback | FR-2, FR-7 |
...
| 1.5 | `tests/unit/observability/test_artifact_generator.py` | Cases: `high` ⇒ tighter than flat; `medium` ⇒ unchanged; authored `spec.requirements.availability` still wins (tier=manifest); byte-identical across two runs | FR-2/3/9, NR-1 |
...
| 2.2 | `observability/artifact_generator_context.py:376` (`_build_context`) | Populate `ctx.deployment_mode` from `spec.deployment.mode` (manifest) | FR-1, FR-4 |
| 2.3 | **ContextCore** `models/core.py` (new `DeploymentSpec`; add to spec) | Optional `spec.deployment.mode` enum (`installed\|deployed`) with a `field_validator` | FR-4, Keiyaku |
| 2.4 | **ContextCore** `cli/init_from_plan_ops.py:514` (`infer_init_from_plan`) | Add cue regex → set `spec.deployment.mode` + `add_inference(..., "plan:deployment_mode_cue", conf)`; absent/ambiguous ⇒ unset | FR-5 |
...
| 2.6 | table (1.1) | Extend `_IMPORTANCE_THRESHOLDS` keys to real `(criticality, deployment_mode)` pairs per OQ-A | FR-2 |
...
- Increment 2 spans **ContextCore** (2.3, 2.4) and **startd8-sdk** (2.1, 2.2, 2.5). Land the SDK
  `BusinessContext.deployment_mode` read (2.1/2.2) first (tolerant of absent field), then the
  ContextCore manifest field + cue (2.3/2.4) — so neither repo hard-depends on an unreleased other.
- No change to the manifest↔generator contract beyond one **optional** field ⇒ backward compatible.

## Risks / guards

- **Provenance laundering (NR-4):** enforce via a test asserting derived thresholds never appear
  under `tier="manifest"` and are never written to the emitted `.contextcore.yaml`.
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/importance-scaled-slo/REQ_IMPORTANCE_SCALED_SLO_THRESHOLDS.md`  ·  **Size:** 220 lines · 2015 words
...
- **FR-1 — `deployment_mode` in generator context.** `BusinessContext` gains
  ...
  it. `None` ⇒ criticality-only scaling (a defined path, never a crash or silent flat default).
...
- **FR-3 — Honest provenance.** An importance-scaled default emits a `DerivationTrace` with a
  distinct tier (`default:importance`) and a transformation string recording the mapping
  (e.g. `"deployed + high → availability 99.9"`). Consumers and diff-review can see it was
...
- **FR-5 — init-from-plan cue inference.** `infer_init_from_plan` infers `deployment_mode` from
  ...
  inference + confidence like it does for criticality. Absent/ambiguous cue ⇒ **unset** (recorded),
...
- **NR-4** — Does **not** write derived thresholds back into the manifest (would masquerade as
  authored; violates provenance honesty).
...
## 4. Open Questions (remaining for CRP)

- **OQ-A — The exact number table.** What are the defensible `(criticality × mode)` values?
  Proposed starting point (overridable, FR-7): `deployed+critical → 99.95/200ms`,
  `deployed+high → 99.9/300ms`, `deployed+medium → 99.5/500ms`, `installed+* → 99/1s`,
  `no-signal → flat 99/500ms`. Needs an SRE sanity pass — deferred to CRP / config, not code.
- **OQ-B — Should `throughput` scale by importance at all?** Availability/latency clearly should;
  throughput is more a capacity fact than an importance fact. Candidate: scale availability+latency,
  leave throughput flat unless authored.
- **OQ-C — Confidence threshold for FR-5.** Below what inference confidence do we leave
  `deployment_mode` unset rather than guess `deployed`? (Guessing `deployed` tightens SLOs — a
  conservative-vs-eager tradeoff.)
