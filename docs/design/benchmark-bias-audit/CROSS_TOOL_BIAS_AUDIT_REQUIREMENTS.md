...
- **Date**: 2026-06-17 21:09:52 UTC
- **Scope**: Architecture-focused review

| ID | Area | Severity | Suggestion | Rationale | Proposed Placement | Validation Approach |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| R3-S1 | Ambiguity | Critical | Add an explicit decision rule for mixed evidence when FR-4, FR-5, FR-6, FR-7, and FR-11 disagree: specify which artifact or signal takes precedence, what counts as a tie, and when the result must default to 'ambiguous-flagged' rather than 'neutral' or 'biased-and-corrected'. | The current verdict rubric names all the signals, but it does not define a strict precedence order or tie-breaker. That leaves the audit vulnerable to post hoc interpretation when one analysis suggests neutrality while another suggests vendor-author bias or tool-capability issues. | FR-8 — Bias-audit report, immediately after the verdict criteria. | Exercise the rule against at least three synthetic case bundles where FR-4/FR-5/FR-6 point in different directions and confirm reviewers independently reach the same outcome. |
| R3-S2 | Ambiguity | High | Define what qualifies as a 'material' OPEN item, 'significant' divergence, and 'threshold-meeting' own-vendor advantage with explicit numeric or categorical cutoffs. | Several requirements hinge on these terms, but their meaning is currently left to reviewer judgment. Without a shared threshold, the same evidence can be classified differently across runs or reviewers, undermining comparability. | FR-1, FR-5, FR-6, and FR-8 as a shared glossary or decision-threshold note. | Run a calibration exercise on borderline examples and verify that reviewers apply the same thresholds consistently. |
| R3-S3 | Ambiguity | High | Specify whether 'unanimity' in FR-7 and FR-8 requires unanimity among accepted samples, among reviewers, or across all tools and samples, and state how partial agreement is recorded. | The document uses unanimity in multiple contexts, but the referent shifts between tool outputs and human adjudicators. This creates ambiguity in the strongest neutral signal and in the verdict criteria. | FR-7 — Attribution via unanimity rule, and FR-8 — Bias-audit report. | Have a reviewer classify a set of edge cases with 3-way tool splits and 2-of-3 reviewer agreement to ensure the term is interpreted consistently. |
| R3-S4 | Ambiguity | Medium | Clarify the scope of 'model-vendor' versus 'author-vendor' in FR-6 when the evaluated model and authoring tool come from the same vendor but differ in product line or release family. | Cross-vendor and same-vendor comparisons are central to the bias claim, but the current terminology may allow inconsistent grouping when a vendor has multiple model families or tooling lines. | FR-1.1 Terminology and FR-6 — Score-impact via model×spec interaction. | Test the taxonomy against at least one multi-product vendor example and verify that each cell is assigned unambiguously. |
| R3-S5 | Consistency | High | Make the acceptance/exclusion policy consistent across FR-3a, FR-3b, FR-11, and FR-8 by stating whether catastrophic failures are excluded from all downstream analyses or only from semantic-bias conclusions, and ensure the same rule is used in every metric. | The document currently allows catastrophic failures to count for tool-capability reporting but not semantic bias calls, while FR-11 still uses them in divergence statistics. That creates the risk of mixing denominators and producing incomparable rates. | FR-3b, FR-11, and FR-8 verdict criteria. | Recompute the same pilot dataset under the documented rule and confirm every table and statistic uses the same inclusion/exclusion basis. |
| R3-S6 | Consistency | Medium | Standardize the term used for the primary artifact type in FR-2b/FR-6 by explicitly distinguishing 'spec', 'proto', and 'oracle/harness' throughout the document and forbidding shorthand that could blur them. | Several sections refer to 'spec', 'behavior/oracle', and 'canonical proto/harness' in close proximity. Without a tighter naming convention, implementers may accidentally treat a proto change as a spec change or vice versa. | FR-1.1 Terminology and FR-2b / FR-6 / FR-9. | Perform a terminology audit on the final draft and verify that every occurrence maps to exactly one artifact class. |
| R3-S7 | Consistency | Medium | Align the report outputs in FR-8 with the remediation inputs in FR-9 by requiring the report to emit a machine-readable remediation candidate ID that can be referenced unchanged in the patch log and re-audit record. | The remediation loop depends on tracing a found issue into a concrete fix, but the current text does not guarantee a stable identifier across report, patch, and re-audit artifacts. | FR-8 — Bias-audit report and FR-9 — Remediation loop. | Run one end-to-end remediation trace and verify the same issue ID appears in the report, patch record, and re-audit manifest. |
| R3-S8 | Traceability | Critical | Add a required provenance chain for every derived artifact: upstream source citation → FR-1 matrix row → generated prompt instance → raw tool output → normalized artifact → analysis result, all linked by immutable IDs. | The document demands traceability, but it does not yet require a single end-to-end chain that can be traversed automatically. Without this, auditors may be able to inspect pieces individually but not reconstruct how a conclusion was produced. | FR-3 — Automatable, reproducible runs and FR-8 — Bias-audit report. | Verify that an external reviewer can start from any result in FR-8 and trace back to the exact source citations and prompt/run metadata without manual guesswork. |
| R3-S9 | Traceability | High | Require that each FR-1 FIXED/OPEN tag include an explicit 'decision owner' field indicating whether the tag was determined by source evidence, schema constraint, or human adjudication. | The current traceability matrix records evidence and rationale, but not who or what authority made the tagging decision. That matters when a tag is later challenged, especially for OPEN items that become hardened by the audit. | FR-1 — Neutral task brief and its source-to-brief traceability matrix. | Spot-check several matrix entries and confirm the decision basis is machine-readable and auditable. |
| R3-S10 | Traceability | Medium | Add a unique manifest schema version and artifact schema version to FR-3 so that later analysis can distinguish changes in the experiment design from changes in serialization or logging format. | The requirements already mention versioned templates and manifests, but not a schema version for the traceability record itself. That omission can make two runs appear comparable when their metadata structure changed in a way that affects interpretation. | FR-3 — Automatable, reproducible runs. | Change the manifest schema in a controlled test and verify the audit can still detect and report the schema mismatch. |

#### Review Round R4

- **Reviewer**: gemini-2.5 (gemini-2.5-flash)
- **Date**: 2026-06-17 21:10:07 UTC
- **Scope**: Architecture-focused review

| ID | Area | Severity | Suggestion | Rationale | Proposed Placement | Validation Approach |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| R4-S1 | Ambiguity | High | Define 'material' consistently across the document (e.g., 'material divergence,' 'material interaction,' 'material OPEN items'). | The term 'material' is used frequently in decision-making contexts (FR-4, FR-5, FR-6, FR-8, FR-11) but lacks a clear, quantitative or qualitative definition. This ambiguity makes it difficult to objectively determine when a divergence or interaction is significant enough to warrant further action or classify a verdict. | Add a definition to §1.1 Terminology, or a dedicated 'Thresholds and Definitions' section, and ensure consistent application. | Review all instances of 'material' to ensure they align with the defined criteria, potentially linking to FR-6's bias threshold or FR-11's statistical thresholds. |
| R4-S2 | Ambiguity | Medium | Provide clearer criteria or examples for 'Claude-idiom leakage' within the FR-1 brief review process. | FR-1 states the brief must be 'Human-reviewed for Claude-idiom leakage (the honesty control),' and OQ-2 highlights this as an open question. Without a more concrete definition or examples of what constitutes 'Claude-idiom leakage,' reviewer judgments may be inconsistent or subjective, undermining the neutrality of the brief. | Expand FR-1's description of the human review process, possibly including a rubric or examples of common Claude-specific phrasing or structural patterns to avoid. | Develop and test a review rubric for 'Claude-idiom leakage' with multiple reviewers, ensuring reasonable inter-rater agreement on identifying such patterns in example briefs. |
| R4-S3 | Consistency | Medium | Standardize the terminology used for different types of 'bias' throughout the document. | The document uses 'Anthropic-authorship bias,' 'systematic-bias surface,' 'author-bias,' 'vendor-authorship bias signal,' 'vendor-author bias candidate,' 'Claude-bias,' and 'semantic bias.' While 'Vendor-authorship bias signal' is defined, the relationship and distinctions between these terms are not always clear, which could lead to confusion in analysis and reporting. | Review and refine §1.1 Terminology to explicitly define or map all variations of 'bias' used, ensuring consistent application. | Conduct a lexical review of the document to ensure all 'bias' terms are used according to defined distinctions or are replaced with the standardized term. |
| R4-S4 | Traceability | High | Require explicit traceability for 'human judgment' supporting FIXED/OPEN tags in the FR-1 matrix. | FR-1 mandates a traceability matrix including 'human judgment supporting the tag.' To ensure auditability and accountability, the specific human judgment itself (who made it, when, and based on what specific evidence or rationale) needs to be recorded and traceable, not just its existence. | Amend FR-1's description of the traceability matrix to include mandatory fields for reviewer ID, date, and a brief rationale/citation for any human judgment made for FIXED/OPEN tagging. | Verify that the FR-1 traceability matrix template includes fields for detailed 'human judgment' provenance and that these fields are consistently populated during pilot execution. |
| R4-S5 | Traceability | Medium | Define and standardize the required content for 'reviewer sign-off' records across all relevant requirements (FR-4, FR-9, FR-12). | Reviewer sign-offs are crucial for validation (oracle, mutants) and remediation processes. However, the document doesn't specify what constitutes a complete 'sign-off' record (e.g., reviewer identity, date, specific item/decision reviewed, brief rationale, any dissenting opinions). This lack of detail hinders consistent record-keeping and auditability. | Add a new sub-requirement or a section under §1.1 Terminology defining the minimal content required for a 'reviewer sign-off' record, applicable to FR-4, FR-9, and FR-12. | Create a standardized sign-off template and ensure its consistent use in all processes requiring reviewer sign-off, verifying that all required fields are completed. |
| R4-S6 | Consistency | Medium | Clarify the timing and distinction between the 'neutral brief' (FR-1) and artifacts incorporating 'pinned' OPEN item resolutions (FR-9). | FR-1 requires a neutral brief that 'must not leak Claude's resolution of any OPEN item.' However, FR-9 describes 'source-ambiguity → pin behavior in the spec + mark the FR-1 matrix human-adjudicated.' This creates potential confusion about whether the 'neutral brief' itself can evolve or if 'pinned' resolutions only apply to generated artifacts. Clarifying this workflow is important for understanding the baseline of neutrality. | Add clarifying text to FR-1 and FR-9 to explicitly distinguish the initial state of the 'neutral brief' from the state of artifacts after remediation and 'pinning' of OPEN items. | The document should clearly state that the FR-1 brief remains the initial neutral reference, while 'pinned' resolutions apply to *derived* artifacts, and any changes to the FR-1 matrix due to adjudication are explicitly marked as such. |
| R4-S7 | Traceability | High | Specify the full required content and structure for the 'adjudication evidence log' mentioned in FR-7. | FR-7 mentions an 'adjudication evidence log' but provides only a high-level description of its purpose. For proper auditability and to reconstruct decisions, the log needs detailed requirements for its content, such as specific versions of all input artifacts (prompts, raw outputs, statistical results), individual reviewer rationales, points of disagreement, final decision, and citations to evidence. | Expand FR-7's description of the 'adjudication evidence log' to list all mandatory data fields and structural requirements. | Develop a template for the 'adjudication evidence log' that incorporates all specified elements, and confirm its consistent use and completeness during pilot adjudication sessions. |
| R4-S8 | Ambiguity | Low | Clarify the meaning of 'human-adjudicated correction' in FR-5's classification rubric. | The term 'human-adjudicated correction' as a classification for divergence in FR-5 is slightly ambiguous. It could refer to a divergence that was *resolved by* a human correction, or a divergence that *required* a human correction. Clarifying this distinction would improve precision in classification. | Refine the wording within FR-5's classification rubric to clearly define what 'human-adjudicated correction' signifies (e.g., 'Divergence resolved by human adjudication and subsequent correction'). | Ensure that the revised definition clearly distinguishes between the classification of a divergence and the act of correction, allowing for unambiguous application by adjudicators. |
| R4-S9 | Feasibility | Medium | Define specific factors or scenarios that would render reviewer blinding 'impractical' in FR-7. | FR-7 states reviewers are 'blinded to author-vendor labels where practical.' The integrity of the adjudication process relies on blinding, so it's critical to define what situations make it 'impractical' and what mitigations are in place. Without this, 'impractical' could be broadly interpreted, reducing the rigor of blinding. | Add a sub-point to FR-7's adjudication protocol detailing specific conditions that would make blinding impractical, and outline any fallback procedures or documentation requirements for unblinded reviews. | Document potential scenarios where blinding is challenging and propose clear guidelines for when to proceed unblinded, ensuring these exceptions are justified and recorded during pilot review. |
| R4-S10 | Completeness | Medium | Provide more explicit criteria for adjudicating catastrophic failures as 'authorship-relevant' in FR-3b. | FR-3b states catastrophic failures are excluded from semantic bias calls 'unless the failures are themselves consistent, vendor-specific, and adjudicated as authorship-relevant.' The criteria for 'authorship-relevant' failures are not detailed, which could lead to inconsistent decisions about which failures are considered evidence of bias versus pure tool-capability issues. | Expand FR-3b's policy to include explicit criteria or examples for what constitutes an 'authorship-relevant' catastrophic failure (e.g., consistent misinterpretation of a specific OPEN item, persistent non-compliance with a FIXED item's semantic rather than syntax). | Develop and document a rubric for classifying catastrophic failures as 'authorship-relevant,' and ensure adjudicators apply it consistently during pilot analysis. |
