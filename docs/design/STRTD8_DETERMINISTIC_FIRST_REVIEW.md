...
| **D2** | The owned/authored **composition seam is not regen-safe**: we mount routers by hand-editing generated `app/server.py` and add nav to generated `base.html`; `generate backend` would overwrite both. | Define a **stable extension seam** the generator preserves or emits (e.g. an owned `routers/_user/*` auto-included, or generator-emitted mounts from a manifest). |
