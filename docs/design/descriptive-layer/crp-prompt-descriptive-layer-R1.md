...
**Generated:** 2026-07-17 17:36:19 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/descriptive-layer/DESCRIPTIVE_LAYER_PLAN.md` | 101 lines · 799 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/descriptive-layer/DESCRIPTIVE_LAYER_REQUIREMENTS.md` | 193 lines · 2000 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/descriptive-layer/crp-focus-descriptive-layer.md` | 35 lines · 319 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/descriptive-layer/DESCRIPTIVE_LAYER_PLAN.md`  ·  **Size:** 101 lines · 799 words

```markdown
# Descriptive Layer — Implementation Plan

**Version:** 0.2 (Post-planning — pairs with requirements v0.3.1)
**Date:** 2026-07-17
**Status:** Draft
...
- **M-DL0 — Manifest schema + wireframe records (FR-DL-1/2, OQ-2).** Define the record schema
  (`what/how/why/do/next[]/degrade{}`) and author one record per wireframe section (Scaffold,
  Services, Entities, Pages, Forms, Views, Completeness). *Externalize the existing inline
  `plan.py` consequence literals into `why`/`degrade`* (FR-DL-5) — this is a move, not a rewrite.
  Decision to make: YAML data file vs Python module (OQ-2). Recommend a YAML file
  `wireframe/descriptive.yaml` for single-sourcing + parity with the capability manifests.

- **M-DL1 — Composer (FR-DL-5/8/9).** A pure function `describe(section, plan) -> DescribedUnit`
  that fills the record's `{{…}}` placeholders from validated plan data (counts, missing keys,
  the resolve command). Deterministic; an unfillable placeholder raises a typed error (CCbC).
  Attaches provenance (record id) to the output.

- **M-DL2 — Derived next-steps (FR-DL-3).** For each non-`planned` section, derive the concrete
  action from plan data: `not_defined views` → `"add views.yaml → composite views"` with the exact
  flag/command. Cap the affordance list with a `+K more` overflow indicator (no silent truncation).

- **M-DL3 — Workflow grounding (FR-DL-4).** Infer workflow position from the status mix
  (`worst()` roll-up already exists): predominantly `not_defined` ⇒ DATA MODEL / early; mostly
  `planned` ⇒ ready-to-generate. Select the position-appropriate `do`/`next` framing.

- **M-DL4 — Renderer augmentation (FR-DL-6/7, NR-5).** Wrap `render_plan`/`_section_node` so each
  section renders in the consistent grammar: WHAT header → existing tree body → WHY note (reusing
  the current `→ consequence` line) → DO/NEXT footer. Degradation messages gain the action-half.
  Behind a flag first (`--describe`) to keep the terse mode default until proven.

- **M-DL5 — Tests + provenance (FR-DL-8/9).** Golden tests: manifest × fixture plan ⇒ stable
  described output; every described unit traces to a record; deterministic (byte-stable) output.
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/descriptive-layer/DESCRIPTIVE_LAYER_REQUIREMENTS.md`  ·  **Size:** 193 lines · 2000 words

```markdown
# Descriptive Layer — Requirements

**Version:** 0.3.1 (Draft — post-planning, lessons- + principle-hardened)
**Date:** 2026-07-17
**Status:** Draft
...
section; generically, a Node or any wrapped command's output block), carrying: `what`, `how`,
`why`, `do`, `next[]`, `degrade{by-status}`. Keyed on the unit's **stable key** (section key /
`FR-`prefix), not its display label. (Ports the 3xl-kcui `Intent`: `responses.text`=what,
`tools`+`compose`=how, dry-run=why, `chips`=next.)

**FR-DL-2 — The four dimensions are mandatory.** Every output-unit MUST declare `what/how/why/do`.
`why` MAY reuse the existing FR-W5 `consequence` where present rather than restate it.
...
next kickoff step. Affordances are **capped with an explicit overflow indicator — no silent
truncation** (3xl-kcui chips: cap + "+K more").
...
MAY be **inferred** from the status mix (predominantly `not_defined` ⇒ early DATA MODEL; mostly
`planned` ⇒ ready-to-generate). `do`/`next` are never generic — they are "from *here*, do Y."

**FR-DL-5 — Single-sourced template composition.** Records are **templates** with placeholders
(`{{count}}`, `{{missing}}`, `{{cmd}}`) filled from the **validated** live plan. Narration text is
**owned by the manifest**, not scattered in the renderer — this spec **externalizes** the current
inline `plan.py` consequence literals into the manifest. An unfillable placeholder is a typed
error, never silent blank text.
...
- **NR-4** — Not an interactive REPL/TUI. `startd8 wireframe` is one-shot; `next[]` renders as printed action hints **with their exact commands**, not selectable chips — until a REPL exists (OQ-3).
...
- **OQ-5 — Navigator reuse.** Does the same manifest also *generate* the navigator READMEs (kickoff/wireframe/capability-index), or drive CLI output only? (If yes, the descriptive layer becomes the single source behind both the static navigators and the live CLI — high Mottainai value.)
