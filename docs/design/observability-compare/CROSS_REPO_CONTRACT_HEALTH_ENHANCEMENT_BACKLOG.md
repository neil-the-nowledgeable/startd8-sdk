...
**CH-4 — `$0` read-only `--validate` preflight (present-vs-defaulted + typo did-you-mean + malformed-value report).** **→ DONE (this branch): `startd8 observability validate-onboarding -m <file>` — reports present-vs-defaulted fields, did-you-mean typos, and malformed/dropped values; exit 1 on findings. `observability/onboarding_validate.py`.** `S` · `wire-existing` · **X4** (CROSS s3-R3 + s3-R4 + s2-datasource; seeders 2+3).
