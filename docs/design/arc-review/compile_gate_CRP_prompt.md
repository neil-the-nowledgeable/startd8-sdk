...
**Generated:** 2026-06-01 14:08:13 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/COMPILE_GATE_PLAN.md` | 43 lines · 591 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/COMPILE_GATE_REQUIREMENTS.md` | 140 lines · 2036 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/arc-review/compile_gate_focus.md` | 40 lines · 299 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/COMPILE_GATE_PLAN.md`  ·  **Size:** 43 lines · 591 words

```markdown
# Generalized Compile-Gate (Go / Java / C#) — Implementation Plan

**Version:** 0.2 (Post-planning)
**Date:** 2026-06-01
**Status:** Draft (ready for convergent review)
**Requirements:** `docs/design/COMPILE_GATE_REQUIREMENTS.md` (v0.2 — FR-1..FR-9)
...
|---|------|----|-------|--------|
| **0 ✅ DONE** | **Discovery** — protocol is `@runtime_checkable` (new member breaks all 7 profiles); provisioning is distinct from compile; build-root ≠ project_root; `test_command` already compiles. See requirements §0. | — | (read-only) | ✅ captured |
| **1** | **Protocol + all-profile capability (ATOMIC).** Add `compile_check_command` + `compile_provision_commands` to `protocol.py` and to **all 7 profiles** (go/java/csharp real; python/prisma/vue/nodejs → values per FR-1/2, non-compiled return `None`/`[]`). Must co-land — registration's `isinstance` breaks until every profile defines them. | FR-1, FR-2 | `languages/protocol.py`, `languages/{go,java,csharp,nodejs,python,vue,prisma}.py` | `LanguageRegistry.discover()` registers all 7 (no `isinstance` error); `compile_check_command` correct per lang |
| **2** | **Build-root locator** — shared util: search upward from generated files for nearest `go.mod`/`build.gradle(.kts)`/`pom.xml`/`*.csproj`/`*.sln`/`tsconfig.json`; return the dir to run from. | FR-7 | `validators/compile_gate.py` (new) | unit: go.mod in subdir → that subdir; single-root → project_root; none → None |
| **3** | **Per-language diagnostic parsers** — pure `parse_*_diagnostics(text) → [CompileDiagnostic]` for Go (`f.go:L:C:`), Java (`F.java:L: error:`), C# (`F.cs(L,C): error CSxxxx:`); reuse `parse_tsc_output` for TS. Tolerate gradle/dotnet banner noise. | FR-3 | `validators/compile_gate.py` | unit: fixture compiler outputs → correct diagnostics; banners ignored |
| **4** | **Unified gate runner** `run_compile_gate(project_root, profile) → CompileGateResult{status,diagnostics,verdict}`: locate build root (2) → run `compile_provision_commands` → `compile_check_command` → parse (3). FR-5 loud degradation (toolchain/build-file/provision missing → `unavailable`, non-pass). | FR-4, FR-5 | `validators/compile_gate.py` | unit (toolchain-gated): coherent → pass, incoherent → fail, absent → unavailable |
| **5** | **TS retrofit** — refactor `ts_toolchain.run_project_typecheck` to be the TS implementation behind `run_compile_gate` (or alias `ToolchainResult`↔`CompileGateResult`); keep all existing TS-gate tests green. | FR-9 | `validators/ts_toolchain.py`, `validators/compile_gate.py` | existing `test_ts_toolchain.py` + cap-dev-pipe gate unchanged behavior |
| **6** | **Integration** — generalize cap-dev-pipe `ts-verify-gate.py` → `verify-gate.py` (resolve language → `run_compile_gate`); generalize `_evaluate_ts_toolchain` → `_evaluate_compile_gate` (dispatch by language, attribute by file, hard-FAIL + Kaizen). Same env toggles; runs after every batch incl. terminal. | FR-6 | `cap-dev-pipe/verify-gate.py`, `cap-dev-pipe/run-prime-contractor.sh`, `contractors/prime_postmortem.py` | compiled-lang batch with cross-file error → gate FAIL + postmortem attribution; Python batch unaffected |
| **7** | **Regression fixtures** per language (FR-8): coherent→pass, syntactically-valid-but-cross-file-broken→fail, toolchain-absent→unavailable; parsers tested from captured output without the real toolchain. | FR-8 | `tests/unit/validators/test_compile_gate.py` + fixtures | per-language pass/fail/unavailable; direct-to-parser lock |

**Co-landing constraint:** Step 1 is atomic — the Protocol member + all 7 profile implementations land together, or `LanguageRegistry` registration breaks (`isinstance` fails). Steps 2–4 then build the runner; Step 5 retrofits TS without behavior change; Steps 6–7 integrate and lock.
...
See `COMPILE_GATE_REQUIREMENTS.md` §0. Net: no blocker; the only sharp edge is Step 1's atomic Protocol-member rollout across all 7 profiles. Provisioning + build-root are net-new but mechanical; the TS gate is the proven template.
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/COMPILE_GATE_REQUIREMENTS.md`  ·  **Size:** 140 lines · 2036 words
...
#### FR-1 — `compile_check_command` on the LanguageProfile protocol
Add a project-level `compile_check_command` property to `LanguageProfile` (distinct from the per-file `syntax_check_command`). Because the protocol is `@runtime_checkable` and `registry.register()` does an `isinstance` check, **all 7 registered profiles MUST implement it** (return `None` where there is no compile gate) or registration breaks. Values:
- **Go:** `["go", "build", "./..."]`
...
- **Python / Prisma / Vue:** `None` (Python has no compile gate; Prisma uses `prisma validate`; Vue script blocks are covered by the TS path — out of scope here).
*Acceptance:* every profile defines `compile_check_command`; `isinstance(profile, LanguageProfile)` still holds for all 7; `LanguageRegistry.discover()` registers all without error.
...
*Acceptance:* the gate runs provisioning before compile; a Go project with a valid go.mod resolves deps; a missing toolchain yields `unavailable`.
...
Parsing MUST tolerate the build tool's wrapper noise (gradle/dotnet banners, progress) and extract only genuine compiler errors.
*Acceptance:* fixture compiler outputs per language parse to the correct diagnostics; banner/progress lines are ignored.

### Group B — The unified gate runner + integration

#### FR-4 — One gate runner, dispatch by resolved language (compile-only)
A single `run_compile_gate(project_root, profile)` → `CompileGateResult{status, diagnostics, verdict: pass|fail|unavailable}` that: locates the build root (FR-7), runs `compile_provision_commands` then `compile_check_command`, and parses output (FR-3). It is **compile-only** — it MUST NOT run `test_command` (tests need fixtures/infra, fail for non-compile reasons, and are slower; the gate isolates the cross-file *resolution* check). The existing `ts_toolchain.run_project_typecheck` is refactored to be the TS implementation behind this runner (FR-9).
*Acceptance:* a coherent Go/Java/C# project → `pass`; a cross-file-incoherent one → `fail` with diagnostics; TS behavior unchanged.

#### FR-5 — Loud degradation (the FR-9 contract, generalized)
When the build toolchain (`go`/`gradle`/`mvn`/`dotnet`), the build file (`go.mod`/`build.gradle`/`pom.xml`/`.csproj`), or dependency provisioning is unavailable, the result MUST be `status` → `verdict=unavailable`, treated by callers as **non-pass** — never a silent pass. A reduced-confidence note records which step was unavailable.
*Acceptance:* missing `go`/`dotnet`/`gradle` → `unavailable` (not pass); missing build file → `unavailable`; provisioning failure → `unavailable`, not `fail`.

#### FR-6 — Pipeline + postmortem integration (generalize the TS wiring)
- **cap-dev-pipe:** generalize `ts-verify-gate.py` → a language-dispatching `verify-gate.py` that resolves the project language and runs `run_compile_gate`. Same toggles (`STARTD8_*_GATE_STRICT`, `*_VERIFY_GATE=0`), same skip-when-absent behavior. Runs after every batch incl. the terminal one (RUN-008 OQ-3).
- **in-process:** generalize `_evaluate_ts_toolchain` → `_evaluate_compile_gate` in `prime_postmortem.py`, dispatching by resolved language, attributing diagnostics to features by file, forcing FAIL (`disk_quality_score=0`, `success=False`, `root_cause=CROSS_FILE_CONTRACT`, `pipeline_stage=CROSS_FEATURE_CONTRACT`) + ≥1 Kaizen suggestion. Env-gated per language.
*Acceptance:* a compiled-language batch with a cross-file error → the gate FAILs and the postmortem attributes it; a Python batch is unaffected (no compile gate).

#### FR-7 — Build-root location
The gate MUST locate the build root by searching upward from the generated files for the nearest build file (`go.mod`, `build.gradle`/`build.gradle.kts`/`pom.xml`, `*.csproj`/`*.sln`) and run provisioning/compile from there — not assume `project_root` (Go multi-module: `go.py:43-44`).
*Acceptance:* a project with `go.mod` in a subdirectory → the gate runs `go build ./...` from that subdirectory; a single-root project runs from `project_root`.

### Group C — Cross-cutting

#### FR-8 — Regression fixtures per language
Tests MUST include, per language: (a) a coherent multi-file project that compiles → `pass`; (b) a cross-file-incoherent project that the per-file syntax check passes but compilation rejects → `fail` (Go: caller with wrong arg count / undefined symbol from a sibling; Java: `import` of a class no feature defines / wrong method signature; C#: `using`/type from a missing namespace, CS0246). Plus a toolchain-absent case → `unavailable`. Diagnostic parsers tested from captured fixture output without invoking the real toolchain (the subprocess path tested behind a toolchain-available guard).

#### FR-9 — TypeScript retrofit (no regression)
The existing merged TS gate (`ts_toolchain`, `ts-verify-gate.py`, `_evaluate_ts_toolchain`) MUST be preserved behaviorally when refactored behind the unified abstraction — same diagnostics, same `unavailable` semantics, same env gates. The retrofit is a refactor, not a rewrite; its existing tests MUST stay green.
...
- **Does NOT replace build-on-test.** Complementary — when tests run with a provisioned toolchain they still catch more; this gate guarantees the *compile/resolution* check even when tests don't run.
...
*v0.2 — Post-planning self-reflective update. 4 assumptions corrected (Protocol-member breadth across all 7 profiles, provisioning as a distinct capability, build-root ≠ project_root, compile-only justification vs test_command), 3 open questions resolved at planning time (no test reuse, single unified abstraction, provisioning host = RUN-008 OQ-3). Scope: Go/Java/C# compile-gate + unified `compile_check_command` abstraction with TS retrofit; test execution out of scope.*
```
