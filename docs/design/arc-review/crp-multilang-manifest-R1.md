...
**Generated:** 2026-05-30 19:33:02 UTC
...
| Role | Path | Size |
|------|------|------|
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/MULTILANG_MANIFEST_VALIDATION_REQUIREMENTS.md` | 198 lines · 1898 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/.crp_focus_multilang.md` | 28 lines · 240 words |
...
# Reviewer Focus — Multi-Language Manifest Validation (where we need input most)

Context: the requirements add element-level contract enforcement for non-Python languages to
ForwardManifest.validate_implementation, by adapting EXISTING per-language parsers
(parse_{csharp,go,java,nodejs}_source, vue) into the common Element/FileManifest shape. Weight the review toward:

1. **FR-5 severity calibration (the crux).** Go and Node/JS/TS parsers are regex-based (can MISS
   elements); C#/Java/Python are AST-grade. The spec makes advisory-tier "missing_element" a
   `warning` (never blocks) and authoritative-tier an `error`. Is this the right boundary? Is
   per-language a static tier, or should confidence be per-parse (e.g. tree-sitter fell back to
   regex → advisory)? Could an advisory `warning` still mislead the Kaizen/Fix-3 classifier?

2. **Kind-mapping / ElementKind expansion (FR-3, OQ-1).** Adding DEFAULT_EXPORT + INTERFACE/ENUM/
   STRUCT/RECORD/FIELD vs mapping language kinds to nearest existing kinds. Risk of an additive enum
   change to existing consumers/serialization? Is a documented kind-string→ElementKind map the right
   seam, and where should it live?

3. **OQ-5 — signature fidelity.** Go/Java/C# parsers emit signature strings. Map into the structured
   Signature model now (enabling return-type contract checks) or store raw/defer? What does
   _validate_file_spec / InterfaceContract actually consume today?

4. **OQ-6 — advisory-tier "missing" suppression.** Should advisory (regex) missing-element warnings
   surface in the review at all, only feed diagnostics, or be suppressed? Trade-off: reviewer noise
   vs Fix-3 classifier signal.

5. **Adapter architecture + blast radius.** New build_multilang_file_manifest vs overloading
   generate_file_manifest (used in 9 files). FR-7 routes lead_contractor_workflow through the same
   builder — any behavior-change risk to that shipped path? Parser-absence degradation (NFR-3).
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/MULTILANG_MANIFEST_VALIDATION_REQUIREMENTS.md`  ·  **Size:** 198 lines · 1898 words

```markdown
# Multi-Language Manifest Validation — Requirements

**Version:** 0.2 (Post-exploration — self-reflective update)
**Date:** 2026-05-30
**Status:** Reviewed against the languages/ parsers + `code_manifest` + `forward_manifest_validator`
...
- **FR-2 Element + import adapter.** Map each language-native element type
  ...
  *Acceptance:* adapter unit tests assert `(kind, name)` tuples for a representative file per language.

- **FR-3 `ElementKind` expansion (additive).** Add `DEFAULT_EXPORT` (required for FR-4) plus the
  language-native kinds the parsers emit but the enum lacks — `INTERFACE`, `ENUM`, `STRUCT`,
  `RECORD`, `FIELD` — so the adapter never silently collapses a kind. Purely additive; existing
  values unchanged. A documented `kind`-string → `ElementKind` map covers every value the parsers
  produce (e.g. `const_function` → `FUNCTION`, `constructor` → `METHOD`). *Acceptance:* every `kind`
  string emitted by the five parsers resolves to an `ElementKind` (no `KeyError`, no silent drop).
...
- **FR-5 Parser-confidence severity calibration.** Each supported language has a **confidence tier**:
  *authoritative* (Python AST, C# tree-sitter, Java javalang) or *advisory* (Go, Node/JS/TS, Vue —
  regex-based, and any AST parser that fell back to regex). A `missing_element` / `missing_import`
  from an **authoritative** parser is severity `error` (can block, as Python does today); from an
  **advisory** parser it is severity `warning` (surfaced, never blocks) — a regex blind spot MUST NOT
  fail a review for an element that is actually present. *Acceptance:* a Go/Node file that legitimately
  contains the element but is mis-parsed by the regex parser produces at most a `warning`, never a
  blocking `error`; a C# file missing a tree-sitter-confirmed element produces an `error`.
...
- **FR-7 Shared builder (subsumes lead-contractor consolidation).** `lead_contractor_workflow._review_draft`'s
  ...
  contractor review gate. Additive: Python behavior unchanged. *Acceptance:* the lead/primary review
  path enforces a non-Python file's contract via the shared builder.
...
- Does **not** change `ForwardFileSpec`/`InterfaceContract` schemas.
...
| Phase | Scope | Gate |
|-------|-------|------|
| **P1** | FR-3 `ElementKind` expansion + the `kind`-string→`ElementKind` map | suite green; map covers all parser kinds |
| **P2** | FR-1 builder + FR-2 adapter for the **authoritative** tier (C#, Java) + FR-5 tiers | per-language adapter tests; C#/Java files enforce at `error` |
| **P3** | FR-2 adapter for the **advisory** tier (Go, Node/JS/TS, Vue) at `warning` severity | advisory-tier files enforce at `warning`, no false blocks |
| **P4** | FR-4 `DEFAULT_EXPORT` node-parser enhancement + framework-registry switch | `next.config`/`tailwind` fixtures validate clean / catch wrong-shape |
| **P5** | FR-6 wire into `validate_implementation`; FR-7 wire into lead/primary review | mixed-language multi-file blob enforces per file |

Each phase is a separate PR/commit set, green independently (FR-8).

---

## 6. Open Questions

*OQ-1…OQ-4 resolved in §0.* Remaining for planning/CRP:

- **OQ-5 — signature fidelity.** Go/Java/C# parsers capture `signature` strings; should FR-2 map
  them into the common `Signature` model (enabling future return-type contract checks), or store raw
  for now? (Leaning: store what's cheap, defer structured signature enforcement to the §4 stretch.)
- **OQ-6 — advisory-tier "missing" suppression.** Should advisory-tier `missing_element` warnings be
  emitted at all, or only logged? (A warning the reviewer ignores is noise; but it feeds Fix 3's
  classifier.) Decide whether advisory misses surface in the review or only in diagnostics.
...
are regex-based, so their "missing element" must be advisory, never blocking, to avoid trading
Python's false-negative-free enforcement for false positives elsewhere. `DEFAULT_EXPORT` needs a real
`nodejs_parser` enhancement (FR-4), not just an enum value. The builder subsumes the earlier
