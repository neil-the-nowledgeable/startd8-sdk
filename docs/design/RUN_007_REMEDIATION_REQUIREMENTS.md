...
### FR-4 — Budget & provider boundary → structured refusal
First (OQ-3, blocking) **prove the dollar budget signal is observable at the micro-prime escalation decision**; if it is not, FR-4 is enforced at the **orchestration layer** that owns the budget (refuse before dispatch) (R1-F2). The outcome is the **same structured refusal** (FR-2.2) for every no-generation boundary: budget would exceed the cap, provider/fallback unavailable, escalation disabled (R3-F4). Document behaviour at the exact cap boundary (spend == cap). No path may write a skeleton or silently skip.
...
- **OQ-3 → CRP-answered (likely NO), confirm in code.** Review (R1-S3/R1-F2) found `engine.py` carries only recursion + token budgets; `prime_adapter` only *records* `element_escalation_cost`. FR-4 assumes the budget may need to be enforced at the orchestration layer. **Step 0 must confirm** before FR-4 implementation.
- **OQ-4 → CRP-answered NO.** `validate_disk_compliance` AST-parses Python only; non-`.py` routes to `_validate_non_python_file`. The FR-5 detector is net-new in the non-Python path (reusing `NodeLanguageProfile.validate_syntax`), not a reuse of an existing parse (R1-S4/R1-F3).
