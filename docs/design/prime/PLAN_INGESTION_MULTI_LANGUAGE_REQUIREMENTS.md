...
|-----|-------------|--------|
| REQ-PLI-100 (partial) | Language detection for .py, .go, .java | `fdf40c2`, `04c9457` |
...
|-----|-------------|----------|----------|
| REQ-PLI-100 (Node.js) | `.js`/`.ts` → `"nodejs"` in lang_detect | P1 | Node.js |
| REQ-PLI-101 | Language ID consistency (lang_detect vs resolution.py) | P1 | Node.js |
| REQ-PLI-102 | Filename-based detection (build.gradle, package.json) | P2 | Java, Node.js |
| REQ-PLI-200 (Java) | PARSE prompt Java metadata fields | P2 | Java |
| REQ-PLI-200 (Node.js) | PARSE prompt Node.js metadata fields | P3 | Node.js |
| REQ-PLI-201 | Language-agnostic PARSE prompt structure | P2 | All |
| REQ-PLI-202 | Primary language detection from plan text | P3 | All |
| REQ-PLI-300 (Java) | Java service metadata inference | P2 | Java |
| REQ-PLI-300 (Node.js) | Node.js service metadata inference | P3 | Node.js |
| REQ-PLI-301 (Java/Node.js) | Context threading for Java/Node.js fields | P2 | Java, Node.js |
| REQ-PLI-402 (Node.js) | package.json deterministic generation | P3 | Node.js |
| REQ-PLI-500 (Java) | Available-imports Java formatting | P2 | Java |
| REQ-PLI-500 (Node.js) | Available-imports Node.js formatting | P3 | Node.js |
| REQ-PLI-501 (Java) | `_build_java_module_section()` | P2 | Java |
| REQ-PLI-501 (Node.js) | `_build_nodejs_module_section()` | P3 | Node.js |
| REQ-PLI-601 | Cross-language dependency ordering | P3 | All |
| REQ-PLI-700 (Java) | ParsedFeature Java fields | P2 | Java |
| REQ-PLI-700 (Node.js) | ParsedFeature Node.js fields | P3 | Node.js |
| REQ-PLI-701 (Java/Node.js) | SeedTask language fields | P2 | Java, Node.js |
...
| 7. Add `_build_X_module_section()` | `_build_go_module_section()` | `_build_java_module_section()` | `_build_nodejs_module_section()` |
...
### P1 — Immediate (enables first Java plan ingestion run)

1. **REQ-PLI-200 (Java)**: Add Java fields to PARSE prompt
2. **REQ-PLI-300 (Java)**: Java service metadata inference in `_infer_service_metadata()`
3. **REQ-PLI-301 (Java)**: Add Java fields to `_CONTEXT_THREADABLE_FIELDS`
4. **REQ-PLI-501 (Java)**: `_build_java_module_section()` in spec_builder
5. **REQ-PLI-700 (Java)**: Add Java fields to `ParsedFeature` and `SeedTask`

### P2 — Quality (improves multi-language robustness)
...
### P3 — Future (Node.js and cross-language)
...
|-----------|--------|-----|------|---------|
| File extension detection | ✅ | ✅ | ✅ | ❌ (maps to "text") |
...
| Available-imports formatting | ✅ | ✅ | Partial | ❌ |
