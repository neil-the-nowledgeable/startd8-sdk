...
**Error semantics for protocol methods:** Strategy method implementations MUST distinguish between "resolved successfully with empty data" and "resolution failed due to I/O or parsing error." On successful resolution with empty or missing source data, methods MUST return their normal return type with empty/default values (e.g., `SeedContext` with empty dicts, `ValidationConfig` with empty validator list, `gen_context` dict with only always-present sections). On resolution failure due to I/O errors, permission errors, or parsing errors (e.g., corrupt JSON in context files, unreadable plan document), methods MUST raise `ContextResolutionError` (a new exception subclass of `RuntimeError`) with a descriptive message indicating the failed resource and root cause. `ContextResolutionError` MUST be raised with exception chaining (`raise ContextResolutionError(msg) from original_exception`) to preserve the root cause for diagnostic logging. Without chaining, the original `FileNotFoundError`, `json.JSONDecodeError`, or `PermissionError` is lost in error logs, and operators see only the `ContextResolutionError` message without knowing whether the failure was due to a missing file, corrupt data, or insufficient permissions. The workflow MUST catch `ContextResolutionError` at the call site and handle it according to mode: in standalone mode, log a warning and continue with empty defaults; in pipeline mode, log an error and — depending on which method failed — either continue with degraded context (`resolve_validation_config`) or handle as specified below for `resolve_seed_context` and `resolve_task_context`.

**Pipeline mode error handling by method:**
- `resolve_seed_context` failure: The workflow MUST distinguish between seed-level failures and secondary resource failures. **Seed-level failures** (corrupt seed JSON, missing seed file, unparsable seed structure) are unrecoverable — the workflow MUST abort, since no seed context means no meaningful generation. **Secondary resource failures** (unreadable context file referenced by the seed, plan document I/O error, permission error on an individual `context_files` entry) are recoverable — the workflow MUST log a warning identifying the failed resource, omit the failed resource from `SeedContext`, and continue with the remaining successfully resolved seed data. This distinction aligns with the standalone strategy's existing behavior for file read errors (REQ-PEM-006) and prevents a single unreachable context file from blocking an otherwise valid pipeline run.
...
- `ContextResolutionError` is defined as a subclass of `RuntimeError` and raised on I/O or parsing failures within strategy methods, always with exception chaining (`from original_exception`)
...
- In pipeline mode, `resolve_seed_context` distinguishes seed-level failures (abort) from secondary resource failures (warn + omit + continue)
...
**`ContextFileEntry.content` lifecycle:** The `content` field is populated by the `ContextResolutionStrategy` during `resolve_seed_context()`. Content is eagerly loaded at seed resolution time — not lazily on first access. Loading failures for individual context files MUST be handled per the strategy's error semantics: in standalone mode, log a warning and omit the file's content (set `content` to `None` or omit the key); in pipeline mode, log a warning and omit the file's content per the secondary resource failure handling defined in REQ-PEM-004 and REQ-PEM-006/007 file error handling. Successfully loaded entries will have `content` populated as a string; entries where loading failed will have `content` absent or `None`. Consumers of `context_files` MUST check for the presence of `content` before use.
...
- `ContextFileEntry.content` is populated during `resolve_seed_context()` with per-strategy error handling for loading failures
...
**Always-present section content specification:** The two sections marked as "always present" MUST contain meaningful content regardless of execution mode or available metadata:
- `general_context` MUST always contain at minimum: feature name, feature description, and target file paths. These three values are always available because they are required fields of `FeatureSpec` (set via `add_feature()` or `add_features_from_seed()`). When additional metadata is available (e.g., `estimated_loc`, `artifact_types_addressed`), it SHOULD be included in `general_context` as supplementary information.
- `scope_boundary` MUST always contain the instruction `"Generate only what is specified."` This instruction constrains the LLM to the declared feature scope and prevents speculative generation of unrequested functionality. The instruction text is a constant — it does not vary by mode, feature, or seed content.
...
- `general_context` always contains feature name, description, and target file paths
- `scope_boundary` always contains the "Generate only what is specified." instruction
...
**Cache hit behavior:** On cache hit (staleness check passes and `--force-regenerate` is not active), existing output files including `generation-manifest.json` MUST NOT be modified. The manifest's existing timestamps and content are preserved as-is. This prevents unnecessary downstream processing by consumers that monitor manifest modification times, and ensures that the manifest accurately reflects the run that produced the cached output.
...
- On cache hit, existing `generation-manifest.json` MUST NOT be modified
...
**Known limitation — external file blind spot:** The `source_checksum` is computed over the seed JSON content only. Changes to external files referenced by the seed (plan documents, context files, design references) do NOT change the seed checksum, which can lead to stale cache hits when the seed is unchanged but referenced files have been modified. This is an inherent limitation of the content-addressable design: the checksum captures the seed's declaration of what files to use, not the content of those files. **Operator guidance:** When external files referenced by the seed are modified without changing the seed itself, operators should either (a) use `--force-regenerate` to bypass the cache, or (b) bump a seed field (e.g., increment a version counter or update a comment field) to produce a new checksum. A future enhancement MAY extend the checksum computation to include hashes of referenced external files, but this is out of scope for the current design.
...
| 56 | Pipeline mode resolve_seed_context distinguishes seed-level vs. secondary resource failures | REQ-PEM-004 | Pipeline |
| 57 | ContextFileEntry.content populated during resolve_seed_context with per-strategy error handling | REQ-PEM-005 | Both |
| 58 | On cache hit, generation-manifest.json is not modified | REQ-PEM-012 | Pipeline |
| 59 | ContextResolutionError preserves root cause via exception chaining (`from original_exception`) | REQ-PEM-004 | Both |
| 60 | `general_context` always contains feature name, description, and target file paths (even with minimal metadata) | REQ-PEM-007 | Both |
| 61 | `scope_boundary` always contains "Generate only what is specified." instruction | REQ-PEM-007 | Both |
...
| R1-F1 | Distinguish seed parsing failures (abort) from secondary resource failures (degrade) in resolve_seed_context pipeline error handling |  | This is substantively the same concern as R1-S1 and should be accepted for the same reasons. The distinction between unrecoverable seed corruption and recoverable secondary resource failures is architecturally important and aligns with existing standalone strategy behavior. Merging with R1-S1 for implementation. | 2026-02-24 17:36:22 UTC |
| R1-F2 | Specify that ContextFileEntry.content is populated during resolve_seed_context() with loading failures handled per strategy error semantics |  | This is substantively the same concern as R1-S6 and should be accepted for the same reasons. The content loading lifecycle must be specified for strategies to implement correctly. Eager loading during resolve_seed_context() with per-strategy error handling is the right approach. Merging with R1-S6 for implementation. | 2026-02-24 17:36:22 UTC |
| R1-F3 | Document the known limitation that staleness detection based on seed checksum doesn't cover changes to externally referenced files |  | This is substantively the same concern as R1-S8 and should be accepted for the same reasons. The limitation is inherent to the design and should be documented with operator guidance rather than engineered away. Merging with R1-S8 for implementation. | 2026-02-24 17:36:22 UTC |
| R1-F4 | Specify that on cache hit, existing output files including generation-manifest.json must not be modified |  | This is substantively the same concern as R1-S9 and should be accepted for the same reasons. Downstream consumers rely on manifest timestamps; specifying no-modification-on-cache-hit prevents unnecessary downstream processing. Merging with R1-S9 for implementation. | 2026-02-24 17:36:22 UTC |
| R1-F1 | Require exception chaining when raising ContextResolutionError to preserve the root cause for diagnostics |  | This is a concrete, low-cost addition that significantly improves debuggability. Without exception chaining, the original FileNotFoundError or JSONDecodeError is lost, and operators see only 'ContextResolutionError: failed to load context file' without knowing whether it was a permission error, missing file, or corrupt data. Python's `from original_exception` pattern is standard practice and should be mandated for a typed exception that explicitly exists for error categorization. | 2026-02-24 22:27:33 UTC |
| R1-F2 | Define minimal content specifications for always-present context sections (general_context and scope_boundary) when feature metadata is minimal |  | REQ-PEM-007 marks general_context and scope_boundary as 'always present' but only describes their content in the context of full pipeline enrichment. For standalone mode or minimal add_feature() usage (REQ-PEM-010), the content of these sections is undefined. Since these sections are always included in the LLM prompt, their content directly affects generation quality. Specifying that general_context always contains feature name, description, and target files, and that scope_boundary always contains the generation constraint instruction, is a minimal clarification that prevents empty or nonsensical always-present sections. | 2026-02-24 22:27:33 UTC |
...
| R1-F3 | Acknowledge that SeedContext immutability is never enforced on cache-hit paths where _generate_code() is never called |  | The suggestion itself acknowledges this is unlikely to cause real bugs and proposes only adding an explanatory note. REQ-PEM-005 already specifies the lifecycle constraint clearly. On cache-hit paths, no mutation opportunity exists (the workflow exits early), so the immutability contract is trivially satisfied by the absence of mutation, not by enforcement. Adding documentation about a non-problem adds complexity without value. The existing specification is correct and complete. | 2026-02-24 22:27:33 UTC |
| R1-F4 | Record the strategy class name in the generation manifest for provenance when custom strategies are injected |  | Custom strategy injection is an advanced extension point mentioned once in REQ-PEM-004's acceptance criteria. The standard case uses one of two built-in strategies determined by the execution mode, which is already recorded in the manifest. Recording strategy class names adds minimal diagnostic value — the effective_config and execution_mode already identify the behavior profile. If a custom strategy is used, the operator who injected it knows which one. This is an implementation nicety, not a requirements gap. | 2026-02-24 22:27:33 UTC |
...
- **Reviewer**: claude-4 (claude-opus-4-6)
- **Date**: 2026-02-24 17:35:39 UTC
- **Scope**: Architecture-focused review (Feature Requirements)

#### Feature Requirements Suggestions
| ID | Area | Severity | Suggestion | Rationale | Proposed Placement | Validation Approach |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| R1-F1 | interfaces | high | REQ-PEM-004 error recovery for `resolve_seed_context` failure should distinguish between seed-level failures (corrupt JSON, missing seed file) and secondary resource failures (unreadable context file, plan document I/O error) — current text says "abort the workflow" for all failures | The blanket abort policy is too aggressive. A corrupt seed file is fundamentally unrecoverable, but a missing context file is recoverable with degraded context. The requirement should specify: seed parsing failure → abort; secondary resource failure → degrade with warning (skip the failed resource, continue with partial context). This aligns with the standalone strategy's existing behavior (REQ-PEM-006: "log a warning and omit the file") and provides consistent error handling across strategies. | REQ-PEM-004, pipeline mode error handling paragraph | Test: valid seed with unreadable context_file; verify pipeline mode logs warning and continues (not aborts) |
| R1-F2 | data | medium | REQ-PEM-005 `ContextFileEntry.content` field lifecycle is unspecified — no requirement states when content is loaded, whether loading failures are recoverable, or how large files are handled | The TypedDict defines the field as `NotRequired[str]` but the population timing is absent from both REQ-PEM-005 and the strategy requirements (REQ-PEM-006, REQ-PEM-007). Strategies need to know whether they receive pre-loaded content or must load it themselves. If pre-loaded, memory concerns arise for large files. If strategy-loaded, error handling must be specified per strategy. | REQ-PEM-005, after ContextFileEntry definition: "content is populated by the ContextResolutionStrategy during resolve_seed_context(). Loading failures for individual files MUST be handled per the strategy's error semantics (standalone: warn and omit; pipeline: warn and omit per REQ-PEM-006/007 file error handling)." | Test: strategy receives ContextFileEntry with path but no content; verify content is populated after resolve_seed_context() |
| R1-F3 | ops | medium | REQ-PEM-013 staleness detection based on seed checksum has a documented blind spot: changes to external files referenced by the seed (plan documents, context files) don't change the checksum, leading to stale cache hits | This is an inherent limitation of content-addressable caching based on the seed alone. The requirements should acknowledge this limitation and provide operator guidance. Without documentation, operators will encounter stale results after updating plan documents and file bug reports. | REQ-PEM-013, after checksum computation paragraph: add "Known limitation" note documenting the external file blind spot and recommending operators bump a seed field when external files change | Documentation review: limitation is present and operator guidance is actionable |
| R1-F4 | validation | medium | REQ-PEM-012 doesn't specify manifest behavior on cache hit (staleness match) — should the manifest be preserved, updated with new timestamps, or rewritten with identical content? | Downstream consumers of the manifest (Stage 7 validators, OTel dashboards) may use the manifest's `generated_at` field or file modification time for their own staleness logic. If the Prime Contractor rewrites the manifest on cache hit (even with identical content), it changes the file mtime and could trigger unnecessary downstream processing. If it preserves the file, `generated_at` accurately reflects when generation actually occurred. | REQ-PEM-012 or REQ-PEM-013: add acceptance criterion: "On cache hit (staleness check passes), existing output files including generation-manifest.json MUST NOT be modified." | Test: cache hit scenario; verify generation-manifest.json mtime is unchanged |

#### Review Round R1

- **Reviewer**: claude-4 (claude-opus-4-6)
- **Date**: 2026-02-24 22:26:38 UTC
- **Scope**: Architecture-focused review (Feature Requirements)

#### Feature Requirements Suggestions
| ID | Area | Severity | Suggestion | Rationale | Proposed Placement | Validation Approach |
| ---- | ---- | ---- | ---- | ---- | ---- | ---- |
| R1-F1 | interfaces | high | REQ-PEM-004 defines `ContextResolutionError` as a subclass of `RuntimeError` but doesn't specify whether strategy implementations should chain the original exception — without chaining, the root cause (e.g., `FileNotFoundError`, `json.JSONDecodeError`) is lost in error logs | When a strategy catches an I/O error and wraps it in `ContextResolutionError`, the original traceback and exception type are diagnostic-critical. Python's `raise ContextResolutionError(...) from original_exc` preserves this. Without requiring chaining, implementations may discard the root cause, making pipeline debugging significantly harder. | REQ-PEM-004, ContextResolutionError definition: add: "ContextResolutionError MUST be raised with exception chaining (`raise ContextResolutionError(msg) from original_exception`) to preserve the root cause for diagnostic logging." | Test: strategy encounters FileNotFoundError; verify ContextResolutionError.__cause__ is the original FileNotFoundError |
| R1-F2 | validation | high | REQ-PEM-007 specifies 9 structured context sections with presence/absence rules but the "always present" sections (`general_context`, `scope_boundary`) have no definition of what they contain when the feature has minimal metadata — e.g., what does `general_context` contain for an `add_feature()` call with only name and description? | The requirement says these sections are "always present" but their content schema is only described for the pipeline case with full enrichment. In standalone mode with a minimal feature (REQ-PEM-010), `general_context` would contain name, description, target_files — but this isn't stated. Without specifying the minimal content of always-present sections, implementations may produce empty sections that waste LLM tokens or omit critical feature identity information. | REQ-PEM-007, after the section list: add minimal content specification for always-present sections: "`general_context` MUST always contain at minimum: feature name, feature description, and target file paths. `scope_boundary` MUST always contain the instruction 'Generate only what is specified.'" | Test: add_feature with minimal args; verify general_context and scope_boundary have specified minimal content |
| R1-F3 | data | medium | REQ-PEM-005 `SeedContext` lifecycle says "treated as immutable after the execution phase begins" but REQ-PEM-013 staleness detection may skip all generation — in the skip case, execution never "begins" per the definition (first `_generate_code()` call), meaning SeedContext remains mutable for the entire workflow lifetime on cache hits | This is a semantic edge case: if staleness detection causes early exit, `_generate_code()` is never called, so the immutability trigger never fires. While this is unlikely to cause real bugs (no code mutates SeedContext after staleness check), the lifecycle contract is technically never enforced on cache-hit paths. The requirement should either broaden the trigger or acknowledge this as acceptable. | REQ-PEM-005, lifecycle section: add: "If staleness detection causes the workflow to skip generation entirely, SeedContext immutability is not enforced (no mutation opportunity exists). This is the expected behavior — immutability enforcement prevents modification during active generation, which does not occur on cache hits." | Review: lifecycle specification accounts for all exit paths |
| R1-F4 | architecture | medium | REQ-PEM-012 manifest `effective_config` captures ModeConfig state but doesn't capture strategy-specific configuration — if a custom strategy is injected (per REQ-PEM-004: "accepts an optional `context_strategy` parameter"), the manifest has no record of which strategy was used | The manifest records mode and config but not strategy identity. Two runs with the same mode but different strategies (e.g., a custom `EnhancedPipelineStrategy`) would produce identical manifests despite potentially different context resolution behavior. For provenance and debugging, the strategy class name should be recorded. | REQ-PEM-012, manifest schema: add `"strategy": "PipelineContextStrategy"` field populated from the strategy's class name | Test: custom strategy injected; verify manifest records custom strategy class name |
