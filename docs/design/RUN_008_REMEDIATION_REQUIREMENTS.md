...
- **OQ-3 (load-bearing, partially resolved by spike) — verification host & timing.** FR-4/FR-5 require `node` + `tsc`/`prisma` and `npm install` + `prisma generate` to have run. **Spike finding:** the run-008 workspace had **no `node_modules`/`tsc`/`prisma`** — verification capability never existed there, so the pipeline must *provision* it, not assume it. The contained spike confirmed a clean `npm install` (11 pkgs, ~14s) + `prisma generate` + `tsc --noEmit` runs fine in a throwaway dir. **Still open:** where this executes in the pipeline (in-process post-batch / sandbox / separate gate) and whether `npm install`+`prisma generate` run per-batch or against a warm cache. This is the biggest remaining implementation unknown and drives FR-9.
