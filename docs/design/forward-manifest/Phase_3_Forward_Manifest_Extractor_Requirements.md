...
* **Import/dependency population:** Phase 3 leaves `ForwardFileSpec.imports` / `.dependencies` at their
  defaults (empty list / `None`) — the *manifest* does not carry imports. **This is not a deferral to "Phase 4
  may populate"** (the original framing, now corrected): under deterministic generation, import completeness is
  the **emitter's** responsibility at render time, per the invariant below. Leaving the manifest's imports empty
  is intentional; producing an import-complete *skeleton* is required.
