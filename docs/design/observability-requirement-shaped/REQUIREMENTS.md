...
**Version:** 0.7 (OQ-5 resolved — Mastodon pilot located + byte-verified; FR-6 series convention-sourced)
**Date:** 2026-07-22
...
- **OQ-5 → RESOLVED (2026-07-22): pilot located + byte-verified** (`~/Documents/dev/OSS/mastadon/`). Confirmed the worker got HTTP SLOs and 6/7 FRs produced nothing; the pivotal finding is that workers may emit **no native metrics**, so FR-6 series are **convention-sourced** (OTel `messaging.*` / exporter), not sniffed. Full findings + 2 incidental bugs in §6.
...
- **FR-6 — Kind→profile table (general, not one row).** Extend `MetricDescriptor._PROFILES` with a **table** of workload profiles — ship `http_server`, `grpc_server`, `async_worker`, **`batch`, `cron`, `stream`** (+ their SLI series/selectors) — plus one `kind→profile` map beside `_TRANSPORT_DEFAULTS`; thread `kind` into `resolve_descriptor` so kind wins over transport. `profile_for_transport`'s HTTP fallback survives **for the request-serving family only**. The requirement is the *table + resolution tier*; each row is ~4 additive lines. No service shall receive an SLI on series it does not emit (e.g. a worker on `http_server_duration`).
  - **FR-6a — Profile series are convention-sourced, not subject-sniffed (grounded by OQ-5).** The Mastodon pilot proved workers may emit **no** native Prometheus metrics (traces-only). Therefore each non-request profile binds to a **declared metrics convention**, not auto-detection: prefer **OTel messaging semconv (`messaging.*`)** for queue/stream signals; fall back to a named exporter convention (Sidekiq/`yabeda-sidekiq`) or the FR's `custom` PromQL. When a declared `signal_kind` has no emitting series at all (e.g. FR-007 freshness), the artifact is still derived (recording-rule/derived-SLO) and the gap is reported via FR-9's unfulfilled class — never silently dropped, never mis-bound to an HTTP series.
- **FR-7 — Per-SLI-kind default thresholds, *composed onto the #234 importance table* (revised — see §0.4).** A `freshness`/`queue_depth`/… FR on *any* service gets a signal-kind-appropriate default, decoupled from the service's kind. **Composition, not a new mechanism:** PR #234 (merged 2026-07-22) shipped `config/importance_thresholds.yaml` + `_select_importance_default` + `load_importance_thresholds`, a `<criticality>.<mode>.<field>` table resolved in `_resolve_threshold` (`artifact_generator_generators.py:129`). That resolver is **already generic over `field_name`**, so FR-7 is delivered by: (i) adding the non-request `signal_kind` cells (`queue_depth`/`retry_rate`/`freshness`/`run_success`/`lag`/`saturation`) into the **same** `importance_thresholds.yaml` under each `<criticality>.<mode>` cell — as `field_name`s alongside `availability`/`latency_p99` (→ a **criticality × mode × signal_kind** table; the `installed`/`deployed` mode rows were populated by PR #247, so the composed structure is **3-level** `<criticality>.<mode>.<field>`, not 2-level — add signal_kind fields under each `<criticality>.<mode>`, not directly under `<criticality>`), and (ii) seeding a criticality-agnostic baseline for each in the flat `_DEFAULT_THRESHOLDS`. **No new resolution code** — the existing manifest→importance→flat tiers apply unchanged (extend-vs-build-separate; Accidental-Complexity). `business.default_thresholds` override plumbing already exists.
...
- **OQ-5 → RESOLVED (2026-07-22).** The pilot was located at `~/Documents/dev/OSS/mastadon/` (dir spelled "mast**a**don"; run `.cap-dev-pipe/pipeline-output/mastodon-status-fanout/run-003-20260722T1043/`). Findings, **byte-verified**:
  - **Wrong output confirmed:** `slos/mastodonsidekiq-slo.yaml` gives the Sidekiq worker two `protocol: http` SLOs on `http_server_duration_count`/`_bucket` (`status=~"5.."`). **Missing confirmed:** grep of the whole `observability/` tree finds **zero** `sidekiq_*`/`queue_*`/`retry_*`/`freshness_*` metric series — FR-002..007 produced no artifacts (~1/7 loosely covered).
  - **Pivotal grounding for FR-6:** the subject emits **no native async metrics** — its OTel is traces-only and Sidekiq is observed via spans, not Prometheus series. So the `async_worker`/`stream`/`batch` profile series **cannot be auto-detected from the subject**; they must come from a **declared exporter convention** (Sidekiq/`yabeda-sidekiq` exporter, or OTel **messaging semconv `messaging.*`**). The only concrete real async series the pilot contains are on the streaming (Node) service: `redis_messages_received_total`, `messages_sent_total`, `connected_clients`, `connected_channels`, `pg_pool_*`. → **This strengthens the contract-first stance (FR-12): for workers, the metric surface is *declared*, not sniffed.**
  - **FR-007 fan-out freshness has no native metric** → a *derived* SLO / an expected-positive — validates FR-9's `resolved≠∅, produced=0` (unfulfilled) class as the correct treatment.
  - **Thresholds are deliberately unspecified** in the pilot reqs (derivation is what the pipeline is scored on) — so FR-7 values come from the criticality×signal_kind table (§0.4), not the pilot.
  - **Two incidental bugs surfaced** (candidate issues): (a) a **phantom `mastadon-slo.yaml`** service — the workspace-dir basename got a full artifact set (service-id extraction false-positive, the OB-calibrated heuristic at `artifact_generator_context.py:220/234/245`); (b) run-003 already resolves thresholds via #234's importance table (99.5%/400ms for `high`), so the issue body's "99/500ms" is stale.
  - **Remaining validation (post-CR-3):** a fresh minimal pilot on a subject that *does* export worker metrics, to confirm FR-6's convention series bind correctly end-to-end.
...
*v0.7 — OQ-5 RESOLVED. The Mastodon status-fanout pilot was located (`~/Documents/dev/OSS/mastadon/`) and its claims byte-verified: worker got HTTP SLOs, 6/7 FRs → zero artifacts. Pivotal finding: the async tier emits NO native metrics (OTel traces-only), so FR-6 profile series are convention-sourced (OTel `messaging.*` / exporter), not subject-sniffed — added FR-6a. Surfaced 2 incidental bugs (phantom dir-basename service; stale threshold numbers in the issue body). Thresholds stay in the §0.4 criticality×signal_kind table (pilot deliberately unspecified them).*
