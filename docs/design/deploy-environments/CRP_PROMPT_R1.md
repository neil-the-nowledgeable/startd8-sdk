...
**Generated:** 2026-06-21 15:22:01 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-cnd-m1/docs/design/deploy-environments/DEPLOY_ENVIRONMENTS_PLAN.md` | 93 lines · 861 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-cnd-m1/docs/design/deploy-environments/DEPLOY_ENVIRONMENTS_REQUIREMENTS.md` | 141 lines · 1260 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-cnd-m1/docs/design/deploy-environments/CRP_FOCUS_R1.md` | 27 lines · 213 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-cnd-m1/docs/design/deploy-environments/DEPLOY_ENVIRONMENTS_PLAN.md`  ·  **Size:** 93 lines · 861 words
...
**Version:** 1.0
**Date:** 2026-06-21
**Tracks:** `DEPLOY_ENVIRONMENTS_REQUIREMENTS.md` (v0.2)
...
- M2: contract lists per-env bindings.
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-cnd-m1/docs/design/deploy-environments/DEPLOY_ENVIRONMENTS_REQUIREMENTS.md`  ·  **Size:** 141 lines · 1260 words
...
- **FR-ENV-2 (Orthogonal to mode).** Environment SHALL be independent of `deployment.mode`. A
  `deployed` app supports N environments; `installed` is a single local environment.
- **FR-ENV-3 (Per-environment value set).** Each environment MAY override: replicas, resources,
  hostnames, `OTEL_EXPORTER_OTLP_ENDPOINT`, log level, secrets scope (Doppler config), autoscaling,
...
- **FR-ENV-7 (Secrets per environment).** Each environment SHALL map to a secrets scope — for the
  default Doppler backend, an environment ↔ Doppler **config** (e.g. dev→`dev`, prod→`prd`) bound to
  the per-env `ExternalSecret`/`SecretStore` ref. The SDK emits the reference, never the values.
- **FR-ENV-8 (Determinism + SOTTO).** No environments declared → byte-identical to today (single
  mode-derived `ENV`, no overlays). Deterministic, $0, drift-checked.
...
> OQ-1..OQ-6 resolved by the planning pass — see §0. Remaining for CRP:

- **OQ-7 (new).** Should per-env DB references live in the overlay (a per-env `DATABASE_URL` via
  ExternalSecret) only, or also be surfaced as named bindings in the contract for non-secret DSNs?
- **OQ-8 (new).** kustomize is the v1 mechanism — should a non-kustomize fallback (plain per-env
  ConfigMaps applied directly) be offered for clusters without kustomize, or is `kubectl -k` assumed?

---

*v0.2 — Post-planning self-reflective update. FR-ENV-4 marked satisfied-by-construction, FR-ENV-6
narrowed (value-binding not new wiring), mechanism resolved to kustomize base+overlays, 6 open
questions resolved, 2 new for CRP. Headline: the app is already environment-ready at runtime — this is
a config-emission capability (declaration grammar + overlays + per-env contract), not app code.*
