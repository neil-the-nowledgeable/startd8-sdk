...
**Generated:** 2026-07-10 06:28:23 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-triage-panel/docs/design/stakeholder-panel/TRIAGE_PANEL_PLAN.md` | 119 lines · 950 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-triage-panel/docs/design/stakeholder-panel/TRIAGE_PANEL_REQUIREMENTS.md` | 189 lines · 1820 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-triage-panel/docs/design/stakeholder-panel/crp-focus-triage-panel.md` | 43 lines · 362 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-triage-panel/docs/design/stakeholder-panel/TRIAGE_PANEL_PLAN.md`  ·  **Size:** 119 lines · 950 words

```markdown
# Grafana Triage Panel Mode — Implementation Plan

**Version:** 1.1 (Post-scope-decision — full VIPP write path)
**Date:** 2026-07-10
**Requirements:** `TRIAGE_PANEL_REQUIREMENTS.md` v0.2
...
| `POST /extract` (confirm) | `{session_id?, confirm_checksum, max_cost_usd?}` | `{status:"staged"\|"deduped", staged[], actual_cost, ceiling_exceeded, ...}` · 409 on checksum mismatch · 412 budget/ceiling · 422 no synthesis |
| `POST /disposition` | `{session_id, domain, value_path, disposition}` | `{..., updated:true}` · 400 bad disposition · 404 not staged |
| `POST /serialize` | `{session_id}` | `{staged, rejected, inbox}` · 409 none accepted · 404 none staged |
...
### M1 — Server: `backlog_markdown` on the triage route (the ONE Python change)  [FR-4]
- `_triage`: after `build_triage`, compute `render_backlog_section(report, project=<root name>)`; add
  `backlog_markdown` to the response (`""` when no candidates / no synthesis).
- **Test** (`tests/unit/…/test_stakeholder_run_server_*`): triage response carries `backlog_markdown`
  (non-empty with candidates; `""` when synthesis absent). Real, in-pytest.
...
### M3 — `TriagePanel.tsx` Phase 1: triage view (read-only)  [FR-2, FR-3, FR-5]
...
### M4 — Phase 2: Extract (the paid step), cost-gated  [FR-8]
- **Preview cost** (`dry_run:true`) → estimate + `synthesis_checksum` → **ConfirmModal** (echo estimate)
  → confirm (`confirm_checksum`) → render `staged[]` + `actual_cost` + `ceiling_exceeded`/`deduped`.
- Mirror the run/facilitate preview→confirm idiom; the paid step is behind its own confirm modal so it
  can't fire accidentally (OQ-6). Surface 412 (budget)/409 (checksum)/422 (no synthesis) honestly.

### M5 — Phase 3: Disposition  [FR-9]
- Per staged rec: Accept / Reject → `proxyPost('stakeholders/disposition',{session_id,domain,
  value_path,disposition})`. Track per-rec state; surface the 404 "stage it first" clearly.

### M6 — Phase 4: Serialize + hand-off  [FR-10, FR-11]
- **Serialize accepted** → `proxyPost('stakeholders/serialize',{session_id})` → show `{staged,
  rejected, inbox}` + a note: "switch to **Apply** mode to preview → ratify the inbox." No apply here.
...
## Test / validation
- **Python:** M1 route test (real). No other server change → existing route tests still cover the write
  routes.
- **TS:** typecheck-pending (no `node_modules`; Actions disabled). Thin driver over tested routes;
  documented manual verify + the dormant `grafana-plugin.yml` CI gate will cover it once Actions is on.

## Requirement → milestone trace
FR-1→M2/M3 · FR-2→M3 · FR-3→M3 · FR-4→**M1**/M3 · FR-5→M3 · FR-6→M2/M3 · FR-7→M7 · FR-8→M4 ·
FR-9→M5 · FR-10→M6 · FR-11→M6.

## Risks
- **R1 — paid action in a dashboard.** Extract spends. Mitigated: dry-run→confirm modal + server
  fail-closed budget (412) + `max_cost_usd` ceiling + checksum echo (can't confirm a stale preview).
...
- **R3 — multi-step state in one component.** Mitigated: explicit phase state machine (idle→triaged→
  staged→dispositioned→serialized), mirroring ApplyPanel's phased approach.

*v1.1 — full write path. M1 is the only server change; M3–M6 are the new stepped panel; Apply mode
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-triage-panel/docs/design/stakeholder-panel/TRIAGE_PANEL_REQUIREMENTS.md`  ·  **Size:** 189 lines · 1820 words
...
- **FR-8 — Extract (the paid step), cost-gated.** A **dry-run → confirm** flow mirroring run/facilitate:
  ...
  the server fail-closes on the blocking budget (412) and honors `max_cost_usd`. Surface `actual_cost`,
  `ceiling_exceeded`, and the `deduped` idempotent-replay state. The confirm button shows the estimate.
...
- **FR-10 — Serialize.** `POST /stakeholders/serialize` pushes *accepted* staged recs to the VIPP
  inbox; surface `{staged, rejected, inbox}`. Guides the operator to the **Apply mode** to ratify.
...
- **NR-6 — No new synthesis_bridge logic** — the panel is a thin driver over existing routes; the only
  server change is surfacing `backlog_markdown` on the triage route (FR-4).
