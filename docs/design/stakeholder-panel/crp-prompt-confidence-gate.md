...
**Generated:** 2026-07-10 17:10:24 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-confidence-gate/docs/design/stakeholder-panel/CONFIDENCE_GATED_APPLY_PLAN.md` | 89 lines · 844 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-confidence-gate/docs/design/stakeholder-panel/CONFIDENCE_GATED_APPLY_REQUIREMENTS.md` | 132 lines · 1161 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-confidence-gate/docs/design/stakeholder-panel/crp-focus-confidence-gate.md` | 38 lines · 366 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-confidence-gate/docs/design/stakeholder-panel/CONFIDENCE_GATED_APPLY_PLAN.md`  ·  **Size:** 89 lines · 844 words
...
**Date:** 2026-07-10
**Requirements:** `CONFIDENCE_GATED_APPLY_REQUIREMENTS.md` v0.1
...
## Approach (FLAG core — HARD-gate is OQ-1, sponsor's call)

### M1 — Server: thread provenance + compute consensus at preview  [FR-1/2/6/7]
1. **Write:** `serialize_buffer(buffer, root, *, source_session_id=None, …)` → add `"source_session_id"`
   to the envelope dict. `serialize_accepted_to_vipp(…, source_session_id=None)` forwards it.
   `_serialize` route passes its `session_id`.
2. **Read:** `ProposalEnvelope` += `source_session_id: str = ""` (in `from_json` via `.get`, in `to_dict`).
3. **Surface:** `PreviewResult` += `source_session_id: str = ""`; `preview_dispositions` sets it from the
   envelope. (vipp stays pure — no consensus import here.)
4. **Compute:** `_apply_preview` — when `preview.source_session_id`, load
   `KickoffViewService(project).load(sid).rounds` → `compute_consensus(rounds, exclude_role_ids=CHALLENGER_IDS)`
   → add `"consensus": result.to_dict()` to the response. Missing session/transcript/≤1 rateable → n/a
   (helper already degrades); best-effort (a consensus failure must never break the preview).
5. **Tests:** serialize→envelope carries `source_session_id`; old inbox (no field) → preview `consensus`
   n/a; a completed-session inbox → preview carries a real consensus label; **content_hash + challenge
   unchanged** vs before (regression: the ratify path is byte-identical).
...
- `ApplyPanel.tsx` preview screen: a consensus chip (reuse the #6 chip pattern) with the synthetic/lexical
  caveat; hidden when n/a. **Real verify:** `npm ci && typecheck && lint && test && build`.
...
FR-1→M1.4 · FR-2→M1.1–3 · FR-3→(FLAG core; M3 if gate) · FR-4→M1.4/M2 · FR-5→M2 · FR-6→M1.4 · FR-7→M1 (hash-safe by construction).

## Risks
- **R1 — write-boundary security.** Mitigated: provenance is envelope metadata outside the content hash;
  the challenge/nonce/strict/confinement gate is untouched; M1.5 regression asserts byte-identical ratify.
- **R2 — two envelope shapes drift.** Mitigated: add the field to both in the same change + a round-trip
  test (write via serialize_buffer → read via ProposalEnvelope.from_json → field survives).
- **R3 — coupling vipp to facilitation.** Avoided: consensus computed in the route, not in vipp.

*v1.0 — planning pass. Headline: session provenance is LOST at serialize → #8's real work is threading
`source_session_id` (hash-safe, additive); consensus at preview is then a thin reuse of #6.*
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-confidence-gate/docs/design/stakeholder-panel/CONFIDENCE_GATED_APPLY_REQUIREMENTS.md`  ·  **Size:** 132 lines · 1161 words
...
- **FR-1 — Consensus in the apply preview.** `_apply_preview` returns `consensus: {label, score, n,
  basis}` for the source facilitation, via `compute_consensus` (reused).
- **FR-2 — Session provenance.** `source_session_id` is carried serialize → inbox envelope → preview
  (added to the write dict + the read model + `PreviewResult`).
...
- **FR-5 — Grafana panel.** `ApplyPanel.tsx` renders the consensus on the preview screen (chip + caveat;
  hidden when n/a).
- **FR-6 — Graceful n/a.** No `source_session_id` (old inbox), missing transcript, ask-all source, or ≤1
  rateable persona → consensus **n/a**; preview renders cleanly. Consensus compute is best-effort — a
  failure never breaks the preview.
- **FR-7 — Do not weaken apply security.** Provenance is a top-level envelope field, **outside** the
  content hash the ratify challenge binds to; the gate is byte-identical (regression-tested).

## 3. Non-Requirements

- **NR-1 — No new consensus logic** (reuse `compute_consensus`).
- **NR-2 — No server-side apply refusal** on low consensus (FLAG core; see OQ-1).
- **NR-3 — No change to the ratify content hash / challenge.**
