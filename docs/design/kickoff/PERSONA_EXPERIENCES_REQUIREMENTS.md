...
- **OQ-2 → ledger, not FieldDef.** `persona-default:<slug>` extends the ledger entry; bump schema to v2.
- **OQ-3 → delimiter-region model required.** Feasible only as marked regions in the same doc (FR-9).
- **OQ-4 → filter is trivial, but writing needs a pre-pass.** `awaiting_fields` gains one predicate; a new `apply_persona_defaults` does the writing (FR-11).
- **OQ-5 → new `persona_defaulted` count bucket.** Otherwise persona-defaults masquerade as human-confirmed in `domain_confirmation` (FR-13).
- **OQ-6 → guided view-model + parity digest, not `KickoffState`.** (FR-14.)
- **OQ-7 → `manifest.py` config layer.** Profiles are in-process data (`PERSONA_PROFILES`), not packaged/downloaded files.
...
- **[Phantom-reference audit]** — grepped every existing symbol the spec leans on. All verified
  ...
  **`kickoff_experience/concierge_view.py`** (`:675` / `:719`), not `concierge/concierge_view.py`.
  Corrected in the plan. New symbols (`resolve_persona_preference`, `apply_persona_defaults`,
  `PERSONA_PROFILES`) are to-be-created and marked as such.
- **[Overloaded-term co-location] — MATERIAL.** "persona" is **already an owned term** in this
  ...
  all **code-level** symbols for this feature are namespaced to a distinct term (working name
  **`audience`** — `KickoffAudience` enum, `audience.py`, `AUDIENCE_PROFILES`, `resolve_audience`),
  reserving "persona" for the existing roster concept. The **user-facing** label (CLI verb, prose)
  is a separate product decision — see **OQ-12**. (The doc keeps the user's word "persona" in prose
  pending that decision; only the code namespace is fixed now.)
...
| Decision provenance | Ledger records `{value, at, mode ∈ set|as-is}` | No `persona-default:<slug>` provenance; `domain_confirmation` can't distinguish it |
...
- **FR-1** *(revised — D-1)* Persona is an explicit, project-remembered, changeable selection that
  ...
  `resolve_persona_preference` clone of `resolve_guided_preference`.
...
- **FR-3** A user can view and change the current persona (`kickoff persona [show | set <slug>]`),
...
- **FR-4** Persona is a **presentation + default-selection** projection only. Given the **same
  explicit decisions**, output (`inputs/`, `confirmed.yaml` values) is **byte-identical** across
  personas. (Enforced by a golden test, not new runtime logic.)
...
- **FR-6** *(revised — D-3)* Every persona-written value carries **ledger** provenance distinguishing
  ...
  `persona-default:<slug>` provenance and bump `LEDGER_SCHEMA` to `kickoff.confirmed.v2`
...
- **FR-7** *(sited — OQ-7)* Each persona has a **default profile** in the `manifest.py` config layer
  (`PERSONA_PROFILES: dict[slug, dict[value_path, value]]`) — a single-source, in-process
...
- **FR-11** *(revised — D-4)* Beginner **reduces** the prompted surface via a **pre-pass**
  (`apply_persona_defaults`) that, for each reduced-surface field not yet in the ledger, writes the
  persona default through the existing `build_confirm_plan` + `apply_confirm` machinery (tagged
  `persona-default:<slug>`). The written fields then drop out of the **unchanged** `awaiting_fields`;
...
- **FR-12** *(scoped — batch of existing as-is)* Advanced exposes the **full** surface with a
  **confirm-all** path: a batch loop over `awaiting_fields` calling `build_confirm_plan(mode="as-is")`
  + `apply_confirm` for each (reuses the single-field `--as-is` machinery that already exists).
- **FR-13** *(revised — OQ-5)* A persona-defaulted field must be **distinguishable from a human
  confirmation** so a beginner can later reach shielded decisions: `domain_confirmation` gains a
  `persona_defaulted` bucket (routed by the FR-6 provenance) instead of counting them as
  `confirmed`; `assess` surfaces it; `kickoff confirm <vp>` re-writes a persona-default into an
...
- **OQ-8** Provenance encoding in the ledger entry: a **new `mode` value** (`mode: "persona-default"`
...
- **OQ-9** For Beginner reduced surface, *which specific fields/domains* are shielded? Is it
  ...
  the `PERSONA_PROFILES` shape and the `awaiting_fields` predicate.)
- **OQ-10** Should `kickoff persona set` re-run the pre-pass immediately (apply beginner defaults on
...
- **OQ-12** *(from §0.1 hardening)* **User-facing label.** Code symbols are namespaced to `audience`
  to avoid the `stakeholder_panel` "persona" collision. What word should the **user** see —
  `startd8 kickoff persona`, `... audience`, `... experience`, or `... level`? The collision is only
  a code-readability hazard; the surface word is a product call. (Recommendation: keep user-facing
  "persona" for warmth, code stays `audience`; but flag that CLI help text near `cli_panel.py`'s
  persona commands could still confuse.)
