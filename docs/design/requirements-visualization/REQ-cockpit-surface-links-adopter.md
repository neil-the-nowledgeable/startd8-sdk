...
- **FR-1 — Web cockpit field rows carry a drill link.** The web overview's per-field status row (`web.py` overview renderer) includes an anchor `<a href="…">` whose `href` is `resolve_surface_link_href(resolved.surface_links["drill"], field_key)`, so a user clicks a cockpit field → lands on the navig8r's `#<key>` full-page view. The link renders only when the href is non-empty (empty-default). Touches: `src/startd8/kickoff_experience/web.py`. Verify: (a) a rendered overview HTML for a project with surface_links contains an `<a>` tag whose href equals `#<field_key>` for each field; (b) when `surface_links` is absent/empty, no link renders (byte-identical to before); (c) `resolve_surface_link_href` is called with the drill binding dict and the field's key — no bespoke URL construction.

- **FR-2 — Grafana v2 board field rows carry a markdown drill link.** The `portal_spec_v2.build_workbook_v2` field-row rendering includes a markdown link `[→ navig8r](#{key})` per field, so the Grafana text panel is navigable to the navig8r route. Renders only when drill href is non-empty. Touches: `src/startd8/kickoff_experience/portal_spec_v2.py`. Verify: (a) a built v2 dashboard JSON for a project with surface_links contains the markdown link pattern `[→ navig8r](#<key>)` in a text panel; (b) absence of surface_links → no link text.

- **FR-3 — Terminal cockpit view annotates the key for copy-paste (DEFERRED).** The Rich-rendered terminal cockpit (`cockpit_view.py`) currently shows only aggregate attention counts, not per-field rows. A per-field `#<key>` annotation requires per-field rendering that doesn't yet exist. **Deferred** to a follow-on that adds per-field terminal rows. Touches: `src/startd8/kickoff_experience/cockpit_view.py` (future). Verify: (deferred — no current test).

- **FR-4 — Zero diff on shared definition + navigator.** The navigator's `view_definition.py`, `wireframe_view/_template.py`, and `graph_projection.py` have zero diff. The adopter reads `surface_links` from the resolved definition — it does not modify, extend, or re-declare it. Touches: (none — this is a no-change assertion). Verify: `git diff --stat src/startd8/navigator/ src/startd8/wireframe_view/` shows 0 files changed.

- **FR-5 — Empty-default: no regression on definitions without drill href.** A resolved definition whose `surface_links` is absent, empty, or whose `drill.href` is falsy produces no drill link on any of the three surfaces. Existing rendering is byte-identical for those cases. Touches: `tests/unit/kickoff/test_cockpit_drill_link.py`. Verify: (a) pass a definition with `surface_links={}` → render has no link; (b) pass a definition with `surface_links.drill` but `href=""` → render has no link.
