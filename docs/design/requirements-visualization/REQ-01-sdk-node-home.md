...
- **FR-2 — Typed grounding survives compose.** WireframeItem gains optional `key`, `lives` (typed tuples), `confidence`, `ships_when`, `was`, `route_state` (defaults empty/None); `compose` `_item_view` emits them **only when set** (omit-when-empty). Existing `paths` remains populated from lives refs for back-compat. Name: WireframeItem carries optional typed grounding fields that compose emits only when set, keeping navigator grounding without changing the classic app path. Touches: `src/startd8/wireframe/plan.py`, `src/startd8/wireframe_view/compose.py`. Lives: code src/startd8/wireframe/plan.py. Lives: code src/startd8/wireframe_view/compose.py. Approve?: are typed lives visible in compose JSON when set · omitted when empty?. Verify: (a) navigator Node with typed lives + confidence 0.9 → compose item JSON includes type+ref and confidence; (b) classic app WireframeItem compose JSON gains **no** new keys. Serves: O-2
- **FR-3 — Derived status + honest-skip gap rule.** Status is derived from evidence × maturity; nodes with `route_state` in `{owned_elsewhere, declared_unimplemented}` are excluded from attention/gap counts (profile `is_gap=False` or need_items filter) — do **not** overload app `GAP_STATUSES`. When no RenderProfile is passed (app path) and `route_state` is empty, existing `GAP_STATUSES` need_items behavior is unchanged. Name: navigator derives status from evidence and maturity and excludes owned-elsewhere or declared-unimplemented nodes from gap counts without overloading the app GAP_STATUSES. Touches: `src/startd8/navigator/models.py`, `src/startd8/wireframe_view/compose.py`, `src/startd8/wireframe/profile.py`. Lives: code src/startd8/wireframe_view/compose.py. Verify: ships_when-only + `route_state=declared_unimplemented` absent from `need_items`; app-plan need_items golden unchanged. Serves: O-2
- **FR-4 — Default confidence heuristic (SDK-owned).** When confidence is unset, `default_confidence(lives)` (★ SDK-owned — **not** a CC port) computes the wireframe-navigator rubric (0.9 code+test · 0.6 partial/doc-only · 0.4 pure spec). Name: the SDK owns a default_confidence rubric that scores a node from its lives evidence when confidence is unset. Touches: `src/startd8/navigator/models.py`, `tests/unit/navigator/test_models.py`. Lives: code src/startd8/navigator/models.py. Approve?: does 0.9 require BOTH code and test lives?. Verify: code+test lives and no authored confidence ⇒ 0.9 (±ε); helper exists only under `startd8.navigator`. Serves: O-2
- **FR-5 — Capability-index source.** Project `docs/capability-index/startd8.sdk.capabilities.yaml` (path override allowed) into Nodes with derived status and typed evidence/`wont`. Name: navigator projects the capability-index YAML into Nodes with derived status and typed evidence. Touches: `src/startd8/navigator/sources_capability.py`, navigator-build. Lives: code src/startd8/navigator/sources_capability.py. Gate: `startd8 navigator build --source capability-index --format json`. Verify: `startd8 navigator build --source capability-index --format json` exits 0 and JSON contains ≥1 live `capability_id` from the v1.27.0+ manifest. Serves: O-1
- **FR-6 — Minimal det-req requirements source.** Project a det-req/0.1 file into FR Nodes (does / Verify / Touches / optional evidence locators / derived fr_health). Default coupling = **vendor_thin** evidence/`fr_health` helper with SCHEMA/extract cite; optional `DET_REQ_KIT` subprocess; forbid required sibling-path import; do not fork the kit schema. Full V-1..V-5 plan-DAG parity is out of scope. Name: navigator projects a det-req file into FR Nodes with vendor-thin evidence and fr_health without forking the det-req kit. Touches: `src/startd8/navigator/sources_requirements.py`, `tests/unit/navigator/test_sources_and_cli.py`, navigator-build. Lives: code src/startd8/navigator/sources_requirements.py. Lives: code src/startd8/navigator/det_req.py. Approve?: is vendor_thin enough for CI without a sibling kit?. Verify: fixture REQ with a commit-anchored code locator builds exit 0 with typed evidence; done-claim without locator ≠ grounded; unit tests pass with det-req-kit **absent** from `sys.path`. Serves: O-1
- **FR-7 — Navigator CLI.** Typer group `startd8 navigator` exposes `build` with `--source`, `--format` (`html`|`json`), `--out`; must not register as `nav`. Name: the startd8 navigator Typer group exposes build without colliding with the nav app-registry command. Touches: navigator-build, exit-navigator, `src/startd8/cli.py`. Lives: code src/startd8/navigator/cli_navigator.py. Lives: code src/startd8/cli.py. Verify: one smoke test — `startd8 navigator --help` lists `build`/`ground`; `startd8 nav --help` still documents the app top-nav registry. Serves: O-1
- **FR-8 — App-path byte identity.** Classic app-scaffold wireframe with no Node fields / no profile remains byte-identical (HTML profile path **and** compose JSON omit-when-empty). Name: the classic app-scaffold wireframe stays byte-identical when no Node fields or profile are present. Touches: `src/startd8/wireframe/plan.py`, `tests/unit/wireframe/test_render_profile.py`, `tests/unit/wireframe/test_determinism_and_json.py`. Lives: test tests/unit/wireframe/test_render_profile.py. Approve?: is the classic app compose JSON keyset unchanged (no new keys)?. Verify: `test_no_profile_is_byte_identical` + canonical JSON determinism pass without golden edits; classic compose JSON keyset unchanged. Serves: O-2
- **FR-9 — Automated grounding pass.** `$0` command enumerates FR- / capability_id keys under a tree, counts code mentions, emits dated grounding JSON. Name: a zero-cost navigator ground command enumerates FR and capability keys under a tree and emits dated grounding JSON. Touches: navigator-ground, exit-navigator. Lives: code src/startd8/navigator/ground.py. Gate: `startd8 navigator ground --root src --out /tmp/ground.json`. Verify: `startd8 navigator ground --root src --out /tmp/ground.json` exits 0; JSON has keys, integer counts, ISO date. Serves: O-3
- **FR-10 — Projection in the SDK.** `nodes_to_wireframe_plan(nodes, *, project_root, group_by)` lives in the SDK (port of CC `navigator/render.py`); HTML render reuses `wireframe_view.render_to_file` + optional RenderProfile. Name: the SDK owns nodes_to_wireframe_plan projecting Nodes to a wireframe plan and rendering HTML without ContextCore. Touches: `src/startd8/navigator/project.py`, `src/startd8/wireframe_view/`. Lives: code src/startd8/navigator/project.py. Verify: unit test builds a plan from two Nodes without importing ContextCore; `html` format writes a file. Serves: O-1
- **FR-11 — Structure-only view.** A profiled navigator view offers a top-of-view **Structure only** switch that strips all descriptive prose (summary band, legend, item details, lives, name/handle, narration, approve/sign-off, status badges) and shows only the section groups + bare node keys — the underlying node structure with no other text. Gated to profiled consumers so the app-scaffold (no-profile) render stays byte-identical. Name: Navigator offers a Structure-only switch that strips prose to section groups and bare node keys. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does Structure only show only groups + node keys with no other text?. Verify: a profiled render carries a `structOnly` checkbox + `body.structure-only` CSS + per-item `lbl-key` spans; the no-profile render omits the checkbox and stays byte-identical (`test_no_profile_is_byte_identical`). Serves: O-1
- **FR-12 — Debugging-layer view-mode panel.** A profiled navigator renders a fixed **top-right** "View mode" panel exposing the mutually-exclusive modes **Content** (default) · **Structure only** · **Combined**. Combined shows the descriptive content AND the per-node structural metadata (`type · default · provenance · ← origin`) together — the debugging lens over the node. Gated to profiled consumers (app path byte-identical). Name: Navigator debugging layer offers a top-right view-mode panel with content, structure-only, and combined modes. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: is the view-mode panel top-right with mutually-exclusive structure-only and combined modes?. Verify: a profiled render carries `id="debug"` + `id="combined"` + `body.combined` CSS; Combined shows both `.det` and `.node-meta`; the no-profile render stays byte-identical. Serves: O-1
- **FR-13 — Live provenance readout.** The debugging panel shows a live **provenance** readout fed by an embedded chrome-provenance summary (`chrome_score` · present/total · orphan list): green when every chrome element traced to a source, ochre listing the **cruft** (orphans) otherwise — the stance *all content is cruft until proven otherwise* made visible in-view. Gated to profiled consumers (app path byte-identical). Name: Navigator debug panel shows a live provenance/cruft readout from the embedded chrome-provenance summary. Touches: `src/startd8/wireframe_view/view.py`, `src/startd8/navigator/project.py`, `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/navigator/project.py. Approve?: does the panel show chrome_score and list any cruft (orphans) live?. Verify: a profiled render embeds `payload.chrome` (score/present/total/orphans) and carries the `dbg-prov` readout; the no-profile render stays byte-identical. Serves: O-1
- **FR-14 — Multi-stage cruft purge toggle.** The debugging panel offers a **Hide app-scaffold chrome** toggle (orthogonal to the view modes) that non-destructively hides the app-scaffold bleed a fresh-eyes cruft audit flagged — the sign-off subsystem (per-section approve rows + sign-off bar), dead mockup drills, EU need-signals/todos, and the delivery-role kit optgroups — so a downstream consumer opts into the purge and elements can resurface later in a different light. Descriptive content (Approve? prompt text, NAME/HANDLE/lives) and the STATUS/SHAPE band + legend are retained. Gated to profiled consumers (app path byte-identical). Name: Navigator debug panel offers a non-destructive Hide-app-scaffold-chrome purge toggle. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does the purge toggle hide the sign-off subsystem and app-scaffold chrome while keeping descriptive content?. Verify: a profiled render carries `id="hideScaffold"` + `body.hide-scaffold .signoff` CSS; toggling adds `body.hide-scaffold` and hides `.signoff`/`#signbar`; the no-profile render stays byte-identical. Serves: O-1
- **FR-15 — Scaffold mode (template anatomy).** The debugging panel offers a **Scaffold mode** toggle that overlays each template REGION with its scaffold **role + data source** (`data-scaffold`) — masthead (profile chrome), reading-guidance (profile.why/do), status roll-up (status_counts + filter), shape (plan.shape), control layer (audience × fluency lenses), legend, section-lead, outline — so an adopter can read the template itself from a debugging standpoint as it becomes the de-facto multi-domain node visualizer (requirements first; legal · benchmark · dev-os next). Orthogonal overlay; gated to profiled consumers (app path byte-identical). Name: Navigator scaffold mode overlays each template region with its role and data source for adopters. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does Scaffold mode label every template region with its role + data source?. Verify: a profiled render carries `id="scaffold"` + `body.scaffold [data-scaffold]` CSS + ≥6 `data-scaffold` region roles + `data-layer` classifications + a `dbg-layers` legend (layer-aware colouring: control · descriptive · computed · node-driven); toggling adds `body.scaffold`; the no-profile render stays byte-identical. Serves: O-1
- **FR-16 — Scaffold-only (anatomy isolation) toggle.** Beneath the Scaffold-mode toggle, a **Scaffold only** checkbox that hides the node-driven CONTENT filling the node layer while keeping every template region's outline + anatomy label — so an adopter sees the empty template skeleton; checking it also enables Scaffold mode (labels/outlines come from it). Orthogonal overlay; gated to profiled consumers (app path byte-identical). Name: Navigator scaffold-only toggle hides node content and shows just the labelled template skeleton. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does Scaffold only keep every anatomy label + region outline while emptying the node-driven content?. Verify: a profiled render carries `id="scaffoldOnly"` + `body.scaffold-only` CSS that hides node-layer content; toggling it adds `body.scaffold-only` and `body.scaffold`; each `data-scaffold` region still shows its label; the no-profile render stays byte-identical (`test_no_profile_is_byte_identical`). Serves: O-1
- **FR-17 — Deterministic masthead identity.** The requirements masthead derives from THIS requirement, not static profile copy: eyebrow = the doc key (`REQ-0N`), headline = the H1 title (parsed from `# <Title> — Requirements`), sub-headline = the DIDL semantic name — via `requirements_profile_for(path)` (a per-doc `RenderProfile` that overrides the static `REQUIREMENTS_PROFILE`'s 'This spec' / 'A first look at this spec' copy); each field falls back to the base when unextractable, and the static base + the no-profile app path stay byte-identical. Name: Navigator derives the requirements masthead eyebrow, headline, and subtitle from the requirement's own key, H1 title, and DIDL semantic name instead of static profile copy. Touches: `src/startd8/navigator/sources_requirements.py`, `src/startd8/navigator/cli_navigator.py`, `tests/unit/navigator/test_sources_and_cli.py`. Lives: code src/startd8/navigator/sources_requirements.py. Approve?: does the masthead show the requirement's own key/title/semantic-name rather than the generic 'This spec'/'A first look at this spec'?. Verify: `requirements_profile_for(REQ-01)` yields eyebrow `REQ-01`, headline `SDK Node Home`, `summary_meta[0]` = the semantic name; a requirements render's masthead reflects them; `test_no_profile_is_byte_identical` stays green. Serves: O-1
- **FR-18 — Deterministic descriptive chrome.** `section_lead` and the page `<title>` derive from the requirement — section_lead = `What {key} defines`, title = `{key} — {H1 title}` (degrading key-only → H1/stem → base) — while `why`/`do` (renderer reading-guidance) and `gap_noun` (domain vocabulary) stay static base copy, extending `requirements_profile_for` with no change to the base profile. Name: Navigator derives the requirements section-lead and page title from the requirement's key and H1 title while keeping reading-guidance and vocabulary static. Touches: `src/startd8/navigator/sources_requirements.py`, `tests/unit/navigator/test_sources_and_cli.py`. Lives: code src/startd8/navigator/sources_requirements.py. Approve?: do section_lead and the page title name THIS requirement while why/do/gap_noun stay static guidance/vocabulary?. Verify: `requirements_profile_for(REQ-01).section_lead == "What REQ-01 defines"` and `.title == "REQ-01 — SDK Node Home"`; `test_no_profile_is_byte_identical` passes unedited. Serves: O-1
- **FR-19 — Logically-grouped control panel.** The profiled navigator's top-right panel groups its controls under three labelled headers by KIND — VIEW (mutually-exclusive content modes: Full / Structure only / Combined) · OVERLAYS (additive: Hide app-scaffold chrome) · TEMPLATE ANATOMY (debug: Scaffold mode → Scaffold only + the layer legend) — with the provenance readout last; reorganization + labelling only (every checkbox id and body-class toggle unchanged), profile-gated so the no-profile app path is byte-identical. Name: Navigator groups the top-right control panel under VIEW, OVERLAYS, and TEMPLATE ANATOMY headers by control kind without changing any toggle behaviour. Touches: `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does the panel read as three labelled control groups with every checkbox id and toggle unchanged?. Verify: the profiled render carries three `dbg-group` headers (VIEW/OVERLAYS/TEMPLATE ANATOMY); all five checkbox ids and toggles are unchanged; `test_no_profile_is_byte_identical` passes unedited. Serves: O-1
