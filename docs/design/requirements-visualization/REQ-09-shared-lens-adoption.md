...
- **FR-3 — apply_node_lenses gets a direct consumer.** Give `node_lenses.apply_node_lenses` at least one **direct** external call site so it is no longer dormant/thin-ice — preferred: route `compose`'s item-view assembly through `apply_node_lenses` as the single chokepoint IFF byte-identical (PREP-confirmed); else the tree/a11y bridge calls `apply_node_lenses` directly rather than only via `project_nodes`. Name: apply_node_lenses gains a direct external consumer so the shared aggregate is no longer a dormant single-internal-caller path. Touches: `src/startd8/wireframe_view/compose.py`, `src/startd8/navigator/render_tree.py`. Lives: code src/startd8/wireframe_view/node_lenses.py. Approve?: does apply_node_lenses have ≥1 direct consumer without any byte drift?. Verify: the Spec Delivery Loop `--reachability` probe reports `apply_node_lenses` as `wired` (real ≥ 1), not `export-only`/`DORMANT`. Serves: O-2
