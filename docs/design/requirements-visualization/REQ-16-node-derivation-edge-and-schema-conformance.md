...
- **FR-1 — Typed derivation edge.** Distinguish a Node's **derivation inputs** (the upstream keys it was derived/compiled from) from its containment `children`, via a typed relation (a documented kind on `child_keys` or a small `derived_from` projection), so `pipeline_provenance` reads the derivation chain from the typed edge instead of longest-prefix ownership. Name: The Node types a derivation edge distinct from containment so the compilation chain is read not reconstructed. Touches: `src/startd8/navigator/models.py`, `src/startd8/navigator/provenance.py`, `tests/unit/navigator/test_provenance.py`. Lives: code src/startd8/navigator/models.py. Approve?: is the derivation edge typed and distinct from containment children?. Verify: a node with a derivation input exposes it distinctly from its `children`, and `pipeline_provenance` for that node returns a chain sourced from the typed edge (not the longest-prefix heuristic). Serves: O-1
