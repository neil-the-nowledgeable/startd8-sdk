...
- **FR-6 — Base-change propagation.** A change to a shared token in the base reaches every non-overriding domain — and only those — with no edit to any domain delta, because `_deep_merge`/`resolve` merge per-leaf so an explicit override wins and silence inherits. Name: A change to the base definition propagates to every non-overriding domain without editing the domain deltas. Touches: `src/startd8/navigator/view_definition.py`, `tests/unit/navigator/test_view_definition.py`. Lives: code src/startd8/navigator/view_definition.py, test tests/unit/navigator/test_view_definition.py. Approve?: does a base change reach all non-overriding inheritors automatically, and leave overriding ones untouched?. Verify: four one-condition tests — C1 base `ink` reaches non-overriding requirements (`test_base_token_change_reaches_non_overriding_requirements_domain`); C2 same `ink` reaches capability, which overrides only `accent` (`test_base_token_change_reaches_non_overriding_capability_domain`); C3 a base `accent` change does NOT reach capability, which overrides it (`test_overriding_domain_is_insulated_from_a_base_change_to_that_key`); C4 the same `accent` change DOES reach requirements, which does not (`test_non_overriding_domain_follows_a_base_change_to_an_overridable_key`); plus the deltas-untouched invariant (`test_propagation_requires_no_edit_to_the_domain_deltas`). Serves: O-2
