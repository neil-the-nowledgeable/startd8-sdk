...
**Generated:** 2026-08-14 16:03:26 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/requirements-visualization/PLAN-01-sdk-node-home.md` | 97 lines · 863 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/requirements-visualization/REQ-01-sdk-node-home.md` | 162 lines · 1974 words |
...
| **Review focus (sponsor)** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/requirements-visualization/crp-focus-sdk-node-home.md` | 17 lines · 199 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/requirements-visualization/PLAN-01-sdk-node-home.md`  ·  **Size:** 97 lines · 863 words

```markdown
# PLAN-01 — SDK Node Home — Implementation Plan

**Pairs with:** `REQ-01-sdk-node-home.md` · **Version:** 0.3.1 · **Date:** 2026-08-14  
**Status:** Ready for CRP (optional) before implementation

## Discoveries (locked into REQ §0)

| Assumption (REQ v0.1) | Planning discovery | Impact on plan |
|---|---|---|
| Invent Node | CC `navigator/models.py` is the lived model | F-1 = **port** field-compatible copy into `startd8.navigator` |
...
| Greenfield requirements source | extract.py parses Lives; CC source is huge | F-3 = **minimal** FR→Node; wrap extract; NR-8 full V-1..V-5 |
| Projection stays in CC | Adapter already works in CC render.py | F-2/F-4 = move `nodes_to_wireframe_plan` into SDK (FR-10) |
...
## Design (post-reflection)

| FR | File · symbol (to-be-created marked ★) | Change |
|----|----------------------------------------|--------|
| FR-1, FR-4 | ★ `src/startd8/navigator/__init__.py`, ★ `models.py` | Port Node / NodeEvidence / derive_status / default_confidence from CC; cite NODE-SCHEMA |
| FR-10 | ★ `src/startd8/navigator/project.py` | Port `nodes_to_wireframe_plan`; call `wireframe_view.render_to_file` |
| FR-2, FR-3, FR-8 | `wireframe/plan.py` WireframeItem · `wireframe_view/compose.py` · `profile.py` | Optional grounding fields; need_items skips honest-skip route_state; determinism green |
| FR-5 | ★ `sources_capability.py` | Load `startd8.sdk.capabilities.yaml` → Nodes |
| FR-6 | ★ `sources_requirements.py` | Minimal det-req → Nodes via extract lives/FR split (no kit schema fork) |
| FR-7 | ★ `cli_navigator.py` + `cli.py` add_typer | `navigator build` |
| FR-9 | ★ `ground.py` + CLI `ground` | `$0` key→count JSON |

**Dependency direction:** startd8 must **not** import ContextCore. CC may later switch to SDK Node.
...
| id | FRs | target | state |
|----|-----|--------|-------|
| F-1 | FR-1, FR-4 | ★ `navigator/models.py` + unit tests | pending |
| F-2 | FR-2, FR-3, FR-8, FR-10 | WireframeItem + compose + ★ `project.py` + determinism green | pending |
| F-3 | FR-5, FR-6 | ★ sources (capability + minimal requirements) | pending |
| F-4 | FR-7 | ★ `cli_navigator.py` wired in `cli.py` | pending |
| F-5 | FR-9 | ★ `ground.py` + CLI | pending |

Dependencies: F-1 → F-2 → F-3 → F-4; F-5 after F-1 (parallel OK with F-3). Acyclic.

## Verify (whole change)

- `pytest tests/unit/wireframe/ tests/unit/navigator/` green.
...
- `startd8 nav --help` still app top-nav; `startd8 navigator --help` lists build/ground.
- `test_no_profile_is_byte_identical` + determinism tests unmodified and green.
...
*v0.3.1 — Aligned with REQ v0.3.1. Ready for CRP before implementation.*
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/requirements-visualization/REQ-01-sdk-node-home.md`  ·  **Size:** 162 lines · 1974 words
...
- **OQ-1 (where does Node live?) → SDK `src/startd8/navigator/`.** Field-compatible with CC so a later CC import can replace its twin; Phase 1 does **not** rewrite ContextCore.
...
| quality | Re-implementing det-req parsing (drift from kit) | Reuse `det-req-kit/extract.py` lives/FR split (import or subprocess); don't fork SCHEMA | high |
...
- **FR-1 — Node model in the SDK (to-be-created).** The SDK exposes a frozen Node + NodeEvidence + derive_status surface field-compatible with ContextCore's `navigator/models.py` and NODE-SCHEMA (`key`, `does`, `wont`, `lives[]`, `ships_when`, `confidence`, `triggers`, `children`, `category`, `orientation`, `route_state`; optional `status_facets` / `attributes` for compatibility). Touches: `src/startd8/navigator/models.py`. Verify: constructing a Node with typed lives and `derive_status(has_code_evidence=True, maturity="beta")` returns thin-or-built per NODE-SCHEMA; field names match the CC model enough that a dict round-trip of the shared fields succeeds. Serves: O-2
- **FR-2 — Typed grounding survives compose.** WireframeItem gains optional `key`, `lives` (typed tuples), `confidence`, `ships_when`, `was`, `route_state` (defaults empty/None); `compose` emits them on each item. Existing `paths` remains populated from lives refs for back-compat. Touches: `src/startd8/wireframe/plan.py`, `src/startd8/wireframe_view/compose.py`. Verify: given a Node with `lives=[{type:code, ref:git:<40-hex>:path}]` and `confidence=0.9`, after `nodes_to_wireframe_plan` → `compose`, item JSON includes type+ref and confidence 0.9 (not paths-only). Serves: O-2
- **FR-3 — Derived status + honest-skip gap rule.** Status is derived from evidence × maturity; nodes with `route_state` in `{owned_elsewhere, declared_unimplemented}` are excluded from attention/gap counts (profile `is_gap=False` or equivalent need_items filter) — do **not** overload app `GAP_STATUSES`. Touches: `src/startd8/navigator/models.py`, `src/startd8/wireframe_view/compose.py`, `src/startd8/wireframe/profile.py`. Verify: a ships_when-only node with `route_state=declared_unimplemented` does not appear in `need_items`; a code+test node derives built or thin and may count as non-gap. Serves: O-2
- **FR-4 — Default confidence heuristic.** When confidence is unset, compute the wireframe-navigator rubric (0.9 code+test · 0.6 partial/doc-only · 0.4 pure spec). Touches: `src/startd8/navigator/models.py`. Verify: code+test lives and no authored confidence ⇒ derived confidence == 0.9 (±ε). Serves: O-2
- **FR-5 — Capability-index source.** Project `docs/capability-index/startd8.sdk.capabilities.yaml` (path override allowed) into Nodes with derived status and typed evidence/`wont`. Touches: `src/startd8/navigator/sources_capability.py`, navigator-build. Verify: `startd8 navigator build --source capability-index --format json` exits 0 and JSON contains ≥1 live `capability_id` from the v1.27.0+ manifest. Serves: O-1
- **FR-6 — Minimal det-req requirements source.** Project a det-req/0.1 file into FR Nodes (does / Verify / Touches / optional Lives / derived fr_health); reuse det-req-kit extract parsing for Lives — do not fork the kit schema. Full V-1..V-5 plan-DAG parity is out of scope. Touches: `src/startd8/navigator/sources_requirements.py`, navigator-build. Verify: fixture REQ with one FR and `Lives: code git:<40-hex>:path` builds exit 0; FR node carries that type+ref; done-claim without lives is not treated as grounded. Serves: O-1
- **FR-7 — Navigator CLI.** Typer group `startd8 navigator` exposes `build` with `--source`, `--format` (`html`|`json`), `--out`; must not register as `nav`. Touches: navigator-build, exit-navigator, `src/startd8/cli.py`. Verify: `startd8 navigator --help` lists `build`; `startd8 nav --help` still documents the app top-nav registry. Serves: O-1
- **FR-8 — App-path byte identity.** Classic app-scaffold wireframe with no Node fields / no profile remains byte-identical. Touches: `src/startd8/wireframe/plan.py`, `tests/unit/wireframe/test_render_profile.py`, `tests/unit/wireframe/test_determinism_and_json.py`. Verify: `test_no_profile_is_byte_identical` and canonical JSON determinism tests pass without golden edits. Serves: O-2
- **FR-9 — Automated grounding pass.** `$0` command enumerates FR- / capability_id keys under a tree, counts code mentions, emits dated grounding JSON. Touches: navigator-ground, exit-navigator. Verify: `startd8 navigator ground --root src --out /tmp/ground.json` exits 0; JSON has keys, integer counts, ISO date. Serves: O-3
- **FR-10 — Projection in the SDK (to-be-created).** `nodes_to_wireframe_plan(nodes, *, project_root, group_by)` lives in the SDK (port of CC `navigator/render.py`); HTML render reuses `wireframe_view.render_to_file` + optional RenderProfile. Touches: `src/startd8/navigator/project.py`, `src/startd8/wireframe_view/`. Verify: unit test builds a plan from two Nodes without importing ContextCore; `html` format writes a file. Serves: O-1
...
*v0.3.1 — Post planning reflection + lessons + design-principle hardening. Ready for CRP review.*
