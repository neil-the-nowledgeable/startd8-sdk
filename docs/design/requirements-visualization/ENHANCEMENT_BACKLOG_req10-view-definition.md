...
| **EC-1** | **`navigator view-definition` CLI** — `--name X`/registry dump of a resolved (or `--raw`) definition as JSON via the existing `to_dict`; the cross-repo `VIEW-SCHEMA` export seam (architecture §7 step 7's first brick) | **S** | wire-existing | ✅ | CROSS(A1+A2+C1) — **→ landed `0140cdee` (auto-adopted, HTH stage-7)** |
| **EC-2** | **Fold `NODE_SCHEMA_PROFILE` into a real 3rd definition** (`NODE_SCHEMA_DEFINITION extends base`) — de-dups the literal, widens the proof to 3 real domains, now inherits the REQ-11 theme | **S** | wire-existing | ✅ | CROSS(A3+C3) — **→ landed `355e2b58` (auto-adopted, HTH stage-7)** |
| **EC-4** | **`definition_diff(domain, base)` helper** + `view-definition --diff` — the leaves a domain overrides/adds vs base; feeds governance + the HOWTO | **S** | wire-existing | ✅ | B4 — **→ landed `0ec39ac8` (auto-adopted, HTH stage-7)** |
| **EC-6** | **`validate_definitions` + `view-definition --validate`** — every registry entry resolves + chrome.bindings reference known context fields; standalone (govern.py governs docs, not code — integration deferred) | **M** | wire-existing | ✅ | B5 — **→ landed `8563ff88` (auto-adopted)** |
| **EC-7** | **HOWTO: author a new domain definition** — base → thin delta → register → project → inspect/validate, grounded in the 3 examples + CLI | **M** | docs | ✅ | B6 — **→ landed `8563ff88` (auto-adopted)** |
