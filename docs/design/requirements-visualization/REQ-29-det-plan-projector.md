...
- **FR-7 — Pilot across the three-cell matrix, golden-first (reflective-adoption fold-back).** Pilot the projector across the iteration-1 matrix and fold friction back into `det-plan/0.1`: (a) GOLDEN-PARITY — project REQ-08 and REQ-01 (which have hand-authored PLANs) and diff the projection against the human plan, the quality bar whose delta is the primary friction source; (b) NEGATIVE-GATE — a solo-by-design REQ (REQ-03) projects nothing; (c) DEMAND-CLEARING — small companionless REQs (REQ-16/17) project a conformant plan; cross-repo pilots (REQ-seat) are deferred (correct-absence). Name: The projector is piloted golden-first across parity negative and demand cells with the golden delta folded back into the grammar. Touches: `tests/unit/plan_codegen/test_projector_pilot.py`, `docs/design/requirements-visualization/SCHEMA_det-plan-0.1.md`. Lives: test tests/unit/plan_codegen/test_projector_pilot.py. Approve?: is the projector piloted golden-first with the parity delta driving the fold-back?. Verify: the projected plan for REQ-08 and REQ-01 diffs against the hand-authored PLAN with the delta recorded as `det-plan/0.1` friction; REQ-03 projects nothing; REQ-16/17 each project a conformant `det-plan/0.1`. Serves: O-3
