...
- **FR-8 — Debug group: raw data + node data views below the sign-off.** The control panel gains a definition-owned **Debug** group with a **Raw data** toggle (reveals the full payload/variant being visualized) and a **Node data** toggle (reveals just the node items being visualized), each rendering a raw-JSON block into a `#rawdata` panel placed **below the export sign-off**; both default off and are additive (no view change), and the panel/JSON is profiled-navigator-only so the app path stays byte-identical. Name: A Debug control group exposes Raw data and Node data toggles that render the raw payload and node items into a panel below the export sign-off. Touches: `src/startd8/navigator/view_definition.py`, `src/startd8/wireframe_view/_template.py`, `tests/unit/wireframe/test_render_profile.py`, `tests/unit/navigator/test_view_definition.py`. Lives: code src/startd8/wireframe_view/_template.py. Approve?: does a Debug group expose the raw data + nodes being visualized in a panel below the sign-off, additive + app-path-safe?. Verify: `resolve(BASE_NAVIG8R_DEFINITION, DEFINITION_REGISTRY).control["groups"]` includes a `debug` group whose toggles are `{rawData, nodeData}`; a profiled render has a `#rawdata` panel positioned after `#signbar`, hidden by default; enabling Raw data fills it with the current variant's JSON and Node data with the node items' JSON; `test_no_profile_is_byte_identical` passes unedited. Serves: O-7
