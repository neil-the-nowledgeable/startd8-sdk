...
- G6. **Both polarities of the silent-mismeasurement Shape unrepresentable (A16/FR-25/FR-26).** Not only the drop-to-0 (a real value read as a false `0`) but ALSO the overshoot-to-1.0 (a non-measured proxy read as a real high value — artifact-presence substituted for strict coverage, and a survivorship average over emitters). A `FieldState.value` can be neither a silent zero nor a silent one.
...
- **FR-23 (SDK emits the grade-ready denominator — LOCKED, A13/OQ-5; population-clarified A16).** The `aggregate` block emits the grade-ready denominator with `excluded` already dropped, alongside the per-state counts (`computed`/`not_computed`/`unbound`/`excluded`) and per-field `state`s for audit. This is the single authority the consumer reads (FR-14/FR-15); the consumer never recomputes it. Rendered by the same serializer path as the rollups (FR-9/FR-21) so it cannot drift from the per-field `FieldState`s it summarizes. **"Grade-ready" now explicitly means (A16): over the FULL service population (FR-25) — with non-emitters carried as counted `not_computed`/`excluded` states, NOT dropped — using STRICT coverage values (FR-26), NOT orientation/artifact-presence proxies.** So the emitted denominator can be neither survivorship-narrowed (the overshoot's population cause) nor presence-inflated (its definition cause); the same single-authority guard that prevents the drop-to-0 re-derivation now also prevents the overshoot-to-1.0.
...
### 7b. Overshoot-to-1.0 twin — population axis + strict-value rule (FR-25, FR-26)

> **The overshoot twin (A16/SA-GD1-ROLLUP-OVERSHOOT-POPULATION-DEFINITION).** Everything above closes the
> *drop-to-0* class (a real value silently arriving as `0`/absent). A GD-1 rollup against a light committed
> `observability-quality.json` surfaced the **opposite polarity of the SAME silent-mismeasurement Shape**:
> `avg_metric_coverage_score = 1.0` where the live value is `0.086` — a false-GREEN **overshoot**. It has
> TWO independent causes, and FR-25/FR-26 make each unrepresentable. **Explicit goal:** BOTH the drop-to-0
> (FR-1..FR-24, already addressed) AND this overshoot-to-1.0 become UNREPRESENTABLE — a `FieldState.value`
> can be neither a silent zero nor a silent one.

- **FR-25 (POPULATION axis — full-population denominator; non-emitters carried as explicit states, never dropped — LOCKED, A16).** The aggregate denominator is the **FULL service population** (the catalog / GroundTruth service inventory), NOT the survivor set that happened to emit. The overshoot's POPULATION cause was survivorship (Wald): the rollup averaged over `services_scored = 3` (the three emitters, all `1.0`) and silently dropped the ~11 non-emitters that score `0` — averaging the survivors. RULE: **every service in the full population is carried as an explicit `FieldState`**; a non-emitter renders `not_computed` (no data on this service) or `excluded` (#286, deliberately not produced) — it is **COUNTED in the population census, never absent-and-dropped from the denominator.** This is the FR-14 grader null-rule ("average the measured, count the not-measured") applied to the **SERVICE/population axis** rather than the field axis. The three states the census must NOT collapse: **computed-0** (an emitter with a genuine `0.0` coverage) ≠ **not_computed** (a non-emitter, no data) ≠ **excluded** (#286, deliberately out). Emitting `avg` over only the emitters — treating a non-emitter as if it were not in the population — is a producer bug (FR-19-class; the aggregate serializer refuses a denominator that is silently narrower than the fed population).
  - **Cross-repo scope (JOINT contract — the SDK can only average what it is FED).** The SDK aggregate averages the services the SDK is *fed*; the authority for the FULL population is the **catalog / GroundTruth / the loop's Phase-0 Live App-Landscape Catalog** (§12 Phase 2). So the population axis is a **JOINT** contract, split honestly like the SDK-owned-vs-routed split elsewhere in this REQ:
    - **SDK-owned (this REQ):** carry every fed service as an explicit `computed`/`not_computed`/`excluded` `FieldState`; compute the grade-ready denominator over the fed population with non-emitters COUNTED (FR-23 extended to the population axis); and — when the fed population is known to be narrower than the catalog — mark the aggregate's own completeness (e.g. an aggregate-level `population_state` / `not_computed` census entry) rather than silently averaging survivors.
    - **ROUTED to the loop / FDE:** *supplying the full-population denominator* — the authoritative catalog/GroundTruth service inventory the SDK averages against — is loop-owned. It must be **fed to `generate`** (the catalog population) or **carried in `coverage-report.json`**, so the SDK's denominator is the full population, not just the emitters it happened to receive. The SDK owns strict value + explicit state; the loop owns the full-population supply. (Same repo-boundary discipline as FR-24 / invariant #9 — the SDK does not hand-roll the catalog; it consumes or routes it.)
- **FR-26 (STRICT-VALUE rule — a coverage `FieldState.value` is the strict-engine number, NEVER orientation/artifact presence — LOCKED, A16).** A coverage `FieldState.value` (when `state == "computed"`) is EXACTLY the strict engine's coverage number — `compute_metric_coverage = |expected ∩ referenced| / |expected|` — and **NEVER** a substitute for it. The overshoot's DEFINITION cause was exactly such a substitution: the rollup used **orientation/artifact PRESENCE** (score `= 1.0` when `locus_grounded`, i.e. "an orientation artifact was generated") in place of the strict coverage value, so a service with a generated-but-uncovering artifact scored `1.0` instead of its true `0.086`. **This is the super-report §2.2 "map-green ≠ binder-bind" inflation** at the scoring layer: the affordance map stamps a family `source_backed`/`covered` (name-shaped / artifact-present) while the binder declines to bind it — "the map's green is not the binder's bind" (super-report §2.2). RULE: **presence-of-an-artifact is a `state` fact (it justifies `computed` vs `not_computed`), NEVER a `value` fact.** A `computed` `FieldState.value` MUST be the strict `compute_metric_coverage` output; the serializer (FR-19) MUST NOT accept a coverage value sourced from `locus_grounded`/orientation-presence or any artifact-existence proxy. Presence answers "did we run the computation?" (the `state`); the strict engine answers "what did it cover?" (the `value`) — and the two are never conflated. This is the field-level twin of FR-25's population-level census: no green (`1.0`) may stand in for a value the strict engine did not produce.
...
4. **The scorer-integrity lane cures the false-zero AND its overshoot twin (FR-25/FR-26).** The GD-1 rollup
   overshoot (`avg_metric_coverage_score = 1.0` vs live `0.086`; `SA-GD1-ROLLUP-OVERSHOOT-POPULATION-DEFINITION`)
   is this same orthogonal scorer-integrity class at the opposite polarity, and it lands squarely on the
   super-report's §2.2 mechanism: its DEFINITION cause is the **"map's green is not the binder's bind"**
   inflation (orientation/artifact PRESENCE substituted for the strict `compute_metric_coverage` value —
   §2.2), and its POPULATION cause is a survivorship average over the emitters (Wald). FieldState's strict-
   value rule (FR-26) and full-population census (FR-25) are the direct scoring-artifact cures for both,
   confirming this lane is orthogonal to live/static (a purely-static light artifact produced the `1.0`) —
   exactly the GD-1 "pure scorer bug, unrelated to live/static" the audit marks `✗` for the live catalog.
...
### 13.4 The overshoot twin — same Shape, opposite polarity (FR-25/FR-26, A16)

§13.1 named the recurring failure as **one Shape wearing many masks: a `0`/absent that means two different
things, read as the wrong one.** The `SA-GD1-ROLLUP-OVERSHOOT-POPULATION-DEFINITION` finding is that SAME
Shape at the **opposite polarity** — not a real value silently arriving as a false `0`, but a *false* value
silently arriving as a `1.0` (`avg_metric_coverage_score = 1.0` where live is `0.086`). Both are silent
mismeasurement; both are cured by the same self-describing-state move:

- **drop-to-0** (FR-1..FR-24) — a MEASURED value (real `0`, or a value that never computed) is read as a
  false zero. Cure: `{value, state, reason}` distinguishes computed-`0` from `not_computed`-`null`.
- **overshoot-to-1.0** (FR-25/FR-26) — a NON-measured proxy (artifact presence, or the surviving emitters)
  is read as a real high value. Cure, the exact twin: (a) FR-26 — a `computed` `value` is the STRICT engine
  number, never an artifact-presence proxy (this is §2.2's "map-green ≠ binder-bind" — a present artifact is
  a `state` fact, not a `value`); (b) FR-25 — the aggregate averages the FULL population with non-emitters
  COUNTED as `not_computed`/`excluded`, never survivor-averaged.

So invariant #2 ("no green trusted until re-measured against the derived artifact") is enforced **in both
directions**: a green can no longer be a silent zero (drop) OR a silent one (overshoot). The overshoot's
population cause is precisely a **survivorship** read (average the survivors — the Wald move the pilot
process's survivorship gate exists to catch), and its definition cause is precisely the §2.2 coverage
inflation — both were already named failure classes in the super-report; FR-25/FR-26 make them
unrepresentable at the schema level rather than caught by manual audit.
...
### Overshoot-twin refinement (A16 — folded into FR-25/FR-26; FR-14/15/23 reconciled)

- **A16 (bus finding `SA-GD1-ROLLUP-OVERSHOOT-POPULATION-DEFINITION` — the overshoot twin of the drop-to-0).** A GD-1 rollup computed against a **light committed** `observability-quality.json` returned `avg_metric_coverage_score = 1.0` while the live value is `0.086` — a false-GREEN **overshoot**, the OPPOSITE POLARITY of the drop-to-0 the REQ already cures, and the SAME silent-mismeasurement Shape (§13). **The loop correctly AVOIDED shipping the `1.0` — this was a *caught* false-green, not a shipped one; A16 makes the catch structural rather than lucky.** The overshoot had TWO INDEPENDENT causes, each now made unrepresentable:
  1. **DEFINITION cause (the `1.0` itself).** The rollup substituted **orientation/artifact PRESENCE** (score `= 1.0` when `locus_grounded`, i.e. "an orientation artifact was generated") for the **STRICT coverage value** (`compute_metric_coverage = |expected ∩ referenced| / |expected|`). Substituting presence for coverage IS the super-report §2.2 **"map-green ≠ binder-bind"** inflation (the affordance map stamps a family `source_backed`/`covered` while the binder declines to bind it — "the map's green is not the binder's bind") surfacing at the scoring layer. → **FR-26:** a coverage `FieldState.value` is the strict-engine number, NEVER an artifact/orientation-presence proxy; presence is a `state` fact, never a `value` fact.
  2. **POPULATION cause (the survivorship overshoot).** The aggregate averaged over `services_scored = 3` (the emitters, all `1.0`) and silently dropped the ~11 non-emitters that score `0` — **averaging the survivors (Wald).** → **FR-25:** the aggregate denominator is the FULL catalog/GroundTruth service population; a non-emitter is COUNTED as an explicit `not_computed`/`excluded` `FieldState`, never absent-and-dropped. This is the FR-14 grader null-rule applied to the SERVICE/population axis; the three states the census must not collapse are **computed-0** (emitter, genuine `0`) ≠ **not_computed** (non-emitter, no data) ≠ **excluded** (#286).
  - **Cross-repo scope (JOINT):** the SDK can only average what it is FED, so the population axis splits like the rest of this REQ — **SDK-owned:** strict value + explicit per-service state + the full-*fed*-population denominator (FR-23 extended); **ROUTED to the loop/FDE:** *supplying* the authoritative full-population denominator (catalog/GroundTruth) — fed to `generate` or carried in `coverage-report.json`. → new §7b (FR-25/FR-26); FR-23 "grade-ready" clarified to mean full-population + strict-value; FR-15 population caveat added; §13.4 ties both polarities to the one Shape. *Rationale:* the drop-to-0 and the overshoot-to-1.0 are one Shape at opposite polarity — a `0`/absent read as a value, and a presence/survivor read as a value; making a `FieldState.value` unable to be either a silent zero OR a silent one closes the Shape in both directions with the same self-describing-state move (S6 generalized). *Reconciliation:* FR-23's "grade-ready denominator" (previously silent about population/value-source) now explicitly means over the FULL population with non-emitters counted, using strict values — no contradiction with FR-14/FR-15, which already forbid consumer re-derivation; FR-25's full-population supply is the *loop's* obligation, distinct from the consumer's "no re-derive," so FR-15 is tightened (population caveat), not overturned.
