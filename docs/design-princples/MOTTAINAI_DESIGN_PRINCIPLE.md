...
| 2026-07-09 | Formalized the **discoverability precondition** as [Genchi Genbutsu](./GENCHI_GENBUTSU_DESIGN_PRINCIPLE.md): Mottainai forwards what already exists; Genchi Genbutsu ensures the authoritative version is *findable* (by binding, boundary, authored-intent, and name) so it *can* be forwarded rather than re-derived. **Gap-40 (code-symbol variant):** the Inventory Problem applies to code, not just pipeline artifacts — three independent observability generators (`generate_prometheus_rule` ×3, `manifest.py` ×7) were re-created because generic naming made the existing one un-discoverable (ContextCore `docs/adr/003`). |
