...
**Generated:** 2026-06-19 01:40:11 UTC
...
| Role | Path | Size |
|------|------|------|
| **Plan** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/otel-demo-corpus/TIER0_REFERENCE_ENV_PLAN.md` | 206 lines · 1489 words |
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/otel-demo-corpus/TIER0_REFERENCE_ENV_REQUIREMENTS.md` | 157 lines · 1910 words |
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/otel-demo-corpus/TIER0_REFERENCE_ENV_PLAN.md`  ·  **Size:** 206 lines · 1489 words
...
### S4 — Optional fan-out  → FR-4 (QW-3)
- `fanout_patch.md` documents adding ONE exporter to `otel-config.yml`:

```yaml
# additive only — preserves existing pipelines; Apache-2.0 header retained
exporters:
  otlp/startd8:
    endpoint: host.docker.internal:4317   # StartD8 Alloy
    tls:
      insecure: true
service:
  pipelines:
    traces:  { exporters: [..., otlp/startd8] }
    metrics: { exporters: [..., otlp/startd8] }
```
- Off by default; documented as opt-in for a unified StartD8 Grafana view. No service touched.
  ...
  "sections": [
    {
      ...
      "signal": "traces",
      "backend": "jaeger",
      "query": "span.kind in (PRODUCER,CONSUMER) and messaging.system=kafka",
...
```
- `observed_names` carries live metric/attribute names → ContextCore `_PROTOCOL_METRICS` downstream consumer (QW-4).
- Capture `telemetry-schema/` reference into the attestation (QW-5).

### S6 — Generic verifier + adapters  → FR-6 (QW-6), OQ-5
- `verify_coverage.py` reads the attestation, dispatches each query to `adapters/{jaeger,prometheus,tempo}.py`, prints a per-section report, exits `0/1/2` (mirrors `verify_otel_trace.py` exit convention without reusing its Artisan checks — NR-6).
- Adapters isolate query-API differences (probes OQ-5: version-gate if Jaeger/Prometheus APIs differ across pinned versions).
...
### S8 — Teardown + docs  → FR-9
- `teardown.sh`: `docker compose ... down -v`.
- `reference-env.md`: tiers, ports, footprint table, teardown, fail-soft guidance.

### S9 — Validation
- Bring up `observe` tier; run `attest_coverage.py`; assert the §4 acceptance table sections are `evidenced` (≥5 languages, gRPC, Kafka, db.system, flagd, span-metrics connector).
- Bring up `profile` tier once; assert Profiles section `evidenced`.
- Run `verify_coverage.py` → exit 0.
...
| 3 | S5 attestation + S6 verifier + adapters | ~1 day | acceptance |
| 4 | S4 fan-out (optional) | ~1 hr | — |
| 5 | S8 teardown/docs + S9 validation | ~½ hr | sign-off |

Critical path ≈ **2 days**, almost all glue/observation code — no service code, matching the Tier-0 "config-not-build" thesis.
...
| Jaeger/Prometheus query API drift across versions | S6 adapter isolation; pin v2.2.0 (FR-8); OQ-5 probe |
| Demo upstream changes service layout | Pinned tag + recorded digests (FR-8); re-capture on bump |
| Attestation rots vs catalog | QW-1 reusable contract; OQ-6 decides CI vs manual |

---

## 7. Phase 5 — Convergent Review offer

The requirements (v0.2) and this plan are aligned and traceable. Recommended next step before any implementation: run a **Convergent Review (`/new-cnvrg-rvw-prmpt`)** on both documents (dual-document mode). The reflective loop corrected wrong *assumptions* (internal perspective); CRP would catch missing *concerns* — e.g. security of the optional fan-out exporter, attestation schema versioning, and whether OQ-5/OQ-6 need resolving before S5/S6. That review would likely produce a v0.3.
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/otel-demo-corpus/TIER0_REFERENCE_ENV_REQUIREMENTS.md`  ·  **Size:** 157 lines · 1910 words
...
Tier 0 is "done" when the attestation (FR-5) evidences, on a single brought-up environment, at minimum:

| Landscape § | Evidenced by | Backend |
| --- | --- | --- |
| §2 Traces | non-empty service trace list | Jaeger |
| §2 Metrics | a `*_duration` / RPC metric series present | Prometheus |
| §2 Profiles | a profile series present (T0-profile) | Pyroscope |
| §3.1 Languages | ≥5 distinct `telemetry.sdk.language` resource values across traces | Jaeger |
| §4.1 OTLP gRPC + HTTP | both receivers active in collector config | collector |
| §5.3 gRPC | spans with `rpc.system=grpc` | Jaeger |
| §5.4 Messaging | a Kafka PRODUCER/CONSUMER span | Jaeger |
| §5.5 Database | a `db.system` span (Postgres and/or Valkey) | Jaeger |
| §5.6 Feature flags | a flagd evaluation span/event | Jaeger |
| §7.1 Connector | span-metrics-derived metric present | Prometheus |
...
- **OQ-5** — Backend query stability: do Jaeger/Prometheus query APIs differ enough across the demo's pinned versions to need version-gated adapters? (Plan S6 probes this.)
- **OQ-6** — Should the attestation run as CI (nightly bring-up) or remain a local/manual artifact? (Cost vs drift-detection trade-off; default local for v1.)
