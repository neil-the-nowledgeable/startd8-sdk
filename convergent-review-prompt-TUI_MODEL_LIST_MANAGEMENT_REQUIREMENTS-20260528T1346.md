...
**Generated:** 2026-05-28 17:46:04 UTC
...
>
> - This prompt asks the reviewing **agent** to **persist suggestions directly into the source documents** by appending a `## Review Suggestions` section to the end of each file. The chat reply is a short write-confirmation only — **no** in-chat numbered list.
> - **Triage** (ACCEPT / REJECT per suggestion, applying suggestions into the prose, removing the appendix after merge) is **your** follow-up work — optionally using `CONVERGENT_REVIEW_AGENT_GUIDE.md` and the parameters below.
...
| Role | Path | Size |
|------|------|------|
| **Requirements** | `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/TUI_MODEL_LIST_MANAGEMENT_REQUIREMENTS.md` | 121 lines · 1286 words |
...
You are a **senior architectural reviewer** with **file-edit tools** (Write/Edit/equivalent) and filesystem access to the source documents listed above. Your job is to produce **improvement suggestions** (structured, anchored, actionable) and **persist them directly into the source documents** by **appending** a `## Review Suggestions — R1 — <your-model-id> — <UTC date>` section to the end of each file you were asked to review.

**Do not** triage your own suggestions (no ACCEPT/REJECT self-review), **do not** modify or rewrite any existing prose above your appended appendix, **do not** simulate multi-round protocol state machines, and **do not** emit a numbered suggestion list in chat — the orchestrator reads them from the files. **Do not** initialize or fill CRP guide Appendix A/B/C — those are orchestrator-side after triage.
...
Use **F-prefix** suggestion IDs in the form **R1-F{n}** (e.g. R1-F1, R1-F2).
...
**Self-filter (do not label as triage):** Omit vague praise (“looks good”), duplicate issues, and purely stylistic nits unless they block comprehension. If something **contradicts stated project constraints**, frame it as a **scope trade-off** suggestion rather than as a mandate.

**Deliverables (mandatory — persist to source files, not chat):**

1. **Append a \`## Review Suggestions — R1 — <your-model-id> — <UTC date>\` section** at the **end** of each source document you reviewed (plan suggestions → plan file; requirements suggestions → requirements file). Use Write/Edit; do **not** modify any existing content above your appendix.
2. **Inside each appendix**, include:
   - **Executive summary** — At most **10 bullets**: top risks, top opportunities, blocking gaps (no triage tables).
   - **Numbered suggestions** — Full list with **R1-** prefixes (see ID rules below). Optional “first pass” / “second pass” subsections for organization — **no** ACCEPT/REJECT columns.
3. **(Dual mode only)** Append a \`## Requirements Coverage Matrix — R1\` section at the **end of the plan file** (after your Review Suggestions appendix) mapping each major requirement ID/section → plan section/task → **Covered / Partial / Gap**. Analysis only; not a substitute for orchestrator triage.
4. **Chat reply** — A **short write-confirmation** (1–3 lines) listing the file paths you wrote to and the suggestion counts (e.g. \`Wrote 8 suggestions to plan.md, 4 to requirements.md\`). **Do not** repeat the suggestion content in chat.

### Optional second-pass suggestions (inside the appended appendix, still no triage)

If you still have budget under the max-suggestions cap after your first list, you may add a `### Stress-test / adversarial pass` subheading **inside the same appendix you appended to each file**, with **additional** numbered suggestions (continue **R1-S\*** / **R1-F\*** numbering — do not simulate R2 triage). Try to break your own prior conclusions where it genuinely helps; skip if redundant. **Still no in-chat list** — keep the chat reply to the short write-confirmation.
...
- Use file-edit tools (Write/Edit/equivalent) to **append** a `## Review Suggestions — R1 — <model-id> — <UTC date>` section to the **end** of each source document you reviewed.
- In dual mode, also append a `## Requirements Coverage Matrix — R1` section to the end of the plan file (after the Review Suggestions appendix).

**You MUST NOT:**

- Triage suggestions (no ACCEPT/REJECT disposition tables or columns for your own or others' suggestions).
- Modify, rewrite, reorder, or delete any existing content **above** your appended appendix in the source files.
- Execute **Phase 5–7** (or dual-document DD triage/merge phases) from the guide.
- Initialize, append to, or modify CRP guide **Appendix A / B / C** (those are orchestrator-side after triage).
- Output a **rewritten** full plan or requirements body, in chat or in the files.
...
<details>
<summary><strong>Expand: full CRP protocol guide</strong> (reference: triage & appendices are orchestrator-side after your suggestions)</summary>
...
**Path:** `/Users/neilyashinsky/Documents/dev/startd8-sdk/docs/design/TUI_MODEL_LIST_MANAGEMENT_REQUIREMENTS.md`  ·  **Size:** 121 lines · 1286 words
...
> This section documents what changed between v0.1 (the naive pre-planning draft)
> and v0.2 (after reading the actual code). The planning pass invalidated or
> reshaped most of the original requirements — the "add models via the TUI"
> capability is **largely already built**, just incomplete and fragmented.
...
| Updating `AGENT_TYPES` + `model_catalog` makes a model fully usable | `AnthropicProvider.HARDCODED_MODELS` is a **third** source and still tops out at `claude-opus-4-6`; `model_catalog` is a **fourth**, used only for tier routing/escalation | A model added in one source is invisible to the others — reconciliation is the core problem |

### Resolved open questions

- **OQ-1 → Persistence already exists.** Discovered models persist to `~/.startd8/discovered_models.json`. A user model list should live in a **sibling file** (e.g. `user_models.json`) so manual entries are not clobbered by the 24h discovery refresh.
- **OQ-2 → Validation is non-blocking.** No allowlist enforcement is needed for agent creation; the requirement is *informational* validation (flag unknown/likely-typo ids), not rejection.
- **OQ-3 → model_catalog needs metadata.** `_MODEL_REGISTRY` entries carry `tier` + `capabilities`, which drive routing/escalation. A TUI-added model must capture at least `tier` to be useful beyond the agent picker, or be explicitly scoped out (see REQ-TMM-203 / Non-Requirements).
...
The TUI agent picker shows **#4 ∪ #2 ∪ #3** (merged). It does **not** consult #1.
Manual one-off entry exists but is not saved. Net gap: **no manual, persistent,
TUI-driven CRUD over the model list, and no reconciliation across sources.**

---

## 2. Requirements

### Manual model-list CRUD (the core gap)

- **REQ-TMM-100** — The TUI MUST provide a "Manage Models" action (under agent/provider configuration) that lists all model ids currently visible for a chosen provider, annotated by origin (baseline / discovered / user-added).
- **REQ-TMM-101** — The TUI MUST let a user **add** a model id to a persistent, user-owned list for a provider, without editing source.
- **REQ-TMM-102** — The TUI MUST let a user **remove** a user-added model from that list. (Baseline/discovered entries MAY be hidden/suppressed but not deleted from their source.)
- **REQ-TMM-103** — The TUI MUST let a user **edit** a user-added model's id and metadata (at minimum `tier`).
- **REQ-TMM-104** — User-added models MUST persist across sessions in a dedicated file (e.g. `~/.startd8/user_models.json`), **separate** from `discovered_models.json` so the 24h discovery refresh never overwrites manual entries.

### Surfacing in the agent-config picker

- **REQ-TMM-110** — The agent-config model picker (`_configure_builtin_agent`) MUST include user-added models, grouped under a clear separator (alongside the existing "Discovered Models" group).
- **REQ-TMM-111** — When a user enters a model via the existing "Enter custom model name" path, the TUI MUST offer to persist it to the user list (REQ-TMM-104), converting one-off entry into reusable entry.

### Validation (informational, non-blocking)

- **REQ-TMM-120** — On add/entry, the TUI MUST flag a model id that is in **none** of the four sources as "unrecognized" with a confirmation prompt, to catch typos (e.g. `claude-opus-4-8`), but MUST allow the user to proceed.
- **REQ-TMM-121** — Validation MUST NOT block agent creation for unknown models (consistent with current provider warn-don't-reject behavior).

### Reconciliation across sources

- **REQ-TMM-130** — A user-added model with a declared `tier` MUST be discoverable by `model_catalog` consumers (`get_model_info`, `is_known_model`, `get_latest_model`, escalation) so that routing is not blind to TUI-added models. (Implementation: `model_catalog` reads the user-models file as an overlay over `_MODEL_REGISTRY`.)
- **REQ-TMM-131** — The merged, user-visible model list MUST be de-duplicated across all sources, preserving a single annotated origin per id (precedence: user-added > discovered > baseline).
...
- **OQ-A** — Should the user-models overlay be global (`~/.startd8/user_models.json`) or per-project (`./.startd8/`)? (Leaning global, matching `custom_agents.json` and `discovered_models.json` defaults.)
- **OQ-B** — For REQ-TMM-130, is `tier` alone sufficient for routing, or must `capabilities` also be captured? (Routing uses tier; capability filters use capabilities. v1 may require `tier`, default `capabilities` to `{"text","code"}`.)
- **OQ-C** — Should "remove" of a *baseline* model be supported as a per-user suppression list, or is hide-only out of scope for v1? (Currently scoped as suppression-optional under REQ-TMM-102.)

---

## 5. Affected Code (from planning pass)

| Area | File / anchor | Change shape |
|------|---------------|--------------|
| Manual CRUD store | new `user_models.py` or extend `model_discovery.py` | Load/save `user_models.json`, merge helpers |
| TUI "Manage Models" menu | `tui_improved.py` (near discover-models flow `~1994-2104`) | New action + add/edit/remove prompts |
| Agent-config picker | `tui_improved.py:2130-2188` | Add user-models group; persist-on-custom-entry |
| Catalog overlay | `model_catalog.py:get_model_info / is_known_model / _MODEL_REGISTRY` | Read overlay file as supplement |
| Unknown-model flag | `tui_improved.py` add/entry paths | Cross-check all four sources before confirm |

---

*v0.2 — Post-planning self-reflective update. Of 11 original v0.1 requirements,
~6 were narrowed or invalidated (capabilities already existed), 4 were reframed
toward reconciliation, and 3 open questions were resolved by reading the code.
The >30% revision rate confirms the v0.1 draft was premature — exactly what the
reflective loop is meant to catch at document cost rather than refactor cost.*
```
...
- [ ] Appended `## Review Suggestions — R1 — <model-id> — <UTC date>` section to the **end** of each source file in scope.
- [ ] Each appendix contains: executive summary (≤10 bullets) + full numbered suggestion list (**R1-S\*** / **R1-F\*** as applicable); optional stress-test subsection if used.
- [ ] No existing prose above the appendix was modified, reordered, or deleted.
- [ ] Chat reply is a **short** (1–3 line) write-confirmation listing file paths and suggestion counts — **not** the suggestion content.

**Stop after persisting** — do not triage, do not emit merged documents in chat or in the files, do not modify existing prose, do not fill CRP guide Appendix A/B/C.
