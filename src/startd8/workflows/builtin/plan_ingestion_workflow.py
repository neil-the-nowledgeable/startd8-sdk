...
# ---------------------------------------------------------------------------
# Mottainai pre-assembly (FR-MPA-001 through FR-MPA-011)
# ---------------------------------------------------------------------------
        ...
        if forward_manifest_dict is not None and forward_manifest is not None:
            try:
                if hasattr(forward_manifest, "file_specs") and forward_manifest.file_specs:
                    ...
                    # FR-MPA-001: Persist skeleton source text (not just metadata)
                    if render_result.specs:
                        skeleton_sources = dict(render_result.specs)

                    # FR-MPA-009/010/011 + FR-MPA-002/003: Element registry
                    # population, cross-run lookup, classification, template fill
