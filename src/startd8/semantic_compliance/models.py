...
"""Semantic Compliance Reviewer data contract.

Mirrors ``docs/design/semantic-compliance/SEMANTIC_COMPLIANCE_REPORT_SCHEMA.md`` v1.0. The
verdict/issue shape matches ``micro_prime.models.SemanticVerificationResult`` (K-7) so the SCR is a
drop-in *producer* of that contract (FR-6/FR-13).
"""
...
class ReportConfig:
    ...
    max_inconclusive_rate: float = 0.10      # R4-F1
...
class SemanticComplianceReport:
    ...
    def to_dict(self) -> dict:
        ...
        # Nest run identity per the schema's `run` object.
        d["run"] = {
            "run_id": d.pop("run_id"),
            "output_dir": d.pop("output_dir"),
            "language": d.pop("run_language"),
        }
        return d
