...
RETROSPECTIVE_SCHEMA = "startd8.kickoff.retrospective.v1"
...
def _milestones(entries: Sequence[dict]) -> List[str]:
    ...
    for e in entries or ():
        if not isinstance(e, dict):
            continue
        if prev is not None:
            pr, cur = prev.get("readiness_percent"), e.get("readiness_percent")
            if pr is not None and cur is not None and cur != pr:
                arrow = "→"
                out.append(f"readiness {pr}% {arrow} {cur}%")
            pb, cb = int(prev.get("blocked", 0) or 0), int(e.get("blocked", 0) or 0)
            if pb > 0 and cb == 0:
                out.append("cleared all blockers")
            pp, cp = int(prev.get("proposals_pending", 0) or 0), int(e.get("proposals_pending", 0) or 0)
            if cp < pp:
                out.append(f"{pp - cp} proposal(s) applied")
            if prev.get("snapshot_status") != "present" and e.get("snapshot_status") == "present":
...
def build_retrospective(status: Dict[str, Any], ledger_entries: Sequence[dict]) -> Dict[str, Any]:
    ...
    readings = [e.get("readiness_percent") for e in entries if e.get("readiness_percent") is not None]
    ...
    journey = {
        ...
        "updated_at": entries[-1].get("ts") if entries else None,
