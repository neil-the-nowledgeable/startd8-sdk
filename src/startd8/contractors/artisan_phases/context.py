...
class TruncationStrategy(CompressionStrategy):
    ...
    def __init__(self, encoding_name: str = "cl100k_base"):
        ...
        try:
            ...
            logger.error(
                f"Invalid encoding name: {encoding_name}. Falling back to cl100k_base."
            )
...
class SummarizationStrategy(CompressionStrategy):
    ...
    def __init__(self, target_ratio: float = 0.3, encoding_name: str = "cl100k_base"):
        ...
        if not 0.0 < target_ratio <= 1.0:
            raise ValueError(
                "target_ratio must be between 0.0 (exclusive) and 1.0 (inclusive)."
            )
    ...
    def compress(self, content: str, target_tokens: int, current_tokens: int) -> str:
        ...
        logger.debug(
            "Using truncation fallback for summarization."
            f" Target: {effective_target} tokens"
        )
...
class TokenCounter:
    ...
    def count_tokens(self, text: str) -> int:
        ...
        if not isinstance(text, str):
            logger.warning(
                "Attempted to count tokens for non-string"
                f" type: {type(text)}. Returning 0."
            )
            return 0
...
class ContextBudget:
    ...
    def __init__(
        ...
        if self.effective_budget <= 0 and total_budget > 0:
            logger.warning(
                f"Total budget {total_budget} is too small"
                f" for safety margin"
                f" ({safety_margin * 100}%) and overhead"
                f" ({structural_overhead} tokens)."
                f" Effective budget is"
                f" {self.effective_budget}."
            )
    ...
    def allocate(
        self,
        token_count: int,
        component_compressed: bool = False,
        ...
        if token_count < 0:
            logger.warning(
                f"Attempted to allocate negative token count: {token_count}. Ignoring."
            )
            return False
        if force_allocate:
    ...
    def get_metrics(self) -> BudgetMetrics:
        ...
        """
        utilization = (
            self._used_tokens / self._total_budget * 100
        ...
        return BudgetMetrics(
            ...
            components_included=self._components_included,
            components_excluded=self._components_excluded,
            components_compressed=self._components_compressed,
...
class ContextAssembler:
    ...
    def __init__(
        ...
        self._compression_strategies: Dict[CompressionType, CompressionStrategy] = {
            CompressionType.NONE: NoCompressionStrategy(),
            CompressionType.TRUNCATE: TruncationStrategy(encoding_name),
            CompressionType.SUMMARIZE: SummarizationStrategy(),
        }
    ...
    def add_component(
        ...
        if not isinstance(content, str):
            content = str(content)
            logger.warning(
                f"Component '{name}' content was not a string. Converted to string."
            )
        ...
        if compression_type == CompressionType.CUSTOM:
            logger.warning(
                "Custom compression type specified for"
                f" '{name}'. No custom strategy"
                " registered, defaulting to TRUNCATE."
            )
    ...
    def add_file_context(
        ...
        try:
            ...
            if len(content) > 1024 * 1024:
                logger.warning(
                    f"File '{file_path}' is very large"
    ...
    def assemble(self, format_template: Optional[str] = None) -> ContextResult:
        ...
        Args:
            format_template: Optional template for
                formatting. Available placeholders:
                component names, {all_content},
                {system_prompt}
        ...
        logger.info(
            f"Starting assembly with"
            f" {len(sorted_components)} components,"
            f" budget={self.budget.effective_budget}"
            " tokens"
        )
        for component in sorted_components:
            self._process_component(component)
        final_content = self._format_context(
            self._included_components, format_template
        )
        actual_tokens = self.token_counter.count_tokens(final_content)
        metrics = self.budget.get_metrics()
        if metrics.is_over_budget:
            self._warnings.append(
                "Context assembly exceeded effective"
                f" budget. Used: {metrics.used_tokens}"
                " / Effective:"
                f" {self.budget.effective_budget}."
                " This is due to force-added"
                " CRITICAL components."
            )
        logger.info(
            f"Assembly complete:"
            f" {metrics.components_included} included,"
            f" {metrics.components_excluded} excluded,"
            f" {metrics.components_compressed}"
            f" compressed, {actual_tokens} total tokens"
        )
        return ContextResult(
            content=final_content,
            total_tokens=actual_tokens,
            components=self._included_components,
            excluded_components=self._excluded_component_names,
            compressed_components=self._compressed_component_names,
            metrics=metrics,
    ...
    def _process_component(self, component: ContextComponent):
        ...
        if self.budget.can_fit(component.token_count):
            ...
            self._warnings.append(
                "CRITICAL component"
                f" '{component.name}' exceeds budget"
                f" ({component.token_count} tokens"
                f" needed,"
                f" {self.budget.remaining_tokens}"
                " available). Force-adding."
            )
        elif component.compression_type != CompressionType.NONE:
            compressed = self._try_compress_component(
                component, self.budget.remaining_tokens
            )
            if compressed:
                ...
                logger.debug(
                    f"Compressed '{component.name}':"
                    f" {component.token_count} ->"
            ...
            self.budget.record_exclusion()
            logger.debug(
                f"Excluded '{component.name}': doesn't fit, no compression allowed"
            )
            return
        allocated = self.budget.allocate(
            token_count=component_to_add.token_count,
            component_compressed=was_compressed,
            force_allocate=component.priority == ContextPriority.CRITICAL,
        )
        if allocated:
            ...
            self._warnings.append(
                ...
                f" '{component.name}' unexpectedly."
                " This may indicate a bug."
            )

    def _try_compress_component(
        self, component: ContextComponent, available_tokens: int
        ...
        try:
            compressed_content = strategy.compress(
                component.content, target_tokens, component.token_count
            )
            ...
            if compressed_token_count <= available_tokens:
                return replace(
                    ...
                    metadata={**component.metadata, "was_compressed": True},
                )
            else:
                logger.warning(
                    "Compression for"
                    f" '{component.name}' failed to"
                    " meet target. Original:"
                    f" {component.token_count},"
                    " Compressed:"
                    f" {compressed_token_count},"
    ...
    def _get_compression_strategy(
        ...
        if strategy is None:
            logger.warning(
                "No compression strategy found for"
                f" {compression_type}. Defaulting to"
    ...
    def _format_context(
        self, components: List[ContextComponent], template: Optional[str] = None
    ) -> str:
        """
        ...
        if template:
            ...
            system_prompt_comp = next(
                (c for c in components if c.metadata.get("is_system_prompt")), None
            )
            ...
            try:
                ...
                logger.warning(
                    f"Template placeholder {e} not"
                    " found in context data."
                    " Using default formatting."
                )
                return "\n\n---\n\n".join((c.content for c in components))
            except Exception as e:
                logger.error(
                    f"Error formatting template: {e}. Using default formatting."
                )
                return "\n\n---\n\n".join((c.content for c in components))
        else:
            return "\n\n---\n\n".join((c.content for c in components))
