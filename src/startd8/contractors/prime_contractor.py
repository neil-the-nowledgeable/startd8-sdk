...
class PrimeContractorWorkflow:
    ...
    def _try_deterministic_file_shortcut(
        ...
        - an **owned frontend-codegen file** (carries the ``// GENERATED from … schema-sha256``
          header) that is **currently in-sync** with the Prisma schema (REQ deterministic-
          frontend FR-8B). Header presence alone is NOT enough — a stale or hand-edited owned
          file must fall through to the LLM, never be silently skipped.

        Owned-codegen skip fires only when the file is already on disk AND the run passed the
        batch dirty-tree gate — i.e. the owned file is committed (clean tree) or the run uses
        ``--allow-dirty``. Automatic pre-write before the feature loop is deferred (there is no
        per-file overwrite guard yet, so writing into a possibly-dirty tree is unsafe).
        ...
        output_dir = self._resolve_output_dir()
        schema_text: Optional[str] = None  # lazily read iff an owned-codegen target appears
        resolved: list[str] = []
        for tf in target_files:
            p = Path(tf)
            is_build = (
            ...
            if not is_build:
                # Owned-codegen path: skip only if the on-disk file is an owned generated
                # file that re-renders byte-identically from the current schema (in-sync).
                try:
                    content = found.read_text(encoding="utf-8")
                except OSError:
                    return None
                if schema_text is None:
                    schema_text = _read_prisma_anchor(
                        self.seed_upstream_anchors, self.project_root
                    )
                from ..frontend_codegen.drift import (
                    is_owned_generated_file,
                    owned_file_in_sync,
                )

                if is_owned_generated_file(content) and not schema_text:
                    # Owned file present but no Prisma anchor resolved — surface the misconfig
                    # instead of silently sending it to the LLM (audit M1).
                    logger.info(
                        "Owned generated file '%s' present but no Prisma anchor resolved "
                        "— not skipping (check seed upstream anchors).",
                        tf,
                    )
                    return None
                if not (schema_text and owned_file_in_sync(schema_text, content)):
                    return None
