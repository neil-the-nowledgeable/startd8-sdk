...
class PrimeContractorWorkflow:
    ...
    def __init__(self, project_root: Optional[Path]=None, dry_run: bool=False, auto_commit: bool=False, strict_checkpoints: bool=False, max_retries: int=6, allow_dirty: bool=False, auto_stash: bool=False, code_generator: Optional[CodeGenerator]=None, instrumentor: Optional[Instrumentor]=None, size_estimator: Optional[SizeEstimator]=None, merge_strategy: Optional[MergeStrategy]=None, on_feature_complete: Optional[FeatureCompleteCallback]=None, on_checkpoint_failed: Optional[CheckpointFailedCallback]=None, max_lines_per_feature: int=150, max_tokens_per_feature: int=500, check_truncation: bool=True, resume: bool=False, cli_mode: Optional[str]=None, force_mode: Optional[str]=None, context_strategy: Optional[ContextResolutionStrategy]=None, strict_mode: bool=False, walkthrough: bool=False, repair_config: Optional[Any]=None, edit_min_pct: int=80, review_enabled: bool=True, review_agent: Optional[str]=None, quality_gate_enabled: bool=True, quality_gate_threshold: float=0.5, skip_deterministic_shortcut: bool=False):
        ...
        # Benchmark / LLM-maximize mode (FR-1/FR-27): disable the entire Phase-0
        # deterministic shortcut cascade so every file is generated by the LLM.
        # Constructor kwarg OR env var STARTD8_LLM_MAXIMIZE. Default off.
        self.skip_deterministic_shortcut: bool = bool(skip_deterministic_shortcut) or (
            os.getenv("STARTD8_LLM_MAXIMIZE", "0").lower() in ("1", "true", "yes", "on")
        )
        # R1-S4: behavioral skip counter — every deterministic shortcut routes through
        # _run_shortcut(), which increments this when a shortcut fires. Benchmark mode
        # asserts it stays 0 (a refactor that re-introduces a shortcut is caught by the
        # counter/test, not by line-number pinning).
        self._deterministic_skip_count: int = 0
    ...
    def _run_shortcut(
        self,
        name: str,
        fn: "Callable[[FeatureSpec], Optional[bool]]",
        feature: FeatureSpec,
    ) -> Optional[bool]:
        """Single chokepoint for the Phase-0 deterministic shortcut cascade.

        FR-1/FR-27 (benchmark / LLM-maximize mode): when
        ``self.skip_deterministic_shortcut`` is set, every deterministic shortcut
        is disabled so the file is generated by the LLM instead of being claimed
        $0-deterministically. R1-S4: the skip counter is incremented whenever a
        shortcut actually fires, giving a behavioral signal (benchmark mode asserts
        it stays 0) instead of relying on line-number-pinned early returns.

        New Phase-0 shortcuts MUST route through this method so they inherit the
        gate and the counter automatically.
        """
        # getattr defaults keep this safe for partial constructions (test helpers that
        # bypass __init__ via __new__) — the attrs always exist after real __init__.
        if getattr(self, "skip_deterministic_shortcut", False):
            return None
        result = fn(feature)
        if result is not None:
            self._deterministic_skip_count = getattr(self, "_deterministic_skip_count", 0) + 1
            logger.debug(
                "Deterministic shortcut '%s' fired for '%s' (skip #%d)",
                name, feature.name, self._deterministic_skip_count,
                extra={"feature_name": feature.name, "shortcut": name},
            )
        return result

    @property
    def deterministic_skip_count(self) -> int:
        """Number of Phase-0 deterministic shortcuts that fired this run (R1-S4).

        In benchmark/LLM-maximize mode this MUST be 0 — every file should be
        LLM-generated. The benchmark harness asserts on this post-run.
        """
        return getattr(self, "_deterministic_skip_count", 0)
    ...
    def develop_feature(self, feature: FeatureSpec, prior_error: Optional[str]=None) -> bool:
        ...
        # Phases 0–0.7: deterministic shortcut cascade. All routed through
        # _run_shortcut() so a single gate (benchmark/LLM-maximize mode, FR-1/FR-27)
        # disables the whole cascade and one counter (R1-S4) records any skip.
        # Phase 0: Copy shortcut
        copy_result = self._run_shortcut("copy", self._try_copy_shortcut, feature)
        if copy_result is not None:
            return copy_result

        # Phase 0.5: Uncomment shortcut (REQ-TCW-300)
        uncomment_result = self._run_shortcut("uncomment", self._try_uncomment_shortcut, feature)
        ...
        det_result = self._run_shortcut("deterministic_file", self._try_deterministic_file_shortcut, feature)
        ...
        corpus_result = self._run_shortcut("corpus", self._try_corpus_shortcut, feature)
        ...
        try:
            # Phase 2: Mottainai reuse (also gated — benchmark mode forces fresh LLM generation)
            reuse_result = self._run_shortcut("mottainai_reuse", self._try_mottainai_reuse, feature)
    ...
    def _try_deterministic_file_shortcut(
        ...
        """Phase 0.6: Skip LLM for features whose targets are deterministically provided.

        A feature is skipped (marked GENERATED, $0.00) when **every** target file is either:
        - a build/config file already generated by ``_ensure_dependency_file()`` / a
          solution/project generator (``go.mod``/``package.json``/``tsconfig.json``/…), or
        - a file judged **deterministically provided and in-sync** by a registered
          :class:`DeterministicFileProvider` (e.g. the Prisma→Zod provider in
          ``frontend_codegen``; later, a proto/gRPC-stub provider for microservices). The core
          asks the provider registry — it does **not** import or know any specific stack. A
          provider reports in-sync only after verifying against the source; a stale/hand-edited
          file falls through to the LLM, never silently skipped.

        Owned skip fires only when the file is already on disk AND the run passed the batch
        dirty-tree gate (committed/clean tree, or ``--allow-dirty``); automatic pre-write before
        the feature loop is deferred (no per-file overwrite guard yet).

        Returns True (skipped), or None (not deterministically provided — proceed to LLM).
        """
        target_files = feature.target_files or []
        if not target_files:
            return None
        ...
        output_dir = self._resolve_output_dir()
        context: Optional["ProviderContext"] = None  # built lazily on first non-build target
        resolved: list[str] = []
        for tf in target_files:
            p = Path(tf)
            is_build = (
                p.name in self._DETERMINISTIC_BUILD_NAMES
                or p.suffix in self._DETERMINISTIC_BUILD_EXTENSIONS
            )
            # Resolve the target on disk: output dir first, then project root.
            if p.is_absolute():
                found = p if p.is_file() else None
            else:
                found = next(
                    (c for c in (output_dir / tf, self.project_root / tf) if c.is_file()),
                    None,
                )
            if found is None:
                return None  # not pre-generated — need the LLM

            if not is_build:
                # Non-build target: skip only if a registered provider judges it
                # deterministically provided AND in-sync with its source. The core stays
                # language-agnostic — Prisma/TS (and future proto/gRPC) knowledge lives in the
                # providers, not here.
                try:
                ...
                if context is None:
                    context = ProviderContext(
                        project_root=self.project_root,
                        source_anchors=tuple(self.seed_upstream_anchors or ()),
                    )
                if not is_deterministically_provided(found, content, context):
                    return None
