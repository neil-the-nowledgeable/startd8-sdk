...
# SPIKE: the always-on default nav derives from THREE inputs (schema + views.yaml + pages.yaml),
# reusing the existing forms-sha256 (views) + pages-sha256 (pages) header lines — no new regex.
_NAV_KINDS: frozenset = frozenset({"nav-partial"})
...
def nav_stale_reason(
    ondisk_text: str, *, schema_sha: str, views_sha: str, pages_sha: str
) -> Optional[str]:
    """SPIKE: for the default-nav partial, why it is **stale**, or ``None`` if all three inputs match.

    Three inputs (schema + views.yaml + pages.yaml) → stale if **any** embedded hash differs (mirrors
    :func:`ai_layer_stale_reason`). Reuses ``embedded_forms_sha`` (views) + ``embedded_pages_sha``.
    """
    checks = (
        ("schema", embedded_schema_sha(ondisk_text), schema_sha),
        ("views", embedded_forms_sha(ondisk_text), views_sha),
        ("pages", embedded_pages_sha(ondisk_text), pages_sha),
    )
    for label, embedded, current in checks:
        if embedded is None:
            return f"missing {label}-sha256 header"
        if embedded != current:
            return f"{label} changed (header {embedded[:12]}… != current {current[:12]}…) — regenerate"
    return None


def _check_nav_drift(
    schema_text, views_text, pages_text, ondisk_text, source_file, kind
) -> DriftResult:
    """SPIKE: drift for the always-on nav partial — stale if schema/views/pages changed; else re-render."""
    from .nav_generator import render_nav_partial

    reason = nav_stale_reason(
        ondisk_text,
        schema_sha=schema_sha256(schema_text),
        views_sha=schema_sha256(views_text or ""),
        pages_sha=schema_sha256(pages_text or ""),
    )
    if reason is not None:
        status = "tampered" if "missing" in reason else "stale"
        return DriftResult(status, DRIFT, reason)
    rendered = render_nav_partial(schema_text, views_text, pages_text, source_file)
    if rendered != ondisk_text:
        return DriftResult(
            "tampered", DRIFT,
            "owned nav partial differs from a fresh render of the unchanged inputs — hand-edited "
            "or generated by a different invocation",
        )
    return DriftResult("in_sync", IN_SYNC, "owned nav partial matches schema + views + pages")
...
def check_drift(
    ...
    *,
    ...
    if kind in _NAV_KINDS:
        # SPIKE: 3-input nav (schema + views.yaml [forms_text] + pages.yaml). owned_file_in_sync
        # already threads BOTH manifests, so the skip-hook recognizes this with no new plumbing.
