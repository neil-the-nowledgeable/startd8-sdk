...
def render_main(schema_text: str, source_file: str = "prisma/schema.prisma") -> str:
    ...
    body = (
        ...
        "    from .nav import load_hidden as _load_nav_hidden\n"
        "except ModuleNotFoundError:\n"
        "    _load_nav_hidden = None\n\n\n"
        "@asynccontextmanager\n"
        "async def lifespan(app: FastAPI):\n"
        "    init_db()\n"
        "    # Read nav visibility once at startup (FR-6/7); the _nav.html partial reads this state.\n"
        "    app.state.nav_hidden = _load_nav_hidden() if _load_nav_hidden is not None else frozenset()\n"
        "    yield\n\n\n"
