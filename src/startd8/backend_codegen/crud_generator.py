...
def render_main(schema_text: str, source_file: str = "prisma/schema.prisma") -> str:
    ...
    body = (
        ...
        "try:  # optional always-on default nav (generate backend; suppressed by --no-nav)\n"
        "    from .nav import visible_nav as _visible_nav\n"
        "except ModuleNotFoundError:\n"
        "    _visible_nav = None\n"
        "try:  # optional live nav toggle store (FR-29): DB-backed hide/reveal layered on the config file\n"
        "    from .nav_store import ensure_nav_table as _ensure_nav_table, resolve_visible as _resolve_nav\n"
        "except ModuleNotFoundError:\n"
        "    _ensure_nav_table = _resolve_nav = None\n\n\n"
        "@asynccontextmanager\n"
        "async def lifespan(app: FastAPI):\n"
        "    init_db()\n"
        "    # Resolve the visible nav at startup (FR-6/7/19/29): config file UNION DB-hidden (live toggle).\n"
        "    if _ensure_nav_table is not None and _resolve_nav is not None:\n"
        "        from sqlmodel import Session\n"
        "        from .db import engine\n"
        "        _ensure_nav_table(engine)  # idempotent CREATE TABLE IF NOT EXISTS (mode-invariant)\n"
        "        with Session(engine) as _s:\n"
        "            app.state.nav = _resolve_nav(_s)\n"
        "    elif _visible_nav is not None:\n"
        "        app.state.nav = _visible_nav()  # config-only fallback (store absent)\n"
        "    else:\n"
        "        app.state.nav = []\n"
        "    yield\n\n\n"
        ...
        "try:  # optional generated home/index (FR-28; suppressed by --no-nav or an authored / page)\n"
        "    from .index import index_router\n"
        "except ModuleNotFoundError:\n"
        "    index_router = None\n"
        "if index_router is not None:\n"
        "    # mounted AFTER pages_router so an authored `/` content page always wins (shadow guard)\n"
        "    app.include_router(index_router)  # GET / -> the 'what's in this app' index\n\n"
        "try:  # optional live nav admin toggle (FR-29; suppressed by --no-nav)\n"
        "    from .nav_admin import nav_admin_router\n"
        "except ModuleNotFoundError:\n"
        "    nav_admin_router = None\n"
        "if nav_admin_router is not None:\n"
        "    app.include_router(nav_admin_router)  # GET/POST /admin/nav (live hide/reveal)\n\n"
        ...
        "try:  # optional first-run onboarding (views.yaml `onboarding:`); welcome route (FR-ONB)\n"
        "    from .onboarding import onboarding_routers\n"
        "except ModuleNotFoundError:\n"
        "    onboarding_routers = []\n"
        "for _onboarding_router in onboarding_routers:\n"
        "    app.include_router(_onboarding_router)  # GET welcome / tips / empty-state checklist\n"
