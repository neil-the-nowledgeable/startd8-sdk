...
#: The observability-generation capabilities THIS startd8 supports — a self-describing feature set
#: stamped into every observability-quality.json. It exists to make the "generate imported a STALE
#: startd8 and silently produced thin/old artifacts" class LOUD instead of a silent coverage drop:
#: a durable pass declares the features it expects and calls ``require_observability_capabilities``,
#: which fails loud (naming the stale sha/path) when the generated quality lacks one — the poka-yoke
...
OBSERVABILITY_CAPABILITIES: FrozenSet[str] = frozenset({
    ...
    "red-descriptor-grounding",      # sdk#400 (F1/F2)
    "l1c-coverage-bind-typegate",    # sdk#397 + sdk#404
    "summary-latency-slo",           # sdk#406 — system axis
    "summary-dashboard-panel",       # sdk#408 — human axis
    "counter-total-strip",           # sdk#409
    "gauge-freshness-installed",     # sdk#411 — gauge system axis (installed mode)
    "deployment-mode-env-override",  # sdk#423 (+ sdk#425 validation)
    "secondary-red-families",        # sdk#429 — secondary counter/summary coverage
})


class StaleSdkError(RuntimeError):
    """Raised when generated observability quality lacks capabilities the caller requires —
    i.e. it was produced by a STALE/older startd8 than expected (the import-shadow class)."""


def require_observability_capabilities(
    quality: Dict[str, Any],
    required: Iterable[str],
    *,
    strict: bool = True,
) -> List[str]:
    """Fail LOUD when an ``observability-quality.json`` was generated by a startd8 MISSING any of
    the *required* capabilities — the poka-yoke for the stale-SDK-import class.

    The generator stamps ``sdk_capabilities`` (+ ``sdk_sha``/``sdk_module_path``) into the quality
    report's provenance. A durable pass that expects e.g. the gauge-freshness + secondary-red-family
    SCORE features calls this with those keys; if the quality was scored by an OLDER startd8 (a
    ...
    # Locate the stamp in EITHER a raw observability-quality.json (top-level provenance/aggregate) OR
    # a scorecard SIDECAR that embeds the generate's quality under a "quality" sub-block — the durable
    # pass reads the sidecar (the run-dir quality.json is torn down), and the sidecar carries the
    # generate's aggregate (where sdk#432 also stamps sdk_capabilities). Search both shapes so the
    # guard is readable regardless of which artifact the caller has. (A stale generate leaves the
    # stamp EMPTY in either shape → the guard correctly fires.)
    _q = quality.get("quality") if isinstance(quality.get("quality"), dict) else {}
    _blocks = [
        quality.get("provenance") or {},
        quality.get("aggregate") or {},
        _q.get("provenance") or {},
        _q.get("aggregate") or {},
    ]
    stamped: Set[str] = set()
    for _b in _blocks:
        if isinstance(_b, dict):
            stamped |= set(_b.get("sdk_capabilities") or [])
    missing = [c for c in required if c not in stamped]
    if missing and strict:
        def _first(key: str) -> Optional[str]:
            for _b in _blocks:
                if isinstance(_b, dict) and _b.get(key):
                    return _b[key]
            return None
        sha = _first("sdk_sha") or "<absent>"
        path = _first("sdk_module_path") or "<unknown>"
        raise StaleSdkError(
            "observability-quality.json was generated by a startd8 MISSING required capabilities "
            f"{sorted(missing)} — it was scored by a STALE/older SDK (sdk_sha={sha}, "
            f"startd8={path}). The generate imported the wrong/stale startd8: pin the generate's "
            "PYTHONPATH to an origin/main-current checkout (or reinstall -e). Stamped capabilities: "
            f"{sorted(stamped) or '<none — pre-capability-stamp SDK>'}."
        )
    return missing
...
def _write_quality_report(
    ...
    aggregate["sdk_capabilities"] = emit_prov.get("sdk_capabilities", [])
