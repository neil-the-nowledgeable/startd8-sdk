...
from .backends import RenderBackend, ScrapeIntent, get_adapter
...
def generate_service_monitor(
    service: ServiceHints,
    business: BusinessContext,
    backend: RenderBackend = RenderBackend.OPERATOR,
) -> ArtifactResult:
    """Generate a scrape artifact for a service.
    ...
    The scrape *intent* (name/namespace/interval/port/path) is backend-neutral; ``backend``
    selects the render mechanism (FR-13 / ADR-008): ``operator`` emits a ServiceMonitor CRD
    (default — byte-identical to before), ``agent`` emits an Alloy ``prometheus.scrape`` block
    for clusters with no prometheus-operator.
    """
    derivations: List[DerivationTrace] = []
    ...
    rendered = get_adapter(backend).render_scrape(intent, _derivation_comment(derivations))
    return ArtifactResult(
...
def generate_runbook(
    ...
    # FR-B5 / R2-F3: Risks must carry ≥1 non-heading line (not a hollow marker).
    risk_bits = [
        f"Criticality is **{business.criticality}**; availability target **{avail}**.",
    ]
...
def generate_collector_enrichment(
    ...
    # Value round-2 #2 (completeness): the services that COULD carry business context but don't.
    # Surfaced only on the partial-adoption path (this artifact was generated ⇒ ≥1 service is tagged),
    # so an operator sees exactly which services still need criticality/owner instead of silence.
    # A fully-untagged project stays presence-gated off entirely (no artifact, no noise).
    _enriched = {sel for sel, _attr, _val in rows}
    services_missing = sorted(
        set((getattr(s, "service_name", "") or "").strip() or s.service_id for s in services)
        - _enriched
    )
...
# Span-metrics error dimension (metric_descriptor SPAN_METRICS profiles). Grounded, not guessed —
# so the error-ratio panel query matches what the connector actually emits.
_SPANMETRICS_ERROR_SELECTOR = 'status_code="STATUS_CODE_ERROR"'
_BUSINESS_CRITICALITY_DASHBOARD_PATH = "dashboards/business-criticality-dashboard-spec.yaml"
_BUSINESS_CRITICALITY_SLO_PATH = "slos/business-criticality-slos.yaml"

# The (D)-wire SLO counterpart to the severity map: an availability target per business-criticality tier
# (a stricter tier gets a tighter error budget). Percentages (matching the SLO ``target`` convention:
# ``_DEFAULT_THRESHOLDS['availability'] == '99'`` ⇒ ``target: 99.0``). Unknown tier ⇒ the default.
_CRITICALITY_TO_AVAILABILITY_TARGET: Dict[str, float] = {
    "critical": 99.9,
    "high": 99.5,
    "medium": 99.0,
    "low": 95.0,
...
def generate_business_criticality_dashboard(
    services: List[ServiceHints],
    business: BusinessContext,
    report: GenerationReport,
) -> ArtifactResult:
    """Project-level dashboard that CONSUMES the `business_criticality` span-metrics dimension
    (value round-2 #1). EC-2 made the enrichment queryable; this closes the loop by generating a
    view that ranks traffic + error ratio by business tier out of the box — so an operator sees
    business impact without hand-writing the PromQL.

    Presence-gated on the same signal as the dimension (≥1 service with criticality); otherwise the
    label wouldn't exist and the panels would be dead. Emitted as a ``dashboard_spec`` so it flows
    through the existing dashboard validation + Grafana-JSON render like every other dashboard.
    """
    has_criticality = any((getattr(s, "criticality", "") or "").strip() for s in services)
    # Stable slug (not project_id) so the rendered Grafana JSON is business-criticality-dashboard.json.
    slug = "business-criticality"
    if not has_criticality:
        return ArtifactResult(
            artifact_type="dashboard_spec",
            service_id=slug,
            output_path=_BUSINESS_CRITICALITY_DASHBOARD_PATH,
            status="skipped",
            skip_reason="no service carries business criticality",
        )

    # Queries use only calls_total + business_criticality + the grounded error selector — all present
    # once the enrichment processor + dimension are deployed (see DEPLOYING.md).
    panels: List[Dict[str, Any]] = [
        {
            "type": "timeseries",
        ...
        },
        {
            ...
            "expr": (
                f"sum by (business_criticality) (rate(calls_total{{{_SPANMETRICS_ERROR_SELECTOR}}}[5m])) "
    ...
    spec_dict: Dict[str, Any] = {
        ...
        "description": (
            "Traffic and error ratio grouped by business.criticality — consumes the span-metrics "
            "dimension emitted by collector_enrichment (REQ_COLLECTOR_ENRICHMENT)."
        ),
        "tags": ["generated", "observability", "business-impact"],
        "datasources": {
            "prometheus": (
                os.environ.get("OBS_PROM_DATASOURCE")
                or business.prometheus_datasource
    ...
    header = (
        "# Generated by startd8 observability artifact generator\n"
        "# Business-criticality overview — consumes the collector_enrichment span-metrics dimension.\n\n"
    )
    return ArtifactResult(
        artifact_type="dashboard_spec",
        service_id=slug,
        output_path=_BUSINESS_CRITICALITY_DASHBOARD_PATH,
        status="generated",
        content=header + yaml.dump(spec_dict, default_flow_style=False, sort_keys=False),
    )


def generate_business_criticality_slos(
    services: List[ServiceHints],
    business: BusinessContext,
    report: GenerationReport,
) -> ArtifactResult:
    """Project-level SLO set that CONSUMES the ``business_criticality`` span-metrics dimension — the SLO
    counterpart to :func:`generate_business_criticality_dashboard`. Where that dashboard *visualizes*
    traffic/error-ratio by tier, this gates an **error budget per tier**: one availability SLO per
    business-criticality tier present in the fleet, a stricter tier (``critical``) getting a tighter
    target than a lax one (``low``). This closes the (D) wire on the SLO side — the generated SLOs now
    query the label the enrichment stamps, instead of baking criticality in only at generation time.

    Presence-gated on the SAME signal as the dimension (≥1 service with criticality); absent ⇒ the label
    wouldn't exist and the SLIs would be dead, so the artifact self-skips (byte-identical to pre-feature).
    Emitted as an OpenSLO ``slo_definition`` so it flows through the existing SLO validation/render.
    """
    tiers = sorted({(getattr(s, "criticality", "") or "").strip()
                    for s in services if (getattr(s, "criticality", "") or "").strip()})
    slug = "business-criticality"
    if not tiers:
        return ArtifactResult(
            artifact_type="slo_definition",
            service_id=slug,
            output_path=_BUSINESS_CRITICALITY_SLO_PATH,
            status="skipped",
            skip_reason="no service carries business criticality",
        )

    window = business.slo_window
    sev_map = business.severity_map or _CRITICALITY_TO_SEVERITY
    default_target = float(_DEFAULT_THRESHOLDS["availability"])
    documents: List[str] = []
    for tier in tiers:
        target = _CRITICALITY_TO_AVAILABILITY_TARGET.get(tier, default_target)
        # Queries use only calls_total + business_criticality + the grounded error dimension — all
        # present once the enrichment processor + span-metrics dimension are deployed (see DEPLOYING.md).
        total_sel = f'{{business_criticality="{tier}"}}'
        good_sel = f'{{business_criticality="{tier}",status_code!="STATUS_CODE_ERROR"}}'
        slo = _openslo_doc(
            name=f"business-criticality-{tier}-availability",
            labels={"business_criticality": tier, "generated_by": "startd8"},
            description=(f"Availability SLO for the {tier} business-criticality tier — consumes the "
                        f"collector_enrichment span-metrics dimension (business_criticality on calls_total)."),
            target=target,
            window=window,
            severity=sev_map.get(tier, "warning"),
            indicator_spec={
                "ratioMetric": {
                    "total": {
                        "metricSource": {
                            "type": "prometheus",
                            "spec": {"query": f"sum(rate(calls_total{total_sel}[5m]))"},
                        },
                    },
                    "good": {
                        "metricSource": {
                            "type": "prometheus",
                            "spec": {"query": f"sum(rate(calls_total{good_sel}[5m]))"},
                        },
                    },
                },
            },
    ...
    header = (
        "# Generated by startd8 observability artifact generator\n"
        "# Business-criticality SLOs — one availability SLO per tier, consuming the collector_enrichment\n"
        "# span-metrics dimension (business_criticality on calls_total). See DEPLOYING.md.\n\n"
    )
    return ArtifactResult(
        artifact_type="slo_definition",
        service_id=slug,
