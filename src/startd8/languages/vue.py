    ...
    **C.3 — Validation (REQ-VUE-P-004, P-005):** :meth:`validate_syntax` runs the
    ...
    def coding_standards(self) -> str:
        ...
        sfc = (
            ...
            "- **REQ-VUE-P-011 (template/style):** the SDK’s **structured** MicroPrime "
            "path is **script-only**; ``<template>`` and ``<style>`` (incl. ``scoped``) "
            "are **LLM-edited** when the task requires UI/CSS — not splicer targets in "
            "this tier. SCSS/Less are out of scope unless the project already uses them. "
            "Preserve non-script blocks when only the script is in scope.\n"
