...
def check(
    ...
    project: Optional[Path] = typer.Option(
        None,
        "--project",
        help="Consumer project root: enables the contract DIFF (prisma/schema.prisma) and the "
        "env-keys agreement check (inputs/build-preferences.yaml).",
    ),
    ...
    live_schema_text = build_preferences_text = None
    if project is not None:
        schema = project / "prisma" / "schema.prisma"
        if schema.is_file():
            live_schema_text = schema.read_text(encoding="utf-8")
        prefs = project / "inputs" / "build-preferences.yaml"
        if prefs.is_file():
