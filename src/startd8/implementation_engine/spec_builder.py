...
def _build_exemplar_section(context: Dict[str, Any]) -> str:
    ...
    parts = [
        f"## Verified Reference (from {run_id}, score: {dq:.2f})\n",
        f"The following implementation was generated by this pipeline for a "
        f"structurally similar task ({fingerprint}) and scored {dq:.2f} with "
        f"full contract compliance. Use the same patterns, import structure, "
        f"and architectural approach.",
    ]
...
def _build_available_imports_section(context: Dict[str, Any]) -> str:
    ...
    try:
        ...
        return (
            "## Available Imports\n\n"
            f"The following packages are installed and available for import:\n\n"
            f"{packages_str}\n\n"
            f"{import_syntax}"
        )
