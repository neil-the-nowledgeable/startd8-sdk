...
MCP_LOG_JSON = _env_flag("STARTD8_MCP_LOG_JSON", default=False)
try:
...
# Startup bookkeeping (filled when running as __main__)
...
# Process correlation id (for logs/metrics)
RUN_ID = (os.getenv("STARTD8_MCP_RUN_ID") or uuid.uuid4().hex[:12]).strip()

# Stable identity labels for observability (logs/metrics)
SERVICE_NAME = (os.getenv("STARTD8_MCP_SERVICE") or "startd8-mcp").strip() or "startd8-mcp"
ENV_NAME = (os.getenv("STARTD8_MCP_ENV") or "local").strip() or "local"
# Default version matches pyproject.toml, but can be overridden at runtime.
VERSION = (os.getenv("STARTD8_MCP_VERSION") or "0.1.0").strip() or "0.1.0"

# Optional tracing (OpenTelemetry). Disabled by default.
MCP_TRACING = _env_flag("STARTD8_MCP_TRACING", default=False)
_TRACE_STATE: Dict[str, Any] = {"enabled": False, "error": None}
_TRACER = None
_TRACE_SPANS_BY_REQUEST_ID: Dict[str, Any] = {}
...
def _init_tracing() -> None:
    """
    Initialize optional OpenTelemetry tracing (OTLP -> Alloy/Tempo).

    Enable by setting:
      STARTD8_MCP_TRACING=1

    Endpoint selection (OTLP/HTTP):
      - OTEL_EXPORTER_OTLP_TRACES_ENDPOINT (preferred, includes /v1/traces)
      - else OTEL_EXPORTER_OTLP_ENDPOINT (base; we'll append /v1/traces)
      - else default: http://localhost:4318/v1/traces (works with Alloy exposing 4318)
    """
    global _TRACER
    if _TRACE_STATE.get("enabled"):
        return
    if not MCP_TRACING:
        return
    try:
        ...
        if not traces_ep:
            ...
            if base:
                traces_ep = base.rstrip("/") + "/v1/traces"
            else:
                traces_ep = "http://localhost:4318/v1/traces"
        ...
        _TRACE_STATE.update({"enabled": True, "error": None})
        _emit_event({"event": "tracing.started", "endpoint": traces_ep})
    except Exception as e:
        _TRACE_STATE.update({"enabled": False, "error": str(e)})
        try:
            _emit_event({"event": "tracing.error", "error": str(e)})
        except Exception:
            pass


def _trace_record_event(payload: Dict[str, Any]) -> None:
    """
    Create/end spans based on tool.start/tool.end events and add correlation ids.

    This gives you Tempo traces without touching every tool implementation.
    """
    if not _TRACE_STATE.get("enabled") or _TRACER is None:
        return
    try:
    ...
    # If we have an active span for this request_id, attach IDs for log->trace correlation.
    if request_id:
        span = _TRACE_SPANS_BY_REQUEST_ID.get(request_id)
        if span is not None:
            try:
    ...
    if event == "tool.start" and request_id and tool:
        try:
            span = _TRACER.start_span(f"mcp.tool.{tool}")
            try:
                ...
                pass
            _TRACE_SPANS_BY_REQUEST_ID[request_id] = span
            try:
                ctx = span.get_span_context()
                payload.setdefault("trace_id", f"{ctx.trace_id:032x}")
                payload.setdefault("span_id", f"{ctx.span_id:016x}")
    ...
    if event == "tool.end" and request_id:
        span = _TRACE_SPANS_BY_REQUEST_ID.pop(request_id, None)
        if span is None:
            return
        try:
            # Attach IDs even on end events.
            try:
                ctx = span.get_span_context()
                payload.setdefault("trace_id", f"{ctx.trace_id:032x}")
                payload.setdefault("span_id", f"{ctx.span_id:016x}")
            ...
            if duration_ms is not None:
                try:
                    span.set_attribute("mcp.duration_ms", int(duration_ms))
                except Exception:
                    pass
            if status != "ok" or err is not None:
                try:
                    from opentelemetry.trace.status import Status, StatusCode  # type: ignore

                    span.set_status(Status(StatusCode.ERROR, description=str(err or status)))
                except Exception:
                    pass
                try:
                    # Record a lightweight exception for visibility in Tempo.
                    span.record_exception(RuntimeError(str(err or status)))
                except Exception:
                    pass
            else:
                try:
...
def _emit_event(event: Dict[str, Any]) -> None:
    """
    Emit one JSON event line to either:
    - STARTD8_MCP_EVENT_LOG_FILE (JSONL), or
    - stderr when STARTD8_MCP_LOG_JSON=1
    """
    try:
        payload = dict(event)
        payload.setdefault("ts", datetime.now(timezone.utc).isoformat())
        payload.setdefault("run_id", RUN_ID)
        # Process identity (helps debug multi-process launches from Cursor)
        payload.setdefault("pid", os.getpid())
        payload.setdefault("ppid", os.getppid())
        payload.setdefault("service", SERVICE_NAME)
        payload.setdefault("env", ENV_NAME)
        payload.setdefault("version", VERSION)
        try:
            _trace_record_event(payload)
        except Exception:
            pass
        try:
            _metrics_record_event(payload)
        except Exception:
            pass
    ...
    path = (os.getenv("STARTD8_MCP_EVENT_LOG_FILE") or "").strip()
    if path:
        try:
    ...
    if MCP_LOG_JSON:
        try:
            _eprint(line, flush=True)
        except Exception:
            return


# -----------------------------------------------------------------------------
# Optional Prometheus metrics (Grafana/Prometheus friendly)
# -----------------------------------------------------------------------------
_METRICS_STATE: Dict[str, Any] = {"enabled": False, "port": None, "addr": None, "error": None}
_METRICS_INIT: bool = False
_METRIC_TOOL_REQUESTS = None
_METRIC_TOOL_DURATION = None
_METRIC_TOOL_INFLIGHT = None
_METRIC_TOOL_ERRORS = None
_METRIC_SKILL_CACHE_HITS = None
_METRIC_SKILL_CACHE_MISSES = None
_METRIC_SKILL_DISCOVERY_DURATION = None
_METRIC_SKILL_LLM_DURATION = None
_METRIC_TASK_PLAN_DURATION = None
_METRIC_TASK_PROMPT_BUILD_DURATION = None
_METRIC_TASK_AGENT_GENERATE_DURATION = None
_METRIC_TASK_ACTION_PARSE_DURATION = None
_METRIC_TASK_APPLY_ACTIONS_DURATION = None
_METRIC_BUILD_INFO = None
_METRIC_UP = None


def _init_metrics() -> None:
    """Initialize an optional Prometheus metrics HTTP endpoint (localhost)."""
    global _METRICS_INIT
    global _METRIC_TOOL_REQUESTS, _METRIC_TOOL_DURATION, _METRIC_TOOL_INFLIGHT, _METRIC_TOOL_ERRORS
    global _METRIC_SKILL_CACHE_HITS, _METRIC_SKILL_CACHE_MISSES, _METRIC_SKILL_DISCOVERY_DURATION, _METRIC_SKILL_LLM_DURATION
    global _METRIC_TASK_PLAN_DURATION, _METRIC_TASK_PROMPT_BUILD_DURATION, _METRIC_TASK_AGENT_GENERATE_DURATION
    global _METRIC_TASK_ACTION_PARSE_DURATION, _METRIC_TASK_APPLY_ACTIONS_DURATION
    global _METRIC_BUILD_INFO, _METRIC_UP
    if _METRICS_INIT:
        return
    _METRICS_INIT = True

    port_raw = (os.getenv("STARTD8_MCP_METRICS_PORT") or "").strip()
    if not port_raw:
        return
    try:
        port = int(port_raw)
    except Exception as e:
        _METRICS_STATE.update({"enabled": False, "port": port_raw, "error": f"Invalid metrics port: {e}"})
        return
    addr = (os.getenv("STARTD8_MCP_METRICS_ADDR") or "127.0.0.1").strip() or "127.0.0.1"

    try:
        with _redirect_stdout_to_stderr():
            from prometheus_client import Counter, Histogram, Gauge, Info, start_http_server  # type: ignore
            from prometheus_client import GCCollector, PlatformCollector, ProcessCollector, REGISTRY  # type: ignore
    except Exception as e:
        _METRICS_STATE.update({"enabled": False, "port": port, "addr": addr, "error": f"prometheus_client unavailable: {e}"})
        return
    ...
    try:
        ...
        # Ensure runtime collectors exist (best-effort; may already be registered).
        try:
            ProcessCollector(registry=REGISTRY)  # type: ignore
        except Exception:
            pass
        try:
            PlatformCollector(registry=REGISTRY)  # type: ignore
        except Exception:
            pass
        try:
            GCCollector(registry=REGISTRY)  # type: ignore
        except Exception:
            pass

        _METRIC_UP = Gauge(
            "startd8_mcp_up",
            "Startd8 MCP server is up (1)",
        )
        try:
            _METRIC_UP.set(1)
        except Exception:
        ...
        _METRIC_BUILD_INFO = Info(
            "startd8_mcp_build_info",
            "Build/runtime information for the Startd8 MCP server",
        )
        try:
            _METRIC_BUILD_INFO.info(
                {
        ...
        _METRIC_TOOL_REQUESTS = Counter(
            "startd8_mcp_tool_requests_total",
            "Total MCP tool calls observed",
            ["tool", "status"],
        )
        _METRIC_TOOL_DURATION = Histogram(
            "startd8_mcp_tool_duration_seconds",
            "MCP tool call duration in seconds",
            ["tool"],
        )
        _METRIC_TOOL_INFLIGHT = Gauge(
            "startd8_mcp_tool_inflight",
            "Current inflight MCP tool calls",
            ["tool"],
        )
        _METRIC_TOOL_ERRORS = Counter(
            "startd8_mcp_tool_errors_total",
            "Total MCP tool errors observed (normalized category)",
            ["tool", "category"],
        )
        _METRIC_SKILL_CACHE_HITS = Counter(
            "startd8_mcp_skill_cache_hits_total",
            "Skill discovery cache hits",
        )
        _METRIC_SKILL_CACHE_MISSES = Counter(
            "startd8_mcp_skill_cache_misses_total",
            "Skill discovery cache misses",
        )
        _METRIC_SKILL_DISCOVERY_DURATION = Histogram(
            "startd8_mcp_skill_discovery_seconds",
            "Skill discovery duration in seconds",
            ["cached"],
        )
        _METRIC_SKILL_LLM_DURATION = Histogram(
            "startd8_mcp_skill_llm_call_seconds",
            "LLM call duration for startd8_use_skill in seconds",
            ["mode", "status"],
        )
        _METRIC_TASK_PLAN_DURATION = Histogram(
            "startd8_mcp_tasks_plan_seconds",
            "Task runner planning duration in seconds",
        )
        _METRIC_TASK_PROMPT_BUILD_DURATION = Histogram(
            "startd8_mcp_tasks_prompt_build_seconds",
            "Task runner prompt build duration in seconds",
        )
        _METRIC_TASK_AGENT_GENERATE_DURATION = Histogram(
            "startd8_mcp_tasks_agent_generate_seconds",
            "Task runner agent generate duration in seconds",
        )
        _METRIC_TASK_ACTION_PARSE_DURATION = Histogram(
            "startd8_mcp_tasks_action_parse_seconds",
            "Task runner action parse duration in seconds",
        )
        _METRIC_TASK_APPLY_ACTIONS_DURATION = Histogram(
            "startd8_mcp_tasks_apply_actions_seconds",
            "Task runner apply actions duration in seconds",
            ["mode"],
        )
        _METRICS_STATE.update({"enabled": True, "port": port, "addr": addr, "error": None})
        _emit_event({"event": "metrics.started", "port": port, "addr": addr})
    except Exception as e:
        _METRICS_STATE.update({"enabled": False, "port": port, "addr": addr, "error": str(e)})


def _normalize_error_category(payload: Dict[str, Any]) -> str:
    """
    Map tool.end payloads to a small set of error categories.
    Keep cardinality LOW: never use raw error messages as labels.
    """
    try:
        status = str(payload.get("status") or "").strip().lower()
        err = str(payload.get("error") or "").strip().lower()
        err_type = str(payload.get("error_type") or payload.get("errorType") or "").strip().lower()
    except Exception:
        return "unknown"

    if status == "not_found" or err in {"not_found", "skill_not_found"}:
        return "not_found"
    if err == "invalid_params":
        return "invalid_params"
    if err == "failed_precondition" or status == "failed_precondition":
        return "failed_precondition"
    if err == "missing_api_key" or "api_key" in err:
        return "auth"
    if "timeout" in err:
        return "timeout"
...
def _metrics_record_event(payload: Dict[str, Any]) -> None:
    """Update Prometheus metrics for relevant events (no-op unless enabled)."""
    if not _METRICS_STATE.get("enabled"):
        return
    if _METRIC_TOOL_REQUESTS is None or _METRIC_TOOL_DURATION is None:
        return
    event = str(payload.get("event") or "")
    if event not in {"tool.start", "tool.end"}:
        return
    tool = str(payload.get("tool") or "unknown")

    # Inflight gauge
    if _METRIC_TOOL_INFLIGHT is not None:
        try:
            if event == "tool.start":
                _METRIC_TOOL_INFLIGHT.labels(tool=tool).inc()
            else:
                _METRIC_TOOL_INFLIGHT.labels(tool=tool).dec()
        except Exception:
            pass
    ...
    # Treat missing status as ok (many tool.end payloads omit it).
    status = str(payload.get("status") or "ok")
    try:
        _METRIC_TOOL_REQUESTS.labels(tool=tool, status=status).inc()
    except Exception:
        pass
    try:
        dur_ms = payload.get("duration_ms")
        if dur_ms is not None:
            _METRIC_TOOL_DURATION.labels(tool=tool).observe(float(dur_ms) / 1000.0)
    except Exception:
        pass

    # Normalized error counter (avoid high-cardinality).
    try:
        if status != "ok" and _METRIC_TOOL_ERRORS is not None:
            category = _normalize_error_category(payload)
            _METRIC_TOOL_ERRORS.labels(tool=tool, category=category).inc()
    except Exception:
        pass

    # Skill LLM call histogram (only when we have explicit call timing).
    try:
        if tool == "startd8_use_skill" and _METRIC_SKILL_LLM_DURATION is not None:
            llm_ms = payload.get("llm_ms")
            mode = str(payload.get("mode") or "unknown")
            if llm_ms is not None:
                _METRIC_SKILL_LLM_DURATION.labels(
                    mode=mode,
                    status=("ok" if status == "ok" else "error"),
                ).observe(float(llm_ms) / 1000.0)
    except Exception:
        pass
...
def _redirect_stdout_to_stderr():
    ...
    # IMPORTANT:
    # This must yield exactly once. A contextmanager generator that yields twice
    # can crash with: "generator didn't stop after throw()".
    try:
        cm = redirect_stdout(sys.stderr)
        cm.__enter__()
    except Exception:
        # If redirection fails (rare), proceed without it.
        yield
        return
    try:
        yield
...
def _find_skills(max_results: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    if max_results is None and MCP_SKILL_CACHE_TTL_SECONDS > 0:
        try:
            ...
            if cache_key == key_now and now < float(_SKILLS_CACHE.get("expires_at", 0.0) or 0.0):
                ...
                try:
                    if _METRIC_SKILL_CACHE_HITS is not None:
                        _METRIC_SKILL_CACHE_HITS.inc()
                except Exception:
                    pass
                try:
                    if _METRIC_SKILL_DISCOVERY_DURATION is not None:
                        _METRIC_SKILL_DISCOVERY_DURATION.labels(cached="true").observe(
                            float(time.perf_counter() - discovery_started)
                        )
                except Exception:
                    pass
            ...
            pass
        try:
            if _METRIC_SKILL_CACHE_MISSES is not None:
                _METRIC_SKILL_CACHE_MISSES.inc()
        except Exception:
            pass
    ...
    try:
        if _METRIC_SKILL_DISCOVERY_DURATION is not None:
            _METRIC_SKILL_DISCOVERY_DURATION.labels(cached="false").observe(
                float(time.perf_counter() - discovery_started)
            )
    except Exception:
        pass
...
def _response(
    ...
    **kwargs: Any,
    ...
    if request_id is not None or meta is not None:
        m = dict(meta or {})
        if request_id is not None:
            m.setdefault("request_id", request_id)
    ...
    _emit_event(
        {
            ...
            "params": {
                "response_format": getattr(params.response_format, "value", str(params.response_format)),
                "include_details": bool(params.include_details),
            },
        }
    )
    try:
            ...
            _emit_event(
                {
                    ...
                    "duration_ms": int((time.perf_counter() - started) * 1000),
                    "status": "not_found",
                    "available_skills_count": len(skill_names),
                }
            )
...
async def startd8_use_skill(params: UseSkillInput) -> str:
        ...
        _emit_event(
            {
                ...
                "request_id": request_id,
                "duration_ms": int((time.perf_counter() - started_outer) * 1000),
                "status": status,
                **fields,
            }
        )
    ...
    _emit_event(
        {
            ...
            "params": {
                ...
                "response_format": getattr(params.response_format, "value", str(params.response_format)),
                "prompt_len": len(params.prompt or ""),
            },
        }
    )
    ...
    try:
        ...
        try:
            ...
            if agent_cls is not None and _AliasClaudeAgent is not None:
                if agent_cls is _AliasClaudeAgent and _ConcreteSkillAgent is not None:
                    ...
                    _finish("error", error="invalid_agent_class", detail=msg)
                    return _response(
            ...
            mode = "sdk" if agent_cls else "anthropic"
            ...
            usage_for_metrics = result.get("usage")
            if not isinstance(usage_for_metrics, dict):
                usage_for_metrics = _normalize_token_usage(usage_for_metrics)
            _finish(
                "ok",
                skill_name=str(skill.get("name", "")),
                mode=mode,
                llm_ms=int(result.get("timing", {}).get("latency_ms") or 0),
                latency_ms=int(result.get("timing", {}).get("latency_ms") or 0),
                input_tokens=usage_for_metrics.get("input_tokens"),
                output_tokens=usage_for_metrics.get("output_tokens"),
                total_tokens=usage_for_metrics.get("total_tokens"),
            )
            ...
            _finish("error", error="anthropic_not_installed", mode=(locals().get("mode") or "anthropic"))
            ...
            try:
                if "start_perf" in locals():
                    llm_ms = int((time.perf_counter() - float(locals()["start_perf"])) * 1000)
            except Exception:
                llm_ms = None
            _finish(
                "error",
                error="api_error",
                mode=str(locals().get("mode") or "unknown"),
                llm_ms=llm_ms,
        ...
        _finish("error", error="exception", error_type=type(e).__name__, error_message=str(e))
        return _handle_error(e)
    ...
    _emit_event(
        {
            ...
            "params": {
                "response_format": getattr(params.response_format, "value", str(params.response_format)),
                "include_skill_names": bool(params.include_skill_names),
                "include_pythonpath": bool(params.include_pythonpath),
            },
        }
    )
    try:
        ...
        info: Dict[str, Any] = {
            ...
            "observability": {
                "event_log_file": (os.getenv("STARTD8_MCP_EVENT_LOG_FILE") or "").strip(),
                "log_json": MCP_LOG_JSON,
        ...
        if "_CONCRETE_SKILL_TOOL_NAMES" in globals():
            try:
                info.setdefault("tools", {})["concrete_skill_tools_registered"] = len(_CONCRETE_SKILL_TOOL_NAMES)
            except Exception:
                pass
            ...
            _emit_event(
                {
                    ...
                    "status": "ok",
                    "format": "json",
                    "skills_count": info.get("skills", {}).get("count"),
                    "startup_errors": len(STARTUP_ERRORS),
                }
            )
...
async def tasks_run(params: TaskRunInput) -> str:
        ...
        _emit_event(
            {
                ...
                "dry_run": bool(params.dry_run),
                "auto": bool(params.auto),
                "agent": (params.agent or DEFAULT_AGENT_NAME),
            }
        )
        return _response(request_id=request_id, **kwargs)
    ...
    try:
        ...
        try:
            ...
            return _resp(error="invalid_params", message=str(e))
        ...
        try:
            ...
            try:
                if _METRIC_TASK_PLAN_DURATION is not None:
                    _METRIC_TASK_PLAN_DURATION.observe(float(time.perf_counter() - plan_started))
            except Exception:
                pass
        ...
        for task_id in execution_order:
            ...
            try:
                if _METRIC_TASK_PROMPT_BUILD_DURATION is not None:
                    _METRIC_TASK_PROMPT_BUILD_DURATION.observe(float(time.perf_counter() - prompt_started))
            except Exception:
                pass
            try:
            ...
            try:
                ...
                try:
                    if _METRIC_TASK_AGENT_GENERATE_DURATION is not None:
                        _METRIC_TASK_AGENT_GENERATE_DURATION.observe(float(time.perf_counter() - gen_started))
                except Exception:
                    pass
            except Exception as e:
                return _resp(
            ...
            try:
                ...
                try:
                    if _METRIC_TASK_ACTION_PARSE_DURATION is not None:
                        _METRIC_TASK_ACTION_PARSE_DURATION.observe(float(time.perf_counter() - parse_started))
                except Exception:
                    pass
            except Exception as e:
                return _resp(
            ...
            try:
                if _METRIC_TASK_APPLY_ACTIONS_DURATION is not None:
                    _METRIC_TASK_APPLY_ACTIONS_DURATION.labels(mode="dry_run").observe(
                        float(time.perf_counter() - apply_started)
                    )
            except Exception:
                pass
            ...
            try:
                if _METRIC_TASK_APPLY_ACTIONS_DURATION is not None:
                    _METRIC_TASK_APPLY_ACTIONS_DURATION.labels(mode="apply").observe(
                        float(time.perf_counter() - apply_started)
                    )
            except Exception:
                pass
        ...
        return _resp(error="internal", message=str(e))
    except Exception as e:
        return _resp(error="internal", message=f"Error running task: {e}")
...
if __name__ == "__main__":
 ...
 |_____/ \__\__,_|_|  |_|\_\_| |_|_____/ \___|\___|_|
    ...
    try:
        ...
        # Emit a structured skills list for external logs (JSONL event log).
        try:
            _emit_event(
                {
                    "event": "startup.skills",
                    "skills_count": len(skills),
                    "skills": [
                        {
                            "name": str(s.get("name", "unknown")),
                            "directory": str(s.get("directory", "")),
                        }
                        for s in skills
                    ],
                }
    ...
    if not MCP_QUIET:
        ...
        # Quiet mode: only print errors (if any)
        if STARTUP_ERRORS:
            _eprint(
    ...
    # Emit a single startup event for external observability (logs/metrics).
    _emit_event(
        {
            ...
            "concrete_skill_resources_registered": len(_CONCRETE_RESOURCE_URIS),
            "concrete_skill_tools_registered": len(_CONCRETE_SKILL_TOOL_NAMES),
            "metrics": dict(_METRICS_STATE),
        }
    )
