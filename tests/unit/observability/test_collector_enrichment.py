...
_FIXTURE = Path(__file__).parent / "fixtures" / "otelcol-business-enrichment.reference.yml"


def _report() -> GenerationReport:
    return GenerationReport(project_id="online-boutique", generated_at="2026-07-23T00:00:00Z")
...
_CRIT = {
    ...
    "high": ["productcatalogservice", "currencyservice", "shippingservice", "quoteservice", "accountingservice"],
    "medium": ["emailservice", "recommendationservice", "adservice", "frauddetectionservice"],
}
_OWNER = {
...
def _boutique_services():
    ...
    return [
        ServiceHints(
            service_id=s, service_name=s, transport="grpc",
            criticality=svc_crit.get(s, ""), owner=svc_owner.get(s),
        )
        for s in allsvc
    ]
...
class TestFR1bExtraction:
    ...
    def test_absent_business_is_empty(self):
        meta = {"instrumentation_hints": {"s": {"transport": "http", "service_name": "s"}}}
        [svc] = extract_service_hints(meta)
...
class TestFR3Emitter:
    def test_generates_expected_shape(self):
        r = generate_collector_enrichment(_boutique_services(), BusinessContext(project_id="ob"), _report())
    ...
    def test_statement_count_is_sum_of_present_pairs(self):
        ...
        stmts = yaml.safe_load(r.content)["processors"]["transform/business"]["trace_statements"][0]["statements"]
        assert len(stmts) == expected

    def test_partial_context_counts_only_present(self):
        services = [
            ServiceHints(service_id="a", service_name="a", criticality="high"),   # 1 stmt
    ...
    def test_presence_gate_skips_when_no_business(self):
        r = generate_collector_enrichment(
            [ServiceHints(service_id="a", service_name="a")], BusinessContext(), _report()
        )
    ...
    def test_uses_real_service_name_not_id(self):
        r = generate_collector_enrichment(
            [ServiceHints(service_id="mastodonweb", service_name="mastodon/web", criticality="high")],
            BusinessContext(), _report(),
        )
    ...
    def test_deterministic_across_shuffled_input(self):
        services = _boutique_services()
        a = generate_collector_enrichment(services, BusinessContext(project_id="ob"), _report()).content
        shuffled = services[:]
        random.Random(7).shuffle(shuffled)
        b = generate_collector_enrichment(shuffled, BusinessContext(project_id="ob"), _report()).content
        assert a == b

    def test_provenance_header_present(self):
        r = generate_collector_enrichment(_boutique_services(), BusinessContext(), _report())
...
class TestFR6Escaping:
    ...
    def test_hostile_owner_roundtrips_intact(self):
        ...
        r = generate_collector_enrichment(
            [ServiceHints(service_id="s", service_name="team/a", criticality="critical", owner=hostile)],
            BusinessContext(), _report(),
        )
        assert r.status == "generated"
...
class TestFR8Validation:
    def test_valid_rows_pass(self):
        validate_collector_enrichment([("s", "criticality", "low"), ("s", "owner", "t")], {"s"})
    ...
    def test_duplicate_pair_raises(self):
        with pytest.raises(CollectorEnrichmentError):
            validate_collector_enrichment([("s", "owner", "x"), ("s", "owner", "y")], {"s"})
    ...
    def test_different_attrs_same_service_ok(self):
        validate_collector_enrichment([("s", "criticality", "high"), ("s", "owner", "t")], {"s"})
...
class TestParity:
    ...
    def test_value_mismatch_detected(self):
        gen = generate_collector_enrichment(
            [ServiceHints(service_id="frontend", service_name="frontend", criticality="critical")],
            BusinessContext(), _report(),
        ).content
        bad_ref = (
            'processors:\n  transform/business:\n    trace_statements:\n    - context: span\n'
        ...
        assert pr.value_mismatch["frontend"]["criticality"] == {"generated": "critical", "reference": "low"}
    ...
    def test_grouping_insensitive(self):
        ...
        gen = generate_collector_enrichment(services, BusinessContext(), _report()).content
        grouped = (
            'processors:\n  transform/business:\n    trace_statements:\n    - context: span\n'
...
class TestWiring:
    ...
    def test_no_artifact_when_no_business_context(self, tmp_path):
        ...
        assert not [a for a in report.artifacts if a.artifact_type == "collector_enrichment"]
