...
class TestRegistryRoundTrip:
    def test_save_load_with_promoted(self, tmp_path):
        reg = ExemplarRegistry(project_id="test")
        reg.add_promoted_template(
            {
                "fingerprint": "go:source:grpc:grpc_server",
                "template_lines": ["line1", "line2"],
                "param_names": ["MyService"],
        ...
        assert len(loaded.promoted_templates) == 1
        assert (
            loaded.promoted_templates[0]["fingerprint"]
            == "go:source:grpc:grpc_server"
        )

    def test_add_promoted_deduplicates(self):
        reg = ExemplarRegistry()
        reg.add_promoted_template(
            {"fingerprint": "go:source:grpc:grpc_server", "version": 1}
        )
        reg.add_promoted_template(
            {"fingerprint": "go:source:grpc:grpc_server", "version": 2}
        )
        assert len(reg.promoted_templates) == 1
        assert reg.promoted_templates[0]["version"] == 2

    def test_mark_template_promoted(self):
        reg = ExemplarRegistry()
        code = "server.Serve()"
        e1 = _make_exemplar("go:source:grpc:grpc_server", code, run_id="run-001")
        e2 = _make_exemplar("go:source:grpc:grpc_server", code, run_id="run-002")
        ...
        reg.mark_template_promoted([e1.id, e2.id])
        for e in reg.exemplars:
            if e.id in [e1.id, e2.id]:
                assert e.maturity == 4

    def test_mark_template_promoted_only_level3(self):
        """Only level-3 exemplars are promoted to level 4."""
        reg = ExemplarRegistry()
        code = "server.Serve()"
        e1 = _make_exemplar(
            "go:source:grpc:grpc_server", code, maturity=2, run_id="run-001"
        ...
        reg.mark_template_promoted([e1.id])
        # Should remain at level 2 since it's not level 3
        for e in reg.exemplars:
            if e.id == e1.id:
                assert e.maturity == 2

    def test_load_without_promoted_templates(self, tmp_path):
        """Loading a registry without promoted_templates key works."""
        reg = ExemplarRegistry(project_id="test")
        path = tmp_path / "registry.json"
        reg.save(path)

        loaded = ExemplarRegistry.load(path)
        assert loaded.promoted_templates == []
