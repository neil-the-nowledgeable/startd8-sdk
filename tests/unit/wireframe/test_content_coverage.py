...
Aggregates the three author→approve surfaces the wireframe already tracks per-item — page bodies,
view copy, and AI prompts — into a `ContentCoverageStats` on the plan and a `content_completeness`
block in the `--json` body (schema_version 2). Visibility only (bucket 2/4), never a gate.

Golden fixture: `about.md` present + `home.md` missing → page bodies 1/2; one AI prompt present →
1/1; one view (`profile_dashboard`) with no `view_prose.yaml` → view copy 0/1 → overall 2/4.
"""
...
def test_rollup_aggregates_the_three_surfaces(golden_root: Path) -> None:
    ...
    assert cov.overall == CoverageStat(2, 4)
...
def test_authoring_view_copy_moves_the_rollup(golden_copy: Path) -> None:
    ...
    assert after.overall == CoverageStat(3, 4)   # the one extra authored surface rolls up
...
def test_json_carries_content_completeness_block(golden_root: Path) -> None:
    ...
    assert set(cc) == {"page_bodies", "view_copy", "ai_prompts", "overall"}
    assert cc["overall"] == {"authored": 2, "total": 4, "ratio": 0.5}
