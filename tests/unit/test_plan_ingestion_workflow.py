...
class TestEndToEnd:
    ...
    def test_requirements_hints_drive_traceability_without_requirements_docs(
        ...
        result = self.wf.run({
            ...
            "project_root": str(tmp_path),  # Prevent cwd fallback finding workspace .contextcore.yaml
            "review_rounds": 0,
        })
    ...
    @patch("startd8.workflows.builtin.architectural_review_log_workflow.ArchitecturalReviewLogWorkflow")
    @patch("startd8.workflows.builtin.plan_ingestion_workflow.resolve_agent_spec")
    def test_traceability_artifact_includes_refine_impact(
        self, mock_resolve, MockReviewWf, tmp_path
    ):
        """After REFINE runs, traceability artifact includes refine_impact delta."""
        plan_file = tmp_path / "plan.md"
        plan_file.write_text(SAMPLE_PLAN)
        ...
        # Simulate REFINE producing one round of successful review.
        mock_review_result = MagicMock()
        mock_review_result.success = True
        mock_review_result.metrics = MagicMock(total_cost=0.05)
        mock_review_result.steps = [
            MagicMock(
                step_name="review:round-1",
                agent_name="test-reviewer",
                input="...",
                output="Applied 2 suggestions",
                time_ms=100,
                input_tokens=50,
                output_tokens=25,
                cost=0.05,
        ...
        # refine_impact should be present since rounds_completed > 0.
        assert "refine_impact" in trace
        impact = trace["refine_impact"]
        assert impact["rounds_applied"] >= 1
        assert "requirements_coverage_before" in impact
        assert "requirements_coverage_after" in impact
        assert "artifact_mapping_before" in impact
        assert "artifact_mapping_after" in impact

    @patch("startd8.workflows.builtin.plan_ingestion_workflow.resolve_agent_spec")
    def test_traceability_no_refine_impact_when_zero_rounds(
        self, mock_resolve, tmp_path
    ):
        """When refine rounds=0, traceability should NOT have refine_impact."""
        plan_file = tmp_path / "plan.md"
        plan_file.write_text(SAMPLE_PLAN)
        ...
        assert result.success
        trace = json.loads(Path(result.output["traceability_path"]).read_text())
        assert "refine_impact" not in trace

    @patch("startd8.workflows.builtin.plan_ingestion_workflow.resolve_agent_spec")
    def test_prime_output_includes_requirement_ids(self, mock_resolve, tmp_path):
        """Prime YAML tasks are enriched with requirement traceability fields."""
        plan_file = tmp_path / "plan.md"
        plan_file.write_text(SAMPLE_PLAN)
        req_file = tmp_path / "requirements.md"
        req_file.write_text(
            "## Requirements\n"
            "- REQ-201: Widget core must handle errors gracefully\n"
        )

        # Parse response that mentions REQ-201 in feature description.
        parse_with_req = json.dumps({
            "title": "My Sample Plan",
            ...
            "features": [
                {
                    "feature_id": "F-001",
                    "name": "Widget core",
                    "description": "Implement REQ-201 error handling in core widget",
                    "target_files": ["src/widget.py"],
                    "dependencies": [],
                    "estimated_loc": 100,
                    "labels": ["core"],
        ...
        # Prime YAML with task that references the feature.
        prime_yaml = textwrap.dedent("""\
            project:
              id: sample
              name: Sample Project
              sprint_id: sprint-1
            ...
            tasks:
              - task_id: PI-001
                ...
                config:
                  task_description: |
                    Implement the core widget with error handling.
        """)

        agent = _make_mock_agent()
        agent.generate.side_effect = [
            _mock_generate_return(parse_with_req, cost=0.01),
            _mock_generate_return(ASSESS_JSON_PRIME, cost=0.02),
            _mock_generate_return(prime_yaml, cost=0.03),
        ]
        mock_resolve.return_value = agent
        ...
        # At least the first task should have requirement_ids in context.
        enriched_task = tasks[0]
        ctx = enriched_task.get("config", {}).get("context", {})
        assert "requirement_ids" in ctx, (
            f"Expected requirement_ids in Prime task context, got keys: {list(ctx.keys())}"
        )
        assert "REQ-201" in ctx["requirement_ids"]

    @patch("startd8.workflows.builtin.plan_ingestion_workflow.resolve_agent_spec")
    def test_preflight_report_path_in_output(self, mock_resolve, tmp_path):
        """Workflow output includes preflight_report_path."""
        plan_file = tmp_path / "plan.md"
        plan_file.write_text(SAMPLE_PLAN)
        ...
        assert result.success
        assert "preflight_report_path" in result.output
        report_path = Path(result.output["preflight_report_path"])
