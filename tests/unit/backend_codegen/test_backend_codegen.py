...
def test_tampered_detail_states_fact_and_both_causes():
    """F-2 (strtd8 2026-06-06): a byte-diff with a matching schema hash may be a hand-edit OR a
    spine generated by a different invocation (e.g. without --completeness). The detail must
    state the fact (differs from a fresh render) and list both causes — never assert
    hand-editing as fact."""
    gen = render_pydantic_models(PILOT_SCHEMA).text
    tampered = gen.replace("    value: float", "    value: int", 1)
    result = check_drift(PILOT_SCHEMA, tampered)
    assert result.status == "tampered"  # status vocabulary stays stable
    assert "differs from a fresh render" in result.detail
    assert "hand-edited, or generated by a different invocation" in result.detail
    assert "was hand-edited" not in result.detail
