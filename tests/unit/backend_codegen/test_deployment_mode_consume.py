...
def test_db_tolerantly_imports_and_consumes_settings():
    ...
    assert "except ImportError:" in db  # installed fallback (no settings module) — catches the
    # 'cannot import name settings' ImportError too, not just ModuleNotFoundError (boot-smoke fix)
    for call in (
